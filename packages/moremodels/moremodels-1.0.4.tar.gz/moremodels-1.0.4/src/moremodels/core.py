...
class WeightedModels:
    ...
    def fit(self, X, y, X_val = None, y_val = None ,showScores = True):
        ...
        for model in self.__models:
            if ('eval_set' in model.fit.__code__.co_varnames) and (X_val is not None) and (y_val is not None):
                model.fit(self.__X_train, self.__y_train, eval_set = [(self.__X_train, self.__y_train), (X_val, y_val)])
            else:
