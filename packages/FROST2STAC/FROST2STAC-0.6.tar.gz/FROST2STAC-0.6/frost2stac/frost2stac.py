...
class Converter(object):
    ...
    def id_detector(self, url):
        observation_json = funcs.json_reader(
            funcs.Things_url(url), constants.featureofinterest_id
        )
        ...
        print(self.Things_FeatureOfInterest_id)

    def datetime_catch(self, url):
        phenomenonTime_json = funcs.json_reader(
            funcs.Things_url(url), constants.phenomenonTime_string
        )
    ...
    def stac_creator(self, url):
        ...
        for i in self.Things_FeatureOfInterest_id:
            thing_json = funcs.json_reader(
                funcs.Things_url(url), "(" + str(i[0]) + ")"
            )
            featureofinterest_json = funcs.json_reader(
                funcs.FeaturesOfInterest_url(url), "(" + str(i[1]) + ")"
            )
            ...
            spatial_extent = pystac.SpatialExtent(
                bboxes=[featureofinterest_json["feature"]["coordinates"]]
            )
