...
def to_proto(transform):
    ...
    elif _should_serialize_function(transform):
    ...
    return UserDefinedFunction(name=transform.__name__, body=code)
...
def _should_serialize_function(func):
    ...
    """
    if not conf.get_bool("UNIFIED_TECTON_OBJECTS_ENABLED"):
        # If notebook dev is not enabled, always serialize functions.
        return True
