...
class PushSource(BaseDataSource):
    """
    Declare a ``PushSource``, used to push events in real time into Tecton’s online store via an online Ingest API.
    ``PushSource`` is currently in private preview, please contact support if you are interested in participating in the preview.

    ``StreamFeatureViews`` ingest data from PushSources. A PushSource may also contain an optional batch config for backfilling.
    """
    ...
    def __init__(
        ...
        *,
        ...
        tags: Optional[Dict[str, str]] = None,
        owner: Optional[str] = None,
        batch_config: Optional[Union[FileConfig, HiveConfig, RedshiftConfig, SparkBatchConfig, SnowflakeConfig]] = None,
    ) -> None:
        """
        ...
        basic_info = prepare_basic_info(name=name, description=description, owner=owner, family=None, tags=tags)
        data_source_type = DataSourceType.PUSH_WITH_BATCH if batch_config is not None else DataSourceType.PUSH_NO_BATCH
        args = prepare_ds_args(
            basic_info=basic_info, batch_config=batch_config, stream_config=None, ds_type=data_source_type
        )

        args.schema.CopyFrom(_prepare_schema(input_schema=schema))
        self._args = args
        self._data_delay = batch_config.data_delay if batch_config is not None else None
        self._source_info = construct_fco_source_info(args.virtual_data_source_id)
