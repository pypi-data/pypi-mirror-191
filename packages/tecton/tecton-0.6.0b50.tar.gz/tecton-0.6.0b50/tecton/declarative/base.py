...
class FeatureReference:
    ...
    def with_join_key_map(self, join_key_map: Dict[str, str]) -> "FeatureReference":
        ...
        self.override_join_keys = {spine_key: fv_key for fv_key, spine_key in join_key_map.items()}
        return self
...
class BaseFeatureDefinition(Fco):
    ...
    def with_join_key_map(self, join_key_map: Dict[str, str]) -> FeatureReference:
        ...
        # As of FWV5, we map from spine key to feature view key. For backwards compatibility with FWV3,
        # self.override_join_keys is a map from feature view key to spine key. So we need to flip the input dict.
        override_join_keys = {spine_key: fv_key for fv_key, spine_key in join_key_map.items()}
        return FeatureReference(feature_definition=self, override_join_keys=override_join_keys)
