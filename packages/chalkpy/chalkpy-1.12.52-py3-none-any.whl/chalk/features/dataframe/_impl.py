...
class DataFrameImpl(metaclass=DataFrameMeta):
    ...
    def __init__(
        ...
        data
            The data. Can be an existing Pandas DataFrame, Polars DataFrame or LazyFrame,
            a sequence of feature instances, or a dict mapping a feature to a sequence of values.
        missing_value_strategy
            The strategy to use to handle missing values.

            A feature value is "missing" if it is an ellipsis (``...``), or it is ``None`` and the feature is not
            annotated as ``Optional[...]``.

            The available strategies are:
                ``'error'``: Raise a ``TypeError`` if any missing values are found. Do not attempt to replace missing values with the default
                    value for the feature.
                ``'default_or_error'``: If the feature has a default value, then replace missing values with the default value for the feature.
                    Otherwise, raise a ``TypeError``.
                ``'default_or_allow'``:  If the feature has a default value, then replace missing values with the default value for the feature.
                    Otherwise, leave it as None. This is the default strategy.
                ``'allow'``: Allow missing values to be stored in the dataframe.
                    This option may result non-nullable features being assigned ``None`` values.
        """
