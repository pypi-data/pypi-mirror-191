...
class ChalkQueryProtocol(Protocol):
    ...
    def incremental(
        ...
        ----------
        incremental_column
            Defaults to the column mapped to the `feature_time` feature for this feature class.
            This should reference a timestamp column in your underlying table, typically something
            like "updated_at", "created_at", "event_time", etc.
        mode
            Defaults to "row", which indicates that only "rows" newer than the last observed row should be considered.
