...
class workflow_lambda_host:
    ...
    def publish_reports(self, workflow_request: request, report_name:str, report_id: str, constant_spatial_id: str, constant_object_id: str, propertyIds:tuple, timestampID: str = None, spatialId: str = None, objectId: str = None):
        ...
        mesh_entity = self.lambda_framework.get_entity(report_id, workflow_request.project_id)
        if not mesh_entity:
        ...
        self.handle_dataframe(workflow_request, result['df'], f"{iso_time}", timestampID, spatialId, objectId, propertyIds, constant_spatial_id, constant_object_id)
        self.lambda_framework.log(f'End process for report {report_id}')    
        return {'success': True, 'id': result.id, 'error_info': 'None'}

    def handle_dataframe(self, workflow_request: request, df_model:pd.DataFrame, timestamp:str, timestampId:str, spatialId: str, objectId: str, propertyIds:tuple, constant_spatial_id:str, constant_object_id:str):
        ...
        if timestampId in df_model or timestamp is not None:
            ...
            local_dump_tsi_file = os.path.join(workflow_request.scrap_folder, "dump_tsi.csv")
            df_dump = pd.DataFrame(eventsToSend) 
            df_dump.to_csv(local_dump_tsi_file, index=False)
            #TODO, call the C# method instead of python submit_time_series_events() call.
            #self.lambda_framework.publish_feed("dump_tsi.csv")
        else:
