...
class workflow_lambda_framework:
    ...
    def publish_feed(self, entity_id, timestamp, timestamp_id, spatialId, objectId, defaultObjectId, propertyIds, constant_spatial_id, constant_object_id):
        return {'success': True}
...
class workflow_lambda_host:
    ...
    def publish_feed(self, entity_id, timestamp, timestamp_id, spatialId, objectId, defaultObjectId, propertyIds, constant_spatial_id, constant_object_id):
        ...
        result = self.lambda_framework.publish_feed(entity_id, timestamp, timestamp_id, spatialId, objectId, defaultObjectId, propertyStr, constant_spatial_id, constant_object_id)
        if result == '':
            return {'success': True}
        else:
            return {'success': False, 'error_info': result}

    def publish_reports(self, workflow_request : request, report_id:str, spatialId: str, objectId: str, propertyIds:tuple):
        ...
        for val in propertyIds:
            self.lambda_framework.log(val[0] + " : " + val[1])
        ...
        if mesh_id != "":
            ...
            apiResult = self.publish_feed(
                ...
                constant_spatial_id=spatialId, 
                constant_object_id=objectId)    

            success = apiResult['success']
            if success:
                self.lambda_framework.log('Done publishing feed')    
                return {'success': True}
            else:
                return {'success': False, 'error_info': apiResult.error_info}
        else:
            self.lambda_framework.log(f'mesh_id was empty.')    
            return {'success': False, 'error_info': 'mesh_id was empty.'}
