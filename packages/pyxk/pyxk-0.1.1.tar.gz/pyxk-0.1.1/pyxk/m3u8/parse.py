...
class M3U8ParseInit:
    ...
    def filename(self, value):
        if value is None:
            ...
            raise TypeError(
                ...
                f", can't be a {type(value).__name__!r}\033[0m")
        value = self.rename(value, self.filepath)[0]
    ...
    @staticmethod
    def rename(filename, filepath, suffix="mp4"):
        """
        重命名本地存在的文件
        """
        ...
        for i in itertools.count(1):
            if not os.path.isfile(file):
                break
            filename = filename.removesuffix(suffix) + f".{i}{suffix}"
            file = os.path.join(filepath, filename)
        return filename, file

    @staticmethod
    def _human_playtime_pr(playtime):
        if not isinstance(playtime, (str, float)):
            return None
...
class M3U8Parse(M3U8ParseInit):
    ...
    def probe(self, uri, **kwargs):
        """
        测试 m3u8 链接
        """
        self.load(uri, None, None, True, False, False, **kwargs)
    ...
    async def __download(self, segments, headers=None):
        ...
        if self._m3u8keys:
            for index, key in self._m3u8keys.items():
                cipher[index] = aes.AESCryptor(**key[1])
    ...
    def __m3u8_paring(self, m3u8obj, rereq=False, sava=True, **kwargs):
        ...
        _display = [
            ...
            f"PlayTime: {self._human_playtime_pr(self._duration)}",
