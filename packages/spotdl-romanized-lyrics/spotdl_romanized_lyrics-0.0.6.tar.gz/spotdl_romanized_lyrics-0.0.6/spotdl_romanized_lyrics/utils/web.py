...
from spotdl_romanized_lyrics.download.progress_handler import NAME_TO_LEVEL, ProgressHandler, SongTracker

from spotdl_romanized_lyrics.types.song import Song
from spotdl_romanized_lyrics.types.album import Album
from spotdl_romanized_lyrics.download.downloader import Downloader
from spotdl_romanized_lyrics.utils.search import get_search_results
from spotdl_romanized_lyrics.utils.config import get_spotdl_romanized_lyrics_path
...
async def download_url(
    ...
    state.downloader.output = str(
        (get_spotdl_romanized_lyrics_path() / f"web/sessions/{client_id}").absolute()
    )
...
async def download_file(file: str, client_id: str) -> FileResponse:
    ...
    return FileResponse(
        str((get_spotdl_romanized_lyrics_path() / f"web/sessions/{client_id}/{file}").absolute()),
        filename=file,
    )
...
def fix_mime_types():
    ...
    incorrect values, which causes spotdl_romanized_lyrics to fail to render on
    the frontend.
    ...
    # https://github.com/spotdl_romanized_lyrics/spotify-downloader/issues/1540
    mimetypes.add_type("application/javascript", ".js")

    # Not known to be problematic, but used by spotdl_romanized_lyrics:
