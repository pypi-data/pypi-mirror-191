...
from spotdl_romanized_lyrics.utils.formatter import create_file_name
from spotdl_romanized_lyrics.utils.m3u import gen_m3u_files
...
def sync(
    ...
    **_,
    ...
    if query and save_path:
        if any(req for req in query if req.endswith(".spotdl_romanized_lyrics")):
            # If the query contains a .spotdl_romanized_lyrics file, and we are about to create
            # .spotdl_romanized_lyrics file, raise an error.
            raise ValueError(
                "Cannot create a sync file with a .spotdl_romanized_lyrics file in the query."
            )
    ...
    if len(query) == 1 and query[0].endswith(".spotdl_romanized_lyrics") and not save_path:
        # Load the sync file
