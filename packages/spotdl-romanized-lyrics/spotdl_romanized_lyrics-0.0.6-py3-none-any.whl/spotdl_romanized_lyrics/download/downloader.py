...
from spotdl_romanized_lyrics.providers.translate import Lyricstranslate
...
class Downloader:
    ...
    def __init__(
        ...
        for lyrics_provider_class in lyrics_providers_classes:
            self.lyrcs_providers.append(lyrics_provider_class())
    ...
    def search_lyrics(self, song: Song, romanized: bool) -> Optional[str]:
        """
        ...
        for lyrics_provider in self.lyrics_providers:
            lyrics = lyrics_provider.get_lyrics(song.name, song.artists, romanized)
            if lyrics:
                self.progress_handler.debug(
    ...
    def translation(self, song: Song) -> Optional[str]:
        for translate_provider in self.translate_providers:
            translate = translate_provider.get_translate(song.name, song.artists)
            if translate:
                return translate
            return None
    ...
    def search_and_download(self, song: Song) -> Tuple[Song, Optional[Path]]:
        ...
        lyrics = self.search_lyrics(song, self.romanized)
        if lyrics is None:
            self.progress_handler.debug(
