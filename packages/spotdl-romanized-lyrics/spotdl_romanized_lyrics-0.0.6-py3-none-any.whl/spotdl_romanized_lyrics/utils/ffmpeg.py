...
from spotdl_romanized_lyrics.utils.formatter import to_ms
...
def get_local_ffmpeg() -> Optional[Path]:
    ...
    ffmpeg_path = Path(get_spotdl_romanized_lyrics_path()) / (
...
def download_ffmpeg() -> Path:
    """
    Download ffmpeg binary to spotdl_romanized_lyrics directory.
    ...
    ffmpeg_path = Path(
        os.path.join(
            get_spotdl_romanized_lyrics_path(), "ffmpeg" + (".exe" if os_name == "windows" else "")
        )
    )
    ...
    # Download binary and save it to a file in spotdl_romanized_lyrics directory
