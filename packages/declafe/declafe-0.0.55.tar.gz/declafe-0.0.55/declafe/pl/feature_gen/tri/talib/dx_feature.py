...
class DXFeature(TriFeature):
  ...
  def _tri_expr(self, col1: pl.Expr, col2: pl.Expr, col3: pl.Expr) -> pl.Expr:
    return cast(pl.Expr, pl.struct([
        col1, col2, col3
    ])).map(lambda s: talib.DX(s.struct.field(self.col1_feature.feature_name),
  ...
  def _feature_names(self) -> list[str]:
    return [
        f'DX({self.timeperiod})({self.col1_feature.feature_name}, {self.col2_feature.feature_name}, {self.col3_feature.feature_name})'
    ]
