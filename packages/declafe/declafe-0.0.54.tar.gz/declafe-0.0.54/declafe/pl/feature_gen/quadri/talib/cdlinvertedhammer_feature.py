...
class CDLINVERTEDHAMMERFeature(QuadriFeature):
  ...
  def _quadri_expr(self, col1: pl.Expr, col2: pl.Expr, col3: pl.Expr,
    ...
    return cast(pl.Expr, pl.struct(
        [col1, col2, col3, col4])).map(lambda s: talib.CDLINVERTEDHAMMER(
  ...
  def _feature_names(self) -> list[str]:
    return [
        f'CDLINVERTEDHAMMER()({self.col1_feature.feature_name}, {self.col2_feature.feature_name}, {self.col3_feature.feature_name}, {self.col4_feature.feature_name})'
    ]
