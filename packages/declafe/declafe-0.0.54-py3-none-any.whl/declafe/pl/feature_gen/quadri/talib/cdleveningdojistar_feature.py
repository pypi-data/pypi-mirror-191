...
class CDLEVENINGDOJISTARFeature(QuadriFeature):
  ...
  def __init__(self, open: ColLike, high: ColLike, low: ColLike, close: ColLike,
               penetration: float):
  ...
  def _quadri_expr(self, col1: pl.Expr, col2: pl.Expr, col3: pl.Expr,
    ...
    return cast(pl.Expr, pl.struct(
        [col1, col2, col3, col4])).map(lambda s: talib.CDLEVENINGDOJISTAR(
  ...
  def _feature_names(self) -> list[str]:
    return [
        f'CDLEVENINGDOJISTAR({self.penetration})({self.col1_feature.feature_name}, {self.col2_feature.feature_name}, {self.col3_feature.feature_name}, {self.col4_feature.feature_name})'
    ]
