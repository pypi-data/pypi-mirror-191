                     ...
                     int64_t    offset )
{
    ...
    else
    {
        ...
        UTILS_DEBUG( "sion_seek( sid=%d, offset=" PRId64 ", SION_SEEK_BEGIN )",
                     sion_file->sion_id, sion_file->rank, offset );
        sion_status = sion_seek( sion_file->sion_id,
                                 offset,
                                 SION_SEEK_BEGIN );
        UTILS_DEBUG( "sion_seek( sid=%d, offset=" PRId64 ", SION_SEEK_BEGIN ) = %d",
...
otf2_file_sion_seek_current_rank( otf2_file_sion* sionFile )
{
    ...
    {
#if SION_MAIN_VERSION >= 2
        UTILS_DEBUG( "sion_switch_logical_file( sid=%d, rank=%d )",
                     sionFile->sion_id,
                     sionFile->rank );
        status = sion_switch_logical_file( sionFile->sion_id, sionFile->rank );
        UTILS_DEBUG( "sion_switch_logical_file( sid=%d, rank=%d ) = %d",
                     sionFile->sion_id,
                     sionFile->rank,
                     status );
#else
