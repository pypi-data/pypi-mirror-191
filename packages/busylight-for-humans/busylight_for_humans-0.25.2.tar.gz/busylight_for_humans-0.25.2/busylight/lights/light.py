...
class Light(abc.ABC, TaskableMixin):
    ...
    @classmethod
    def supported_device_ids(cls) -> Dict[Tuple[int, int], List[str]]:
        """A dictionary of device identfiers supported by this class.

        Keys are a tuple of integer vendor_id and product id, values
        are the lists of marketing names associated with that
        device. Some tuples may identify devices with the same name.
        """
        supported = {}
        for subclass in cls.subclasses():
            for (vid, pid), value in subclass.supported_device_ids().items():
                supported.setdefault((vid, pid), []).append(value)
        return supported
