...
class Webapp:
    def __init__(self, domain: str) -> None:
        self.base_url = get_api_endpoint().format(username=getpass.getuser(), flavor="webapps")
        self.domain = domain
        self.domain_url = f"{self.base_url}{self.domain}/"
    ...
    def create(self, python_version: str, virtualenv_path: Path, project_path: Path, nuke: bool) -> None:
        ...
        response = call_api(
            self.base_url, "post", data={"domain_name": self.domain, "python_version": PYTHON_VERSIONS[python_version]}
        )
    ...
    def delete_log(self, log_type: str, index: int = 0) -> None:
        ...
        base_log_url = f"{get_api_endpoint().format(username=getpass.getuser(), flavor='files')}path/var/log/{self.domain}.{log_type}.log"
