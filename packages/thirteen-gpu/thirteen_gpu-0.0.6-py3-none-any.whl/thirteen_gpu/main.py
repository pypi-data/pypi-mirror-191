...
def main():
    ...
    while True:
        ...
        for project_path in glob(f"{WORKSPACE_DIR}/*"):
            user = os.system(f"cat {project_path}/user.txt")
            submit_at = os.system(f"cat {project_path}/submit_at.txt")
        ...
        jobs = [job for project in projects for job in project.jobs.values() if job.status.value == JobStatus.PENDING.value]
