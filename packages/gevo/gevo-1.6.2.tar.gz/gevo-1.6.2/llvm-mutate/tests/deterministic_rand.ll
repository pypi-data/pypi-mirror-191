; RUN: llvm-mutate -f %s -p Rand,Rand -p Rand,A1 -p U8.OP0,Rand -d --seed 2 2>&1 | filecheck %s
; CHECK:        oprepl failed(Rand,Rand). cannot find an OP that has the same type as U6
; CHECK:        opreplaced U6.OP1,A1
; CHECK:        opreplaced U8.OP0,C1
...
; CHECK:        %U6 = fadd contract float %U4, %A1
