...
def subsample(file, n_trees, n_required, subp = True):
    ...
    while len(interesting_points) < n_required:
        ...
        if d_MD1_P > d_MD1_MD2: 
            ...
            trees.pop(P_tree)
            if random.randint(0, 10) > 5: MD2, MD2_prep = P, maple_RF.prepareTreeComparison(P, rooted=False)
            else: MD1, MD1_prep = P, maple_RF.prepareTreeComparison(P, rooted=False)
            interesting_points.append(P_tree)
            idxs.append(P_idx)
