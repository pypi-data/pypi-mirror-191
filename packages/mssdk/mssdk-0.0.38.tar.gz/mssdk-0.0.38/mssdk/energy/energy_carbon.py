...
def energy_carbon_bj() -> pd.DataFrame:
    ...
    for i in tqdm(range(1, int(total_page)+1), desc="Please wait for a moment", leave=False):
        if i == 1:
            i = ""
    ...
    temp_df['成交单位'] = temp_df['成交额'].str.split("(", expand=True).iloc[:, 1].str.split("）", expand=True).iloc[:, 0].str.split(")", expand=True).iloc[:, 0]
    temp_df['成交额'] = temp_df['成交额'].str.split("(", expand=True).iloc[:, 0].str.split("（", expand=True).iloc[:, 0]
    temp_df['成交量'] = pd.to_numeric(temp_df['成交量'])
...
def energy_carbon_sz() -> pd.DataFrame:
    ...
    for page in tqdm(range(2, page_num+1), desc="Please wait for a moment", leave=False):
...
def energy_carbon_hb() -> pd.DataFrame:
    ...
    big_df = pd.DataFrame()
    for page in tqdm(range(1, page_num+1), desc="Please wait for a moment", leave=False):
