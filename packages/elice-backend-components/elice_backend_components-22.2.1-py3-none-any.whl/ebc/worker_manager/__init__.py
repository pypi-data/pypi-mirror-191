...
class _PrinterType(Protocol):
    def __call__(
        self, prefix: str, txt: str, *, is_stdout: bool = False
    ) -> None:
        ...
...
def _process_launcher(
    ...
    for idx in range(worker_config.num_workers):
        ...
        printer(
            ...
            (
                "Started a process"
                f" (PID {pwrapper.pid}, PGID {pwrapper.pgid})"
            ),
        )
...
def run(worker_config_dict: WorkerConfigDict) -> None:
    ...
    def _signal_handler(signum: int, frame: FrameType | None) -> None:
        ...
        term_event.set()
        print(
            "Signal handler called with signal :", term_signal.name, flush=True
        )
    ...
    for _, target_worker_entries in sorted(
        stop_order_to_worker_entries.items()
    ):
