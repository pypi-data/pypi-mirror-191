...
class OculScholarsPortalPreservation(models.Model):
    ...
    def import_data(url, bucket="", s3client=None):
        # download the data file from S3 bucket
        xml_file = utils.download_remote(False, OculScholarsPortalPreservation,
                                         url, bucket=bucket, s3client=s3client,
                                         decode=False)
    ...
    def create_preservation(output) -> None:
        ...
        publisher, created = \
            Publisher.objects.get_or_create(name=output['publisher'])

        OculScholarsPortalPreservation.objects.create(
            issn=output['issn'], title=output['title'],
            preserved_volumes=output['volume'],
            preserved_no=output['issue'], preserved=output['status'],
            verified=output['verification'], publisher=publisher
        )

        logging.info(f'Added {output["title"]} to '
