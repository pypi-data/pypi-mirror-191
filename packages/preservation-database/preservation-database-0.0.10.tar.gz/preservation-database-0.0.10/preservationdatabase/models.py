...
class OculScholarsPortalPreservation(models.Model):
    ...
    def create_preservation(output) -> None:
        ...
        publisher = 'Undefined' if 'publisher' not in output \
                                   or output['publisher'] is None \
            else publisher = output['publisher']
        ...
        for volume, issue in zip(volumes, issues):
            ...
            OculScholarsPortalPreservation.objects.create(
                ...
                preserved=output['status'], verified=output['verification'],
                publisher=publisher
            )

            logging.info(f'Added {output["title"]} ({volume}.{issue}) to '
