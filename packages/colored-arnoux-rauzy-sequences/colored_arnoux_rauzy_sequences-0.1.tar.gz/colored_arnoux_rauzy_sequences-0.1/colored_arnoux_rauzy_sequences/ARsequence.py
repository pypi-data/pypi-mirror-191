...
class ArnouxRauzySubstitutiveSequence:
    ...
    def product_betas(self, i, k_1, k_2):
        d = self._d
        p = len(self._period)
        if k_1 < 1 or k_2 > d-1:
            raise TypeError("There is a bug.") 
            return None
        betas = self.betas()
        product = 1
    ...
    def nominators_L(self):
        ...
        for i in range(p):
            ...
            for j in range(2, d+1):
                index = (i-j+1) % p
                nominator += ((d-j)*a[index] + 1)*self.product_betas(i, j, d-1)
            nominators.append(nominator)
        return nominators

    def denominators_L(self):
        d = self._d
        a = self._period
        p = len(self._period)
        denominators = []
        for i in range(p):
            denominator = self.product_betas(i, 1, d-1)
    ...
    def asymptotic_critical_exponent(self):
        ...
        betas = self.betas()
        limits = []
        nominators_L = self.nominators_L()
        denominators_L = self.denominators_L()
        for index in range(p):
            L = a[index] + Integer(1)/(d-1)*nominators_L[index]/denominators_L[index]
            limits.append(L)
