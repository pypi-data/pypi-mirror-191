...
def main():
    ...
    parser = argparse.ArgumentParser(formatter_class=argparse.ArgumentDefaultsHelpFormatter,description=f'ALASPO: ASP + Large-Neighborhood Search (version {__version__})')
    ...
    if args.solver_type == 'clingo':
        ...
        internal_solver = solver.ClingoDl(minimize_variable=args.minimize_variable, seed=seed_value)
    ...
    if args.config_file != None:
        with open(args.config_file, 'r') as f:
            con = f.read()
            strat, initial_operator, relax_operators, search_operators = json_config.parse_config(con, internal_solver)
        ...
        strat, initial_operator, relax_operators, search_operators = json_config.parse_config(json_config.DEFAULT_CONFIG, internal_solver)
    ...
    print('Solving...')
    lns_solver = lns.ClingoLNS(internal_solver, program, initial_operator, relax_operators, search_operators, strat)
    ...
    def signal_handler(sig, _):
        ...
        if verbose > 0:
            print_solution(lns_solver.best_solution, lns_solver.search_complete)
