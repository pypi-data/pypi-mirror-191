...
class DynamicStrategy(AbstractStrategy):
    ...
    def prepare(self, relax_operators, search_operators):
        ...
        logger.debug(f'search operators: {[ o.name() for o in search_operators ]}')
    ...
    def __select_new_pair(self):
        ...
        logger.debug(f'search operator: {self.__current_search_operator.name()}')
...
class RouletteStrategy(AbstractStrategy):
    ...
    def update_weights(self, operators, ratio):
        ...
        logger.debug(f'updating weight of {(operators[0].name(), operators[1].name())}: {self._weights[operators]} -> {new_weight}')
