...
class ClingoSearchOperator(AbstractSearchOperator):
    ...
    def execute(self, assumptions, time_left):
        ...
        logger.debug(f'operator "{self.__name}" executing search for {timeout} seconds')
    ...
    def flatten(self):
        ...
        for timeout in self.__timeouts:
            operators += [ ClingoSearchOperator(internal_solver=self.__internal_solver, name=self.__name, configuration=self.__configuration, timeouts=[timeout]) ]
