...
RTValue TorchInferOp::FromIValue(const torch::jit::IValue& i_val) const {
  ...
  if (i_val.isTuple()) {
    HVM_CHECK(false) << "Tuple is only supported as first level output";
  }
  ...
  HVM_CHECK(false) << "can't convert " << *(i_val.type()) << " to RTValue";
  return RTValue();
}
...
IValueType TorchInferOp::ToIValue(const Any& rt_value) const {
  ...
  if (type_code == TypeIndex::kRuntimeADT)  {
    ...
    HVM_CHECK(obj.tag() == 0) << "can't covert ADT[tag=" << obj.tag() << "] to IValue";
    int num = obj.size();
  ...
  */
  HVM_CHECK(false) << "can't convert " << TypeIndex2Str(type_code) << " to IValue";
...
void TorchInferOp::Init() {
  ...
  if (device_ == NONE_DEVICE) {
    if (has_device_attr) {
      ...
      HVM_LOG(INFO) << "[TorchInferOp] use devices: " << use_device;
    }
  } else {
    if (has_device_attr && (op_device_id < 0)) {
      use_device = -1;  // don't use session's device when the mode is on cpu
      HVM_LOG(INFO) << "[TorchInferOp] use cpu devices: " << use_device;
    } else {
      use_device = device_;
      HVM_LOG(INFO) << "[TorchInferOp] use session devices: " << use_device;
    }
  }
  th_model_ = std::dynamic_pointer_cast<TorchModel>(belong_to_->FindOp("TorchModel", model));
  HVM_CHECK(th_model_ != nullptr) << "cnn't find model: " << model;
  sub_ops_ = {th_model_};
  if (use_device >= 0) {
  ...
  HVM_CHECK(engine_ != nullptr) << "init engine failed!";
}
...
RTValue TorchInferOp::Process(PyArgs inputs) const {
  ...
  try {
    ...
    HVM_THROW << "PyTorch forward failed: " << e.what();
  }
  if (ival_output.isTuple()) {
    auto& elements = (ival_output.toTuple())->elements();
    HVM_CHECK(!elements.empty()) << "model output is empty tuple";
    if (elements.size() == 1) {
