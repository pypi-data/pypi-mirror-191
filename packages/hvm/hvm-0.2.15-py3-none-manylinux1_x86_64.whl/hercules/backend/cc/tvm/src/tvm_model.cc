...
void TVMEngine::init(Options opt) {
  ...
  if (!FileUtil::Exists(params_path) && !FileUtil::Exists(graph_path)) {
    ...
    HVM_THROW << "TVM model not found or incomplete at " << graph_path;
  }

  // INFO("load tvm module input data...");
  num_outputs_ = module_.GetFunction("get_num_outputs")();
  HVM_CHECK_EQ(options_.output_names.size(), num_outputs_);

  func_run_ = module_.GetFunction("run");
  HVM_CHECK(func_run_ != nullptr) << "[TVMEngine] run function is not found";
  func_get_output_ = module_.GetFunction("get_output");
  HVM_CHECK(func_get_output_ != nullptr) << "[TVMEngine] get_output function is not found";
  func_get_input_ = module_.GetFunction("get_input");
  HVM_CHECK(func_get_input_ != nullptr) << "[TVMEngine] get_input function is not found";

  HVM_LOG(INFO) << "[TVMEngine] finish init";
}
