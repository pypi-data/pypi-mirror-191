...
void TVMInferOp::Init() {
  ...
  if (device_ == NONE_DEVICE) {
    if (HasAttr("device")) {
      ...
      HVM_LOG(INFO) << "[TVMInferOp] devices not config, use default device: " << options_.device;
      options_.device = -1;
    }
  } else {
    options_.device = device_;
    HVM_LOG(INFO) << "[TVMInferOp] use devices: " << options_.device;
  }
  ...
  for (auto& item : models) {
    ...
    HVM_CHECK(!model_name.empty()) << "[TVMInferOp] model_name is empty";
...
RTValue TVMInferOp::Process(PyArgs inputs) const {
  ...
  for (auto& input : inputs) {
    ...
    for (auto items : input_dict.items()) {
      ...
      switch (items.first.type_code()) {
        ...
        default: {
          /* not compatible type */
          HVM_CHECK(false) << "[TVMInferOp] input type error, \n"
                         << "optional: Dict[bytes, NDArray] or Dict[str, NDArray], \n"
  ...
  HVM_CHECK(engine_itr != engines_.end()) << "[TVMInferOp] not supported batch size: " << batch_size;
