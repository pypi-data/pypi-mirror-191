...
#define HERCULES_CHECK_FT_DICT_ARGS(FuncName, NumArgs)                                  \
  HVM_CHECK(NumArgs == args.size()) << "[" << DemangleType(typeid(FTDictNode<K, V>).name()) \
...
const FTObjectBaseNode::FunctionTable FTDictNode<K, V>::function_table_ = {
    ...
    {"pop",
     [](RTView self, PyArgs args) -> RTValue {
       ...
       switch (args.size()) {
         ...
         default: {
           HVM_THROW << "[" << cls_name << "::pop] Expect 1 or 2 arguments but get " << args.size();
         } break;
       }
       return None;
     }},
    {"setdefault",
     [](RTView self, PyArgs args) -> RTValue {
       HVM_THROW << "dict.setdefault is not supported";
       return None;
     }},
    {"get",
     [](RTView self, PyArgs args) -> RTValue {
       ...
       switch (args.size()) {
         ...
         default: {
           HVM_THROW << "[" << cls_name << "::get] Expect 1 or 2 arguments but get " << args.size();
         } break;
       }
       return None;
     }},
    {"popitem",
     [](RTView self, PyArgs args) -> RTValue {
       HVM_THROW << "dict.popitem is not supported";
       return None;
     }},
    {"update",
     [](RTView self, PyArgs args) -> RTValue {
       HVM_THROW << "dict.update is not supported";
       return None;
     }},
};
