...
class NodeFunctor<R(const ObjectRef& n, Args...)> {
 ...
 public:
  ...
  R operator()(const ObjectRef& n, Args... args) const {
    HVM_CHECK(can_dispatch(n)) << "NodeFunctor calls un-registered function on type "
                             << n->GetTypeKey();
  ...
  TSelf& set_dispatch(FPointer f) {  // NOLINT(*)
    ...
    HVM_CHECK(func_[tindex] == nullptr) << "Dispatch for " << TNode::_type_key << " is already set";
    func_[tindex] = f;
    return *this;
  }
  ...
  TSelf& set_dispatch_unsafe(uint32_t tindex, FPointer f) {  // NOLINT(*)
    ...
    HVM_CHECK(func_[tindex] == nullptr) << "Dispatch for " << tindex << " is already set";
    func_[tindex] = f;
    return *this;
  }
  /*!
  ...
  TSelf& clear_dispatch() {  // NOLINT(*)
    uint32_t tindex = TNode::RuntimeTypeIndex();
    HVM_CHECK_LT(tindex, func_.size()) << "clear_dispatch: index out of range";
