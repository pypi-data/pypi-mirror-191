...
#define HVM_THROW ::hercules::runtime::LogMessageFatal(__FILE__, __LINE__).stream() << ": "
#define HVM_CHECK_LT(x, y) HERCULES_CHECK_BINARY_OP(_LT, <, x, y)
#define HVM_CHECK_GT(x, y) HERCULES_CHECK_BINARY_OP(_GT, >, x, y)
#define HVM_CHECK_LE(x, y) HERCULES_CHECK_BINARY_OP(_LE, <=, x, y)
#define HVM_CHECK_GE(x, y) HERCULES_CHECK_BINARY_OP(_GE, >=, x, y)
#define HVM_CHECK_EQ(x, y) HERCULES_CHECK_BINARY_OP(_EQ, ==, x, y)
#define HVM_CHECK_NE(x, y) HERCULES_CHECK_BINARY_OP(_NE, !=, x, y)
#define HVM_CHECK_NOTNULL(x)                                                           \
  ((x) == NULL ? ::hercules::runtime::LogMessageFatal(__FILE__, __LINE__).stream() \
                     << "Check  notnull: " #x << ' ',                                \
   (x)                                                                               \
               : (x))

#define HVM_LOG_DEBUG \
...
#define HVM_LOG_ERROR \
  ::hercules::runtime::LogMessage(__FILE__, __LINE__, ::hercules::runtime::LoggingLevel::ERROR)
#define HVM_LOG_WARNING                \
  ::hercules::runtime::LogMessage( \
      __FILE__, __LINE__, ::hercules::runtime::LoggingLevel::WARNING)
#define HVM_LOG_FATAL ::hercules::runtime::LogMessageFatal(__FILE__, __LINE__)
#define HVM_LOG_QFATAL HVM_LOG_FATAL

#define HVM_LOG(severity) HVM_LOG_##severity.stream()
#define HVM_LG HVM_LOG_INFO.stream()
#define HVM_LOG_IF(severity, condition) \
  !(condition) ? (void)0 : ::hercules::runtime::LogMessageVoidify() & HVM_LOG(severity)
