...
class Op : public HLOExpr {
 ...
 private:
   ...
   * \return The attr map.
   */
  HERCULES_DLL static const runtime::AttrRegistryMapContainerMap<Op>& GetAttrMapContainer(
      const StringRef& key);
};
...
inline OpRegEntry& OpRegEntry::set_attr(  // NOLINT(*)
    ...
    const ValueType& value,
    int plevel) {
  HVM_CHECK_GT(plevel, 0) << "plevel in set_attr must be greater than 0";
