...
class TypeFunctor<R(const Type& n, Args...)> {
 ...
 public:
  ...
  virtual R VisitType_(const NDArrayTypeNode* op, Args... args) HERCULES_TYPE_FUNCTOR_DEFAULT;
  ...
  virtual R VisitTypeDefault_(const Object* op, Args...) {
    HVM_LOG(FATAL) << "Do not have a default for " << op->GetTypeKey();
 ...
 private:
  ...
  static FType InitVTable() {
    ...
    HERCULES_TYPE_FUNCTOR_DISPATCH(NDArrayTypeNode);
