...
class PrimExprFunctor<R(const PrimExpr& n, Args...)> {
 ...
 public:
  ...
  virtual R VisitExprDefault_(const Object* op, Args...) {
    HVM_THROW << "[" << runtime::DemangleType(typeid(*this).name()) << "] Do not have a default for "
            << op->GetTypeKey();
    return R();
  }
  ...
  virtual void VisitExpr(const BaseExpr& expr) {
    if (expr.defined()) {
      if (expr->IsInstance<PrimExprNode>()) {
        ...
        HVM_THROW << "[ExprVisitor] not supported expr node: " << expr;
      }
    }
  }
  ...
  virtual BaseExpr VisitExpr(const BaseExpr& expr) {
    if (expr.defined()) {
      if (expr->IsInstance<PrimExprNode>()) {
        ...
        HVM_THROW << "[ExprMutator] not supported expr node: " << expr;
      }
    }
    return expr;
  }
