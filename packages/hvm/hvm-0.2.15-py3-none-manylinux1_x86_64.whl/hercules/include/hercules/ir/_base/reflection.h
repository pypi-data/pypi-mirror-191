...
#include <hercules/ir/_base/repr_printer.h>
...
class AttrVisitor {
 public:
  ...
  HERCULES_DLL virtual void Visit(const char* key, std::string* value) = 0;
  HERCULES_DLL virtual void Visit(const char* key, String* value) = 0;
  HERCULES_DLL virtual void Visit(const char* key, Unicode* value) = 0;
  HERCULES_DLL virtual void Visit(const char* key, void** value) = 0;
  HERCULES_DLL virtual void Visit(const char* key, DataType* value) = 0;
  HERCULES_DLL virtual void Visit(const char* key, NDArray* value) = 0;
...
class ReflectionVTable {
 public:
   ...
   */
  HERCULES_DLL ObjectPtr<Object> CreateInitObject(const String& type_key,
   ...
   */
  HERCULES_DLL ObjectRef CreateObject(const String& type_key, const PyArgs& kwargs);
  /*!
   * \brief Create an object by giving kwargs about its fields.
   *
   * \param type_key The type key.
   ...
   * \return The created object.
   */
  HERCULES_DLL ObjectRef CreateObject(const String& type_key, const Map<StringRef, ObjectRef>& kwargs);
  /*!
   ...
   */
  HERCULES_DLL RTValue GetAttr(Object* self, const StringRef& attr_name) const;

  /*!
   * \brief List all the fields in the object.
   * \return All the fields.
   */
  HERCULES_DLL std::vector<String> ListAttrNames(Object* self) const;
...
class ReflectionVTable::Registry {
 public:
  ...
  Registry& set_creator(FCreate f) {  // NOLINT(*)
    HVM_CHECK_LT(type_index_, parent_->fcreate_.size());
  ...
  Registry& set_repr_bytes(FReprBytes f) {  // NOLINT(*)
    HVM_CHECK_LT(type_index_, parent_->frepr_bytes_.size());
...
#define TVM_REFLECTION_REG_VAR_DEF                                                     \
  static HERCULES_ATTRIBUTE_UNUSED ::hercules::runtime::ReflectionVTable::Registry \
      __make_reflectiion
...
#define HERCULES_REGISTER_REFLECTION_VTABLE(TypeName, TraitName) \
  HERCULES_STR_CONCAT(TVM_REFLECTION_REG_VAR_DEF, __COUNTER__) = \
      ::hercules::runtime::ReflectionVTable::Global()->Register<TypeName, TraitName>()
...
#define HERCULES_REGISTER_NODE_TYPE(TypeName)                                                   \
  ...
  HERCULES_REGISTER_REFLECTION_VTABLE(TypeName,                                                 \
                                        ::hercules::runtime::detail::ReflectionTrait<TypeName>) \
...
inline ReflectionVTable::Registry ReflectionVTable::Register() {
  ...
  fvisit_attrs_[tindex] = ::hercules::runtime::detail::SelectVisitAttrs<T, TraitName>::VisitAttrs;

  fsequal_reduce_[tindex] =
      ::hercules::runtime::detail::SelectSEqualReduce<T, TraitName>::SEqualReduce;

  fshash_reduce_[tindex] =
      ::hercules::runtime::detail::SelectSHashReduce<T, TraitName>::SHashReduce;
...
inline void ReflectionVTable::VisitAttrs(Object* self, AttrVisitor* visitor) const {
  ...
  if (tindex >= fvisit_attrs_.size()) {
    HVM_LOG(FATAL) << "TypeError: " << self->GetTypeKey()
