...
class ArrayNode : public Object, public InplaceArrayBase<ArrayNode, ObjectRef> {
 public:
  ...
  static ObjectPtr<ArrayNode> CopyFrom(int64_t cap, ArrayNode* from) {
    ...
    HVM_CHECK_GE(cap, size) << "ValueError: not enough capacity";
...
class Array : public ObjectRef {
 ...
 public:
  ...
  const T operator[](int64_t i) const {
    ...
    HVM_CHECK(p != nullptr) << "ValueError: cannot index a null array";
    HVM_CHECK(0 <= i && i < p->size_)
  ...
  const T front() const {
    ArrayNode* p = GetArrayNode();
    HVM_CHECK(p != nullptr) << "ValueError: cannot index a null array";
    HVM_CHECK_GT(p->size_, 0) << "IndexError: cannot index an empty array";
 ...
 public:
  ...
  void insert(iterator position, const T& val) {
    HVM_CHECK(data_ != nullptr) << "ValueError: cannot insert a null array";
  ...
  void pop_back() {
    HVM_CHECK(data_ != nullptr) << "ValueError: cannot pop_back because array is null";
    int64_t size = GetArrayNode()->size_;
    HVM_CHECK_GT(size, 0) << "ValueError: cannot pop_back because array is empty";
  ...
  void erase(iterator position) {
    HVM_CHECK(data_ != nullptr) << "ValueError: cannot erase a null array";
    int64_t st = std::distance(begin(), position);
    int64_t size = GetArrayNode()->size_;
    HVM_CHECK(0 <= st && st < size) << "ValueError: cannot erase at index " << st
  ...
  void erase(iterator first, iterator last) {
    ...
    HVM_CHECK_LT(st, ed) << "ValueError: cannot erase array in range [" << st << ", " << ed << ")";
    HVM_CHECK(0 <= st && st <= size && 0 <= ed && ed <= size)
  ...
  void resize(int64_t n) {
    HVM_CHECK_GE(n, 0) << "ValueError: cannot resize an Array to negative size";
    if (data_ == nullptr) {
      SwitchContainer(n);
      return;
    }
 ...
 public:
  ...
  void Assign(IterType first, IterType last) {
    ...
    HVM_CHECK_GE(cap, 0) << "ValueError: cannot construct an Array of negative size";
    ArrayNode* p = GetArrayNode();
