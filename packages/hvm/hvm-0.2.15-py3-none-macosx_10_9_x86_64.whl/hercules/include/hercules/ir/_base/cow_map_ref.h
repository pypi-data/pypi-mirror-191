...
class SmallMapNode : public MapNode,
 ...
 public:
  ...
  const mapped_type& at(const key_type& key) const {
    ...
    HVM_CHECK(itr.index < size_) << "IndexError: key is not in Map";
    return itr->second;
  }
  /*!
   * \brief Index value associated with a key, throw exception if the key does not exist
 ...
 private:
  ...
  static void InsertMaybeReHash(const KVType& kv, ObjectPtr<Object>* map) {
    ...
    HVM_CHECK_GT(next_size, map_node->slots_);
...
class DenseMapNode : public MapNode {
 ...
 private:
  ...
  mapped_type& At(const key_type& key) const {
    ...
    HVM_CHECK(!iter.IsNone()) << "IndexError: key is not in Map";
    return iter.Val();
  }
  /*!
  ...
  static ObjectPtr<DenseMapNode> Empty(uint32_t fib_shift, uint64_t n_slots) {
    HVM_CHECK_GT(n_slots, uint64_t(SmallMapNode::kMaxSize));
    HVM_CHECK_EQ((n_slots & -n_slots), n_slots);
  ...
  static void InsertMaybeReHash(const KVType& kv, ObjectPtr<Object>* map) {
    ...
    HVM_CHECK_GT(map_node->slots_, uint64_t(SmallMapNode::kMaxSize));
