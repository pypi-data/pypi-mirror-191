...
class AttrRegistry {
 public:
  ...
  void UpdateAttr(const StringRef& attr_name, const KeyType& key, RTValue value, int plevel) {
    ...
    HVM_CHECK(p.second != plevel) << "Attribute " << attr_name << " of " << key->AttrRegistryName()
                                << " is already registered with same plevel=" << plevel;
    HVM_CHECK(value.type_code() != runtime::TypeIndex::kRuntimeNullptr)
  ...
  const AttrRegistryMapContainerMap<KeyType>& GetAttrMap(const StringRef& attr_name) {
    ...
    if (it == attrs_.end()) {
      HVM_LOG(FATAL) << "Attribute \'" << attr_name << "\' is not registered";
    }
