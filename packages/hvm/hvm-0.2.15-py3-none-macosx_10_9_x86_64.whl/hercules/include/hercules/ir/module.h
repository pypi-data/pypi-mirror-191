...
class IRModuleNode : public Object {
 public:
   ...
   * environment.
   */
  HERCULES_DLL void Add(const GlobalVar& var, const BaseFunc& func, bool update = false);
   ...
   */
  HERCULES_DLL void AddUnchecked(const GlobalVar& var, const BaseFunc& func);
   ...
   * environment.
   */
  HERCULES_DLL void AddTypeDef(const GlobalTypeVar& var, const ClassType& type, bool update = false);
   ...
   */
  HERCULES_DLL void AddTypeDefUnchecked(const GlobalTypeVar& var,
   ...
   */
  HERCULES_DLL void Update(const GlobalVar& var, const BaseFunc& func);
   ...
   * \param type The User ClassType.
   */
  HERCULES_DLL void UpdateTypeDef(const GlobalTypeVar& var, const ClassType& type);

  /*!
   * \brief Remove a function from the global environment.
   * \param var The name of the global function to update.
   */
  HERCULES_DLL void Remove(const GlobalVar& var);
   ...
   */
  HERCULES_DLL bool ContainGlobalVar(const StringRef& name) const;
   ...
   * \returns true if contains, otherise false.
   */
  HERCULES_DLL bool ContainGlobalTypeVar(const StringRef& name) const;
   ...
   */
  HERCULES_DLL GlobalVar GetGlobalVar(const StringRef& str) const;

  /*!
   * \brief Collect all global vars defined in this module.
   * \returns An array of global vars
   */
  HERCULES_DLL runtime::Array<GlobalVar> GetGlobalVars() const;
   ...
   * \returns The global variable.
   */
  HERCULES_DLL GlobalTypeVar GetGlobalTypeVar(const StringRef& str) const;

  /*!
   * \brief Collect all global type vars defined in this module.
   * \returns An array of global type vars
   */
  HERCULES_DLL runtime::Array<GlobalTypeVar> GetGlobalTypeVars() const;
   ...
   */
  HERCULES_DLL BaseFunc Lookup(const GlobalVar& var) const;
   ...
   */
  HERCULES_DLL BaseFunc Lookup(const StringRef& name) const;
   ...
   */
  HERCULES_DLL ClassType LookupTypeDef(const GlobalTypeVar& var) const;
   ...
   * \return The type definition.
   */
  HERCULES_DLL ClassType LookupTypeDef(const StringRef& var) const;
   ...
   */
  HERCULES_DLL std::unordered_set<StringRef> Imports() const;
...
class IRModule : public ObjectRef {
 public:
   ...
   */
  HERCULES_DLL explicit IRModule(
      runtime::Map<GlobalVar, BaseFunc> functions,
      runtime::Map<GlobalTypeVar, ClassType> type_definitions = {},
   ...
   */
  HERCULES_DLL static IRModule FromExpr(
      const HLOExpr& expr,
