...
class HERCULES_DLL FTList : public FTObjectBase {
 ...
 public:
  ...
  HERCULES_INLINE_VISIBILITY int64_t _check_index_error(int64_t i) const {
    ...
    HVM_CHECK((i >= 0 && i < len) || (i < 0 && i >= -len)) << "ValueError: index overflow";
  ...
  HERCULES_INLINE_VISIBILITY FTList<value_type> __getslice__(int64_t b,
    ...
    HVM_CHECK_GT(step, 0) << "FTList.slice_load step must be gt 0";
    int64_t len = MutableImpl().size();
  ...
  HERCULES_INLINE_VISIBILITY void __setslice__(int64_t start,
    ...
    auto& cons = MutableImpl();
    HVM_CHECK(start >= 0 && end >= 0 && start <= end);
  ...
  HERCULES_INLINE_VISIBILITY value_type pop(int64_t index = -1) const {
    ...
    if (index >= 0 && index < cons.size()) {
      ...
      if (cons.empty()) {
        HVM_THROW << "[List.pop] IndexError: pop from empty list";
      } else {
        HVM_THROW << "[List.pop] IndexError: pop index out of range";
      }
    }
    return ret;
  }
  ...
  HERCULES_INLINE_VISIBILITY void remove(const U& item, std::true_type) const {
    ...
    if (it != cons.end()) {
      ...
      HVM_THROW << "[list.remove] " << item << " not in list";
    }
  }
...
#define HERCULES_CHECK_FT_LIST_ARGS(FuncName, NumArgs)                               \
  HVM_CHECK(NumArgs == args.size()) << "[" << DemangleType(typeid(FTListNode<T>).name()) \
    ...
    {"__getslice__",
     [](RTView self, PyArgs args) -> RTValue {
       ...
       switch (args.size()) {
         ...
         default: {
           HVM_THROW << "[" << DemangleType(typeid(FTListNode<T>).name())
    ...
    {"index",
     [](RTView self, PyArgs args) -> RTValue {
       switch (args.size()) {
         ...
         default: {
           HVM_THROW << "TypeError: index expected at most 3 arguments, got 4";
         } break;
       }
       return -1;
     }},
