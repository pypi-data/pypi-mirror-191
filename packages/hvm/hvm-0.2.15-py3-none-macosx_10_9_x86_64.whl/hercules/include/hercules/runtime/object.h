...
#define HVM_DPTR(Class) Class##Node* d = static_cast<Class##Node*>(data_.get())
#define HVM_CHECK_DPTR(ClassName) \
  HVM_DPTR(ClassName);            \
  HVM_CHECK(d != nullptr) << "[" << #ClassName << "] object is None"
#define HVM_QPTR(Class) Class* q = static_cast<Class*>(q_ptr)
...
inline SubRef Downcast(BaseRef ref) {
  if (ref.defined()) {
    HVM_CHECK(ref->template IsInstance<typename SubRef::ContainerType>())
        << "Downcast from " << ref->GetTypeKey() << " to " << SubRef::ContainerType::_type_key
        << " failed.";
  } else {
    HVM_CHECK(SubRef::_type_is_nullable) << "Downcast from nullptr to not nullable reference of "
...
struct traits_helper_step2<false, TYPE> {
  static inline int32_t GetRuntimeIndex() {
    HVM_THROW << "unknown type index: " << typeid(TYPE).name();
