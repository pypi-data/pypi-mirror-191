...
def script(compiling_obj, *args, engine=None, **kwargs):
    ...
    if isinstance(engine, str):
        engine = engine.lower()
    if engine in ('torch', 'pytorch'):
        return backend.pytorch.script(compiling_obj, *args, **kwargs)
    elif engine in ('tensorflow',):
        return backend.tensorflow.script(compiling_obj, *args, **kwargs)
    else:
        if backend.pytorch.check(compiling_obj):
            return backend.pytorch.script(compiling_obj, *args, **kwargs)
        elif backend.tensorflow.check(compiling_obj):
            return backend.tensorflow.script(compiling_obj, *args, **kwargs)
