...
def sync_database(database: Database,
    ...
    with database.watch(
        ...
        while cursor.alive:
            ...
            LOGGER.info('database name found to be  %s', database_name)

            tap_stream_id = f'{change["ns"]["db"]}-{change["ns"]["coll"]}'
            # tap_stream_id = f'{change["ns"]["coll"]}'
            operation = change['operationType']

            LOGGER.info('change found in  %s, operation found to be [%s]...', tap_stream_id, operation)
