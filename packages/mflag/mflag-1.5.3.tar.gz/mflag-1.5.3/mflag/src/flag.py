...
class JobManager:
    ...
    def __initial_check(self):
        ...
        if not os.path.exist(self.job_file_path):
            job_dict = {'job_id':self.job_id, 'job_name':self.job_name, 'job_description':self.job_description, 'process_name':self.process_name, 'issue_date':datetime.now()}
    ...
    def __finish_job(self):
        with open(self.job_file_path, 'r') as f:
            job_dict = json.load(f)
            job_dict['finish_date'] = datetime.now()
        with open(self.job_file_path, 'w') as f:
            json.dump(job_dict, f)
