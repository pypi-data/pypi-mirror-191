...
def login() -> dict:
    ...
    if browser is None:
        console.print(
            f"1. On your computer or mobile device navigate to: [blue]{device_confirmation_url}[/]"
        )
        console.print(
            f"2. Confirm it shows the following code: [bold]{device_user_code}[/]"
        )
    else:
        console.print("Launching your default browser...")
        console.print(
            f"Check the opened page and confirm the it shows the following code: [bold]{device_user_code}[/]"
        )
        # prevent the browser window to open too quick and disrupt the flow
        time.sleep(1.5)
    ...
    with console.status("Waiting for confirmation...") as status:
        while True:
            ...
            if token_response.status_code == 200:
                ...
                console.print(f"{CHECKMARK} Authenticated successfully!")
