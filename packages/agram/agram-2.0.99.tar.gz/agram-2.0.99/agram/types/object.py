...
class Object:
    def __init__(self, client: "agram.Client" = None):
        self._client = client

    def bind(self, client: "agram.Client"):
        """Bind a Client instance to this and to all nested agram objects.

        Parameters:
            client (:obj:`~agram.types.Client`):
                The Client instance to bind this object with. Useful to re-enable bound methods after serializing and
                deserializing agram objects with ``repr`` and ``eval``.
        """
        self._client = client
