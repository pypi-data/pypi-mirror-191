...
import agram
from agram import raw
from agram import types, utils, enums
...
class ChatEvent(Object):
    ...
    Parameters:
        ...
        action (:obj:`~agram.enums.ChatEventAction`):
            Event action.

        user (:obj:`~agram.types.User`):
            User that triggered the event.

        old_description, new_description (``str``, *optional*):
            Previous and new chat description.
            For :obj:`~agram.enums.ChatEventAction.DESCRIPTION_CHANGED` action only.

        old_history_ttl, new_history_ttl (``int``, *optional*):
            Previous and new chat history TTL.
            For :obj:`~agram.enums.ChatEventAction.HISTORY_TTL_CHANGED` action only.

        old_linked_chat, new_linked_chat (:obj:`~agram.types.Chat`, *optional*):
            Previous and new linked chat.
            For :obj:`~agram.enums.ChatEventAction.LINKED_CHAT_CHANGED` action only.

        old_photo, new_photo (:obj:`~agram.types.Photo`, *optional*):
            Previous and new chat photo.
            For :obj:`~agram.enums.ChatEventAction.PHOTO_CHANGED` action only.

        old_title, new_title (``str``, *optional*):
            Previous and new chat title.
            For :obj:`~agram.enums.ChatEventAction.TITLE_CHANGED` action only.

        old_username, new_username (``str``, *optional*):
            Previous and new chat username.
            For :obj:`~agram.enums.ChatEventAction.USERNAME_CHANGED` action only.

        old_chat_permissions, new_chat_permissions (:obj:`~agram.types.ChatPermissions`, *optional*):
            Previous and new default chat permissions.
            For :obj:`~agram.enums.ChatEventAction.CHAT_PERMISSIONS_CHANGED` action only.

        deleted_message (:obj:`~agram.types.Message`, *optional*):
            Deleted message.
            For :obj:`~agram.enums.ChatEventAction.MESSAGE_DELETED` action only.

        old_message, new_message (:obj:`~agram.types.Message`, *optional*):
            Previous and new message before it has been edited.
            For :obj:`~agram.enums.ChatEventAction.MESSAGE_EDITED` action only.

        invited_member (:obj:`~agram.types.ChatMember`, *optional*):
            New invited chat member.
            For :obj:`~agram.enums.ChatEventAction.MEMBER_INVITED` action only.

        old_administrator_privileges, new_administrator_privileges (:obj:`~agram.types.ChatMember`, *optional*):
            Previous and new administrator privileges.
            For :obj:`~agram.enums.ChatEventAction.ADMINISTRATOR_PRIVILEGES_CHANGED` action only.

        old_member_permissions, new_member_permissions (:obj:`~agram.types.ChatMember`, *optional*):
            Previous and new member permissions.
            For :obj:`~agram.enums.ChatEventAction.MEMBER_PERMISSIONS_CHANGED` action only.

        stopped_poll (:obj:`~agram.types.Message`, *optional*):
            Message containing the stopped poll.
            For :obj:`~agram.enums.ChatEventAction.POLL_STOPPED` action only.

        invites_enabled (``bool``, *optional*):
            If chat invites were enabled (True) or disabled (False).
            For :obj:`~agram.enums.ChatEventAction.INVITES_ENABLED` action only.

        history_hidden (``bool``, *optional*):
            If chat history has been hidden (True) or unhidden (False).
            For :obj:`~agram.enums.ChatEventAction.HISTORY_HIDDEN` action only.

        signatures_enabled (``bool``, *optional*):
            If message signatures were enabled (True) or disabled (False).
            For :obj:`~agram.enums.ChatEventAction.SIGNATURES_ENABLED` action only.

        old_slow_mode, new_slow_mode (``int``, *optional*):
            Previous and new slow mode value in seconds.
            For :obj:`~agram.enums.ChatEventAction.SLOW_MODE_CHANGED` action only.

        pinned_message (:obj:`~agram.types.Message`, *optional*):
            Pinned message.
            For :obj:`~agram.enums.ChatEventAction.MESSAGE_PINNED` action only.

        unpinned_message (:obj:`~agram.types.Message`, *optional*):
            Unpinned message.
            For :obj:`~agram.enums.ChatEventAction.MESSAGE_UNPINNED` action only.

        old_invite_link, new_invite_link (:obj:`~agram.types.ChatInviteLink`, *optional*):
            Previous and new edited invite link.
            For :obj:`~agram.enums.ChatEventAction.INVITE_LINK_EDITED` action only.

        revoked_invite_link (:obj:`~agram.types.ChatInviteLink`, *optional*):
            Revoked invite link.
            For :obj:`~agram.enums.ChatEventAction.INVITE_LINK_REVOKED` action only.

        deleted_invite_link (:obj:`~agram.types.ChatInviteLink`, *optional*):
            Deleted invite link.
            For :obj:`~agram.enums.ChatEventAction.INVITE_LINK_DELETED` action only.
    """
