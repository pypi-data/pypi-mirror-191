...
class SentCode(Object):
    ...
    Parameters:
        type (:obj:`~agram.enums.SentCodeType`):
            Type of the current sent code.

        phone_code_hash (``str``):
            Confirmation code identifier useful for the next authorization steps (either
            :meth:`~agram.Client.sign_in` or :meth:`~agram.Client.sign_up`).

        next_type (:obj:`~agram.enums.NextCodeType`, *optional*):
            Type of the next code to be sent with :meth:`~agram.Client.resend_code`.

        timeout (``int``, *optional*):
            Delay in seconds before calling :meth:`~agram.Client.resend_code`.
    """
