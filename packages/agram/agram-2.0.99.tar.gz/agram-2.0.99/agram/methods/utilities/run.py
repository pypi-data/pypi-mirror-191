...
import agram
from agram.methods.utilities.idle import idle
...
class Run:
    def run(
        ...
        :meth:`~agram.Client.start`, :meth:`~agram.idle` and :meth:`~agram.Client.stop` in sequence.
        ...
        operation. This is almost the same as :py:obj:`asyncio.run` except for the fact that agram's ``run`` uses the
        current event loop instead of a new one.

        If you want to run multiple clients at once, see :meth:`agram.compose`.
