...
class SignIn:
    async def sign_in(
        ...
        Parameters:
            ...
            phone_code_hash (``str``):
                Code identifier taken from the result of :meth:`~agram.Client.send_code`.

            phone_code (``str``):
                The valid confirmation code you received (either as Telegram message or as SMS in your phone number).

        Returns:
            :obj:`~agram.types.User` | :obj:`~agram.types.TermsOfService` | bool: On success, in case the
            authorization completed, the user is returned. In case the phone number needs to be registered first AND the
            terms of services accepted (with :meth:`~agram.Client.accept_terms_of_service`), an object containing
