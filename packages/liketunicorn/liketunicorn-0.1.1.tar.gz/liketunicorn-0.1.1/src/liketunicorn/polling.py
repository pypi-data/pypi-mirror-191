...
async def _feed(app: Callable[..., Awaitable[None]], update: TelegramObject) -> None:
    await app(update)
...
async def _polling(
    ...
    async for update in _listen(session=session, config=config):
        await _feed(app=app, update=update)
