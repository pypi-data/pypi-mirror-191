        ...
        for field in protobuf_class.DESCRIPTOR.fields:
            ...
            if (field.message_type is not None) and (field.label != FieldDescriptor.LABEL_REPEATED):
                ...
                if field.name in kwargs:
                    sub_default_args = convert_to_dict(kwargs.pop(field.name))
                ...
                sub_selected_args = ExperimentBuilder._recursive_switch_activation_(field_message_class, sub_default_args, sub_switch_args, kwargs)
    ...
    def _recursive_build_(protobuf_class, default_args, selected_switch_args, kwargs, used_args, hyperparameters):
        ...
        for field in protobuf_class.DESCRIPTOR.fields:
            ...
            if field.message_type is None or field.label == FieldDescriptor.LABEL_REPEATED:
                ...
                # we accumulate all the necessary args for the next recursion
                # default values are put in sub_default_args
                # selected_switch_args are put in sub_selected_switch_args
                ...
                # This fields is a protobuf message, we build it recursively
                field_message_class = load_proto_class_from_protobuf_descriptor(field.message_type)
                exp_builder = ExperimentBuilder._recursive_build_(field_message_class, sub_default_args, sub_selected_switch_args, kwargs, used_args, hyperparameters)
