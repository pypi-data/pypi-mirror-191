...
class CodeGen(Visitor[JSVisitorContext]):
    ...
    # def visit_Comment(self, node: ast.PyCF_TYPE_COMMENTS)
    ...
    def visit_ClassDef(self, node: ast.ClassDef, ctx: JSVisitorContext):
        ...
        builder = BaseBuilder()\
            ...
            .write(SurroundBuilder(surround='{}')
                   .write_if(self.python_compatible, lambda: '''constructor(...args) {if ('__init__' in this) this.__init__(...args); return new Proxy(this, { apply: (target, self, args) => target.__call__(self, ...args)})}''')
    ...
    def visit_With(self, node: ast.With, ctx: JSVisitorContext):
        ...
        builder.write(
            ...
            .write(BaseBuilder(separator=';').write(BaseBuilder().write_if(expr.optional_vars, f'{self.visit(expr.optional_vars)}=').write(f'__with_{i}.__enter__()') for i, expr in enumerate(node.items)))\
            .write(self.visit(node.body))\
            .write(BaseBuilder(separator=';').write(BaseBuilder().write_if(expr.optional_vars, f'{self.visit(expr.optional_vars)}=').write(f'__with_{i}.__exit__()') for i, expr in enumerate(node.items)))
