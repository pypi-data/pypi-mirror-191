...
def create_randomized_networks(guild_ex, interactome, output_path, n=100):
    print ('DIO')
    subprocess.run('bash -c "echo PORCO"')
    subprocess.run('bash -c "python %s %s %s"' %(guild_ex,interactome,n),
                   shell=True)
...
def guild(algorithm, path_guild, nodes, interactome, output_path, out_log, global_log):
    if algorithm == "s":
        ...
        if len(os.listdir(rand_net))==0:
            ...
            rand_interactomes = create_randomized_networks(path_guild+"/src/create_random_networks_for_netzcore.py", interactome, rand_net)
        run_netzcore(path_guild, nodes, interactome, rand_net+"/interactome.sif.", output_path+"/output_netZcore")
