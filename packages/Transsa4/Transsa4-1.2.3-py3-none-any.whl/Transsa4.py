...
########### Transformar los valores predichos al entero más cercano
def Transformar_int(y1_predB,y2_predB):
    y1_predB1=[]
    for i in range(len(y1_predB)):
        if y1_predB[i]-int(y1_predB[i])>=abs(y1_predB[i]-int(y1_predB[i]+1)):
            num=int(y1_predB[i]+1)
        else:
            num=int(y1_predB[i])
        y1_predB1.append(num)

    y2_predB2=[]
    for i in range(len(y2_predB)):
        if y2_predB[i]-int(y2_predB[i])>=abs(y2_predB[i]-int(y2_predB[i]+1)):
            num=int(y2_predB[i]+1)
        else:
            num=int(y2_predB[i])
        y2_predB2.append(num)
    return y1_predB1,y2_predB2
...
def Error_banos2(reml2,valest,modelo): ##
    ...
    tabla=pd.concat([tabla, tablaux],ignore_index=True);
    return tabla
def Error_dormitorios(reml2,valest,modelo): ##
    real=reml2['nro_dormitorios']
    estimado=reml2[valest]
    errores=reml2[f'Error_{modelo}']
    tabla= pd.DataFrame.from_dict({'Error de dormitorios':[],
                                   f'%_{modelo}_Acumulado': [],f'Cantidad_{modelo}': []});

    col = ['Error de dormitorios',f'%_{modelo}_Acumulado',f'Cantidad_{modelo}'];
    cant=[0]
    porc=[0]
    k=[0,1,2]
    for lim in range(0,len(k)):
    ...
    tabla=pd.concat([tabla, tablaux],ignore_index=True);
    return tabla
def Distribution_of_errors(reml1,var_est): #var_est: 'Número de baños' o 'Número de dormitorios'
    maxi,mini=0,0
    for k in ['RF','XGB','RFC','GBC','DT']:
        if maxi<max(reml1[f'Error_{k}']):
            maxi=max(reml1[f'Error_{k}'])
        if maxi>min(reml1[f'Error_{k}']):
            mini=min(reml1[f'Error_{k}'])
    fig = plt.figure(figsize=(12,10))
    gs1 = gs.GridSpec(nrows=3, ncols=4,hspace=0.3,wspace=0.5)

    ax1=plt.subplot(gs1[0,0:2])
    sns.histplot(data=reml1['Error_RF'],color='blue',ax=ax1, kde=True,discrete=False,bins=100)
    ax1.grid(True, color='lightgrey',linestyle='--')
    ax1.set(xlabel=var_est)
    ax1.set(ylabel='Cantidad')
    ax1.set(xlim=[mini,maxi])
    ax1.set_title('Error RF');

    ax2=plt.subplot(gs1[0,2:4])
    sns.histplot(data=reml1['Error_XGB'],color='red',ax=ax2, 
                 kde=True,discrete=False,bins=100)
    ax2.grid(True, color='lightgrey',linestyle='--')
    ax2.set(xlabel=var_est)
    ax2.set(ylabel='Cantidad')
    ax2.set(xlim=[mini,maxi])
    ax2.set_title('Error XGB');

    ax3=plt.subplot(gs1[1,0:2])
    sns.histplot(data=reml1['Error_RFC'],color='green',ax=ax3, 
                 kde=True,discrete=False,bins=100)
    ax3.grid(True, color='lightgrey',linestyle='--')
    ax3.set(xlabel=var_est)
    ax3.set(ylabel='Cantidad')
    ax3.set(xlim=[mini,maxi])
    ax3.set_title('Error RFC');

    ax4=plt.subplot(gs1[1,2:4])
    sns.histplot(data=reml1['Error_GBC'],color='silver',ax=ax4,
                 kde=True,discrete=False,bins=100)
    ax4.grid(True, color='lightgrey',linestyle='--')
    ax4.set(xlabel=var_est)
    ax4.set(ylabel='Cantidad')
    ax4.set(xlim=[mini,maxi])
    ax4.set_title('Error GBC');

    ax5=plt.subplot(gs1[2,1:3])
    plt.rcParams["figure.figsize"] = (5.28,3.6)
    sns.histplot(data=reml1['Error_DT'],color='orange',ax=ax5,
                 kde=True,discrete=False,bins=100)
    ax5.grid(True, color='lightgrey',linestyle='--')
    ax5.set(xlabel=var_est)
    ax5.set(ylabel='Cantidad')
    ax5.set(xlim=[mini,maxi])
    ax5.set_title('Error DT');
    plt.show()
def bar_graph(TablaTotal,reml1,var):#var: 'nro_banos' o 'nro_dormitorios'
    plt.rcParams['figure.figsize'] = (15,7);
    data = np.array(TablaTotal[['Cantidad_RF','Cantidad_XGB','Cantidad_RFC','Cantidad_GBC','Cantidad_DT']])
    columns =('Cantidad RF (Acumulado)','Cantidad XGB (Acumulado)','Cantidad RFC (Acumulado)',
              'Cantidad GBC (Acumulado)','Cantidad DT (Acumulado)')
    rows=[]
    texto='Total de datos'
    rows.append(texto)
    if var=='nro_banos':
        for x in [2,1,0]:
            if x >1:
                texto='Error menor o igual que '+str(x)+' baños'
            elif x==1:
                texto='Error menor o igual que 1 baño'
            else:
                texto='Estimaciones exactas'
            rows.append(texto)
    elif var=='nro_dormitorios':
        for x in [2,1,0]:
            if x >1:
                texto='Error menor o igual que '+str(x)+' dormitorios'
            elif x==1:
                texto='Error menor o igual que 1 dormitorio'
            else:
                texto='Estimaciones exactas'
            rows.append(texto)
    colors = plt.cm.Set2(np.linspace(0.2, 1, len(rows)))
    ...
    plt.ylim(0,reml1.shape[0]+50)
    plt.xticks([])
    plt.grid(True)
    plt.title('Errores de estimación')
    plt.show()

def Matrixs_confusions(reml1,hs,ws,var):
    fig = plt.figure(figsize=(15,15))
    gs1 = gs.GridSpec(nrows=3, ncols=4,hspace=hs,wspace=ws)
    
    ################## Matriz de confusión de RF ##################
    ax1=plt.subplot(gs1[0,0:2])
    y_true=reml1[var].tolist()
    x=reml1[var].unique()
    x.sort()
    y_pred_RF=reml1['ValEst_RF'].tolist()
    y=reml1['ValEst_RF'].unique()
    y.sort()
    mat=confusion_matrix(y_true,y_pred_RF)
    sns.heatmap(mat.T,square=True,annot=True,fmt='d',cbar=False,
                xticklabels=x,yticklabels=y,ax=ax1)
    if var='nro_banos':
        ax1.set_xlabel('Baños reales')
        ax1.set_ylabel('Baños predichos')
    elif var='nro_dormitorios':
        ax1.set_xlabel('Dormitorios reales')
        ax1.set_ylabel('Dormitorios predichos')
    ax1.set_title('Matriz de confusión (RF)');

    ################## Matriz de confusión de XGB ##################
    ax2=plt.subplot(gs1[0,2:4])
    y_true=reml1[var].tolist()
    x=reml1[var].unique()
    x.sort()
    y_pred_XGB=reml1['ValEst_XGB'].tolist()
    y=reml1['ValEst_XGB'].unique()
    y.sort()
    mat=confusion_matrix(y_true,y_pred_XGB)
    sns.heatmap(mat.T,square=True,annot=True,fmt='d',cbar=False,
                xticklabels=x,yticklabels=y,ax=ax2)
    if var='nro_banos':
        ax2.set_xlabel('Baños reales')
        ax2.set_ylabel('Baños predichos')
    elif var='nro_dormitorios':
        ax2.set_xlabel('Dormitorios reales')
        ax2.set_ylabel('Dormitorios predichos')
    ax2.set_title('Matriz de confusión (XGB)');

    ################## Matriz de confusión de RFC ##################
    ax3=plt.subplot(gs1[1,0:2])
    y_true=reml1[var].tolist()
    x=reml1[var].unique()
    x.sort()
    y_pred_RF=reml1['ValEst_RFC'].tolist()
    y=reml1['ValEst_RFC'].unique()
    y.sort()
    mat=confusion_matrix(y_true,y_pred_RF)
    sns.heatmap(mat.T,square=True,annot=True,fmt='d',cbar=False,
                xticklabels=x,yticklabels=y,ax=ax3)
    if var='nro_banos':
        ax3.set_xlabel('Baños reales')
        ax3.set_ylabel('Baños predichos')
    elif var='nro_dormitorios':
        ax3.set_xlabel('Dormitorios reales')
        ax3.set_ylabel('Dormitorios predichos')
    ax3.set_title('Matriz de confusión (RFC)');

    ################## Matriz de confusión de GBC ##################
    ax4=plt.subplot(gs1[1,2:4])
    y_true=reml1[var].tolist()
    x=reml1[var].unique()
    x.sort()
    y_pred_XGB=reml1['ValEst_GBC'].tolist()
    y=reml1['ValEst_GBC'].unique()
    y.sort()
    mat=confusion_matrix(y_true,y_pred_XGB)
    sns.heatmap(mat.T,square=True,annot=True,fmt='d',cbar=False,
                xticklabels=x,yticklabels=y,ax=ax4)
    if var='nro_banos':
        ax4.set_xlabel('Baños reales')
        ax4.set_ylabel('Baños predichos')
    elif var='nro_dormitorios':
        ax4.set_xlabel('Dormitorios reales')
        ax4.set_ylabel('Dormitorios predichos')
    ax4.set_title('Matriz de confusión (GBC)');
    ################## Matriz de confusión de DT ##################
    ax5=plt.subplot(gs1[2,1:3])
    y_true=reml1[var].tolist()
    x=reml1[var].unique()
    x.sort()
    y_pred_XGB=reml1['ValEst_DT'].tolist()
    y=reml1['ValEst_DT'].unique()
    y.sort()
    mat=confusion_matrix(y_true,y_pred_XGB)
    sns.heatmap(mat.T,square=True,annot=True,fmt='d',cbar=False,
                xticklabels=x,yticklabels=y,ax=ax5)
    if var='nro_banos':
        ax5.set_xlabel('Baños reales')
        ax5.set_ylabel('Baños predichos')
    elif var='nro_dormitorios':
        ax5.set_xlabel('Dormitorios reales')
        ax5.set_ylabel('Dormitorios predichos')
    ax5.set_title('Matriz de confusión (DT)');
    plt.show()
...
def Modelo_final_dormitorios(TablaTotal,scls,models,cve): ### MOD X TENER RL Y ENET
    RF_list=np.array(TablaTotal['%_RF_Acumulado'].tolist())
    XGB_list=np.array(TablaTotal['%_XGB_Acumulado'].tolist())
    RFC_list=np.array(TablaTotal['%_RFC_Acumulado'].tolist())
    GBC_list=np.array(TablaTotal['%_GBC_Acumulado'].tolist())
    ...
    p=max(L)
    if p==RF_list[0]: 
        print("El modelo para estimar dormitorio será el de Random Forest. Se procede a guardarlo")
        scaler_file = "escala_Final_dormitorios"+str(cve)+".save"
        joblib.dump(scls[0], scaler_file)
        joblib.dump(models[0],"Modelo_Final_dormitorios"+str(cve)+".joblib")
        return models[0],scls[0]
    
    elif p==XGB_list[0]: 
        print("El modelo para estimar dormitorios será el de Extreme Gradient Boosting. Se procede a guardarlo")
        scaler_file = "escala_Final_dormitorios"+str(cve)+".save"
        joblib.dump(scls[1], scaler_file)
        joblib.dump(models[1],"Modelo_Final_dormitorios"+str(cve)+".joblib")
        return models[1],scls[1]
    elif p==RFC_list[0]: 
        print("El modelo para estimar dormitorios será el de RF (Clasificador). Se procede a guardarlo")
        scaler_file = "escala_Final_dormitorios"+str(cve)+".save"
        joblib.dump(scls[2], scaler_file)
        joblib.dump(models[2],"Modelo_Final_dormitorios"+str(cve)+".joblib")
        return models[2],scls[2]
    elif p==GBC_list[0]: 
        print("El modelo para estimar dormitorios será el de GB (Clasificador). Se procede a guardarlo")
        scaler_file = "escala_Final_dormitorios"+str(cve)+".save"
        joblib.dump(scls[3], scaler_file)
        joblib.dump(models[3],"Modelo_Final_dormitorios"+str(cve)+".joblib")
        return models[3],scls[3]
    elif p==DT_list[0]: 
        print("El modelo para estimar dormitorios será el de DecisionTree. Se procede a guardarlo")
        scaler_file = "escala_Final_dormitorios"+str(cve)+".save"
        joblib.dump(scls[4], scaler_file)
        joblib.dump(models[4],"Modelo_Final_dormitorios"+str(cve)+".joblib")
        return models[4],scls[4]