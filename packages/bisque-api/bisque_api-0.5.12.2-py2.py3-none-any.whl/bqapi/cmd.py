...
def bisque_config(parser=None, args=None, write_config=True):
    ...
    if not (pargs.host and pargs.user and pargs.password):
        ...
        if write_config:
            ...
            pargs.host = input("BisQue URL [{}] ".format(pargs.host)) or pargs.host
            pargs.user = input("username[{}] ".format(pargs.user)) or pargs.user
            pargs.password = input("password[{}]: ".format(pargs.password)) or pargs.password
...
def bisque_session(parser=None, args=None, root_logger=None):
    ...
    if pargs.host and pargs.user and pargs.password:
        ...
        if session is None:
            print(
                "Could not create bisque session with host={} user={} pass={}".format(
                    pargs.host, pargs.user, pargs.password
                )
            )
            return session
        elif not pargs.quiet:
            print("Session for  ", pargs.host, " for user ", pargs.user, " created")
