...
class BaseServiceProxy(object):
    ...
    def __init__(self, session, service_name, timeout=DEFAULT_TIMEOUT):
        ...
        self.service_url = session.service_map.get(service_name, None) or posixpath.join(session.c.root , service_name)
...
class AuthProxy (BaseServiceProxy):
    ...
    def add_share(self, resource_uniq, sharee, permission="read",notify=False):
        """
        Args:
        Returns:
        """
        auth = etree.Element('auth', user=sharee, permission=permission)
        data = self.session.service("data_service")
        notify = str(notify).lower()
        data.post(path=[resource_uniq, "auth"], params={"notify":notify}, data = etree.tostring(auth))

    def del_share(self, resource_uniq):
        data = self.sesssion.service("data_service")
        data.delete(path=[resource_uniq, "auth", resource_uniq])
