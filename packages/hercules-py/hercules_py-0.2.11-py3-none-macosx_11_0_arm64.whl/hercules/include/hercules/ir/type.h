...
class TupleType : public Type {
 public:
   ...
   * \param span The span of the type.
   */
  HERCULES_DLL explicit TupleType(runtime::Array<Type> fields, Span span = Span())
      : TupleType(std::move(fields), false, std::move(span)) {
  }

  HERCULES_DLL explicit TupleType(runtime::Array<Type> fields, bool is_std_tuple, Span span = Span());
...
class FuncType : public Type {
 public:
   ...
   */
  HERCULES_DLL FuncType(runtime::Array<Type> arg_types,
                    Type ret_type,
                    runtime::Array<TypeVar> type_params,
                    runtime::Array<TypeConstraint> type_constraints,
                    Span span = Span());
...
class NDArrayTypeNode : public TypeNode {
 public:
  ...
  HERCULES_DECLARE_FINAL_OBJECT_INFO(NDArrayTypeNode, TypeNode);
};

class NDArrayType : public Type {
 public:
  HERCULES_DLL explicit NDArrayType(int64_t ndim, PrimType dtype, Span span = Span());

  HERCULES_DEFINE_NOTNULLABLE_OBJECT_REF_METHODS(NDArrayType, Type, NDArrayTypeNode);
};
