...
def player(image,x,y,move_left='a',move_right='d',move_up='w',move_down='s',jump='space',speed=3,jump_height=20, collide_sides=False, sticky_keys=True):
            ...
            if jump != '':
                if jumped:
                    ...
                    if jump_velocity < -jump_height:
                        jumped = False
                        jump_velocity = jump_height
    elif not sticky_keys:
        if not collide_sides:
            if kb.is_pressed(move_left):
                if move_left != '':
                    player_x -= speed
            elif kb.is_pressed(move_right):
                if move_right != '':
                    player_x += speed
            elif kb.is_pressed(move_up):
                if move_up != '':
                    player_y -= speed
            elif kb.is_pressed(move_down):
                if move_down != '':
                    player_y += speed
            if jump != '':
                if jumped:
        ...
        if collide_sides:
            if collide_sides:
                if kb.is_pressed(move_left) and player_x > 0:
                    if move_left != '':
                        player_x -= speed
                elif kb.is_pressed(move_right) and player_x < screen_width - player_width:
                    if move_right != '':
                        player_x += speed
                elif kb.is_pressed(move_up) and player_y > 0:
                    if move_up != '':
                        player_y -= speed
                elif kb.is_pressed(move_down) and player_y < screen_height - player_height:
                    if move_down != '':
                        player_y += speed
                if jump != '':
                    if jumped:
...
class Bar:
    def __init__(self,full_heart_img, empty_heart_img, x,y,health,draw_lives=True):
