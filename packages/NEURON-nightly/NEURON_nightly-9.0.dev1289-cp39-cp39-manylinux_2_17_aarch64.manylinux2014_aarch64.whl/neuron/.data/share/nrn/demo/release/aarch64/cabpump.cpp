...
#define _ion_cao	*(_ppvar[0].get<double*>())
#define _ion_ica	*(_ppvar[1].get<double*>())
#define _ion_cai	*_ppvar[2].get<double*>()
...
static void nrn_alloc(Prop* _prop) {
 ...
 prop_ion = need_memb(_morphology_sym);
 	_ppvar[5] = &prop_ion->param[0]; /* diam */
 prop_ion = need_memb(_ca_sym);
 nrn_check_conc_write(_prop, prop_ion, 1);
 nrn_promote(prop_ion, 3, 0);
 	_ppvar[0] = &prop_ion->param[2]; /* cao */
 	_ppvar[1] = &prop_ion->param[3]; /* ica */
 	_ppvar[2] = &prop_ion->param[1]; /* cai */
 	_ppvar[3] = &prop_ion->param[4]; /* _ion_dicadv */
 	_ppvar[4] = &(prop_ion->dparam[0].literal_value<int>()); /* iontype for ca */
...
double ss (  ) {
 ...
 if(error){fprintf(stderr,"at line 146 in file cabpump.mod:\nFUNCTION ss() (mM) {\n"); nrn_complain(_p); abort_run(error);}
    if (secondorder) {
    int _i;
    for (_i = 0; _i < 12; ++_i) {
 ...
 v=_v;
{
 ...
 if(error){fprintf(stderr,"at line 85 in file cabpump.mod:\nBREAKPOINT {\n"); nrn_complain(_p); abort_run(error);}
    if (secondorder) {
    int _i;
    for (_i = 0; _i < 12; ++_i) {
      _p[_slist1[_i]] += dt*_p[_dlist1[_i]];
