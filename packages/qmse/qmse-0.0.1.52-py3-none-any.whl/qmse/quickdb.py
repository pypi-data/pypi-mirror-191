...
class QuickDB:
    ...
    def getBdays(self, stringify=True, inverted=False):
        if stringify:
            _ = load_pkl(os.path.join(self.path, 'ts/IDX.data')).index.map(lambda x : x.strftime("%Y%m%d"))
        else:
            _ = load_pkl(os.path.join(self.path, 'ts/IDX.data')).index
        if inverted:
