...
class Hujiang:
    ...
    def dict_search_jp(cls, text: str, search_type: str = 'jc') -> dict:
        ...
        content = utils.request_get(url, cookies={'HJ_UID': '0', 'HJ_SID': '0'}).content.decode()
        result = {
            'text': text,
        ...
        for details_pane in word_details_panes:
            ...
            details = []
            detail_groups = word_details_content.find('section', attrs={'class': 'detail-groups'})
            if detail_groups:
                for dl in detail_groups.findAll('dl'):
                    category = dl.find('dt').text.strip()
                    paraphrases = []
                    for dd in dl.findAll('dd'):
                        paraphrase = dd.find('h3').findAll('p')[-1].text.strip()
                        examples = []
                        ...
                        paraphrases.append({'paraphrase': paraphrase, 'examples': examples})
