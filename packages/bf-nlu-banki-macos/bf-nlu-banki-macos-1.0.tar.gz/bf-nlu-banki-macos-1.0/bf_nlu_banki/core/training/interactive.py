...
import bf_nlu_banki.shared.data
from bf_nlu_banki.shared.nlu.constants import TEXT, INTENT_NAME_KEY
from bf_nlu_banki.shared.nlu.training_data.loading import bf_nlu_banki, bf_nlu_banki_YAML
from bf_nlu_banki.shared.core.constants import (
    USER_INTENT_RESTART,
    ACTION_LISTEN_NAME,
    LOOP_NAME,
    ACTIVE_LOOP,
...
from bf_nlu_banki.core import run, utils
import bf_nlu_banki.core.train
from bf_nlu_banki.core.constants import DEFAULT_SERVER_FORMAT, DEFAULT_SERVER_PORT
from bf_nlu_banki.shared.core.domain import (
    Domain,
    KEY_INTENTS,
    KEY_ENTITIES,
    KEY_RESPONSES,
...
import bf_nlu_banki.shared.core.events
from bf_nlu_banki.shared.core.events import (
    ActionExecuted,
    ActionReverted,
    BotUttered,
    Event,
...
from bf_nlu_banki.shared.core.trackers import EventVerbosity, DialogueStateTracker
from bf_nlu_banki.shared.core.training_data import visualization
from bf_nlu_banki.shared.core.training_data.visualization import (
    VISUALIZATION_TEMPLATE_PATH,
    visualize_neighborhood,
)
from bf_nlu_banki.core.utils import AvailableEndpoints
from bf_nlu_banki.shared.importers.bf_nlu_banki import TrainingDataImporter
from bf_nlu_banki.utils.common import update_sanic_log_level
from bf_nlu_banki.utils.endpoints import EndpointConfig
from bf_nlu_banki.shared.exceptions import InvalidConfigException

# noinspection PyProtectedMember
from bf_nlu_banki.shared.nlu.training_data import loading
from bf_nlu_banki.shared.nlu.training_data.message import Message
...
# still works.
import bf_nlu_banki.utils.io as io_utils

from bf_nlu_banki.shared.core.generator import TrackerWithCachedStates
...
def format_bot_output(message: BotUttered) -> Text:
    ...
    if "buttons" in data and data["buttons"] is not None:
        output += "\nButtons:"
        choices = bf_nlu_banki.cli.utils.button_choices_from_message_data(
    ...
    if "elements" in data and data["elements"] is not None:
        ...
        for idx, element in enumerate(data["elements"]):
            element_str = bf_nlu_banki.cli.utils.element_to_string(element, idx)
            output += "\n" + element_str
...
def _slot_history(tracker_dump: Dict[Text, Any]) -> List[Text]:
    ...
    for k, s in tracker_dump.get("slots", {}).items():
        colored_value = bf_nlu_banki.shared.utils.io.wrap_with_color(
            str(s), color=bf_nlu_banki.shared.utils.io.bcolors.WARNING
        )
    ...
    while True:
        try:
            ...
            answer = questionary.confirm(
                f"Failed to export '{export_path}': {e}. Please make sure 'bf_nlu_banki' "
    ...
    questions = questionary.form(
        export_stories=questionary.text(
            ...
            validate=io_utils.file_type_validator(
                bf_nlu_banki.shared.data.YAML_FILE_EXTENSIONS,
        ...
        export_nlu=questionary.text(
            ...
            default=PATHS["nlu"],
            validate=io_utils.file_type_validator(
                list(bf_nlu_banki.shared.data.TRAINING_DATA_EXTENSIONS),
...
def _split_conversation_at_restarts(
    ...
    split_events = bf_nlu_banki.shared.core.events.split_events(
...
def _collect_messages(events: List[Dict[Text, Any]]) -> List[Message]:
    ...
    import bf_nlu_banki.shared.nlu.training_data.util as bf_nlu_banki_nlu_training_data_utils
    ...
    for event in events:
        if event.get("event") == UserUttered.type_name:
            ...
            bf_nlu_banki_nlu_training_data_utils.remove_untrainable_entities_from(data)
            msg = Message.build(
...
def _write_stories_to_file(
    ...
    from bf_nlu_banki.shared.core.training_data.story_writer.yaml_story_writer import (
        YAMLStoryWriter,
    )
    ...
    if bf_nlu_banki.shared.data.is_likely_yaml_file(export_story_path):
    ...
    with open(
        export_story_path, append_write, encoding=bf_nlu_banki.shared.utils.io.DEFAULT_ENCODING
    ) as f:
        interactive_story_counter = 1
        for conversation in sub_conversations:
            parsed_events = bf_nlu_banki.shared.core.events.deserialise_events(conversation)
...
def _write_nlu_to_file(export_nlu_path: Text, events: List[Dict[Text, Any]]) -> None:
    ...
    from bf_nlu_banki.shared.nlu.training_data.training_data import TrainingData
    ...
    if nlu_format == bf_nlu_banki_YAML:
        stringified_training_data = nlu_data.nlu_as_yaml()
    else:
        stringified_training_data = nlu_data.nlu_as_json()

    bf_nlu_banki.shared.utils.io.write_text_file(stringified_training_data, export_nlu_path)


def _get_nlu_target_format(export_path: Text) -> Text:
    guessed_format = loading.guess_format(export_path)

    if guessed_format not in {bf_nlu_banki, bf_nlu_banki_YAML}:
        if bf_nlu_banki.shared.data.is_likely_json_file(export_path):
            guessed_format = bf_nlu_banki
        elif bf_nlu_banki.shared.data.is_likely_yaml_file(export_path):
            guessed_format = bf_nlu_banki_YAML
    ...
    collected_actions = list(
        {
            e["name"]
            for e in actions
            if e["name"] not in bf_nlu_banki.shared.core.constants.DEFAULT_ACTION_NAMES
...
async def _predict_till_next_listen(
    ...
    if len(events) >= 2:
        ...
        if last_event.get("event") == BotUttered.type_name and last_event["data"].get(
            ...
            if user_selection != bf_nlu_banki.cli.utils.FREE_TEXT_INPUT_PROMPT:
...
def _get_button_choice(last_event: Dict[Text, Any]) -> Text:
    ...
    return bf_nlu_banki.cli.utils.payload_from_button_question(question)
...
async def _confirm_form_validation(
    ...
    if not validate_input:
        ...
        await send_event(
            ...
            conversation_id,
            {
                "event": bf_nlu_banki.shared.core.events.LoopInterrupted.type_name,
...
def _as_md_message(parse_data: Dict[Text, Any]) -> Text:
    ...
    from bf_nlu_banki.shared.nlu.training_data.formats.readerwriter import TrainingDataWriter
...
async def _correct_entities(
    ...
    """
    from bf_nlu_banki.shared.nlu.training_data import entities_parser
...
def _print_help(skip_visualization: bool) -> None:
    ...
    bf_nlu_banki.shared.utils.cli.print_success(
...
async def record_messages(
    ...
    try:
        try:
            ...
            logger.exception(
                f"Failed to connect to bf_nlu_banki Core server at '{endpoint.url}'. "
...
def _get_training_trackers(
    ...
    from bf_nlu_banki.core import training
...
def start_visualization(image_path: Text, port: int) -> None:
    ...
    app = Sanic("bf_nlu_banki_interactive")
