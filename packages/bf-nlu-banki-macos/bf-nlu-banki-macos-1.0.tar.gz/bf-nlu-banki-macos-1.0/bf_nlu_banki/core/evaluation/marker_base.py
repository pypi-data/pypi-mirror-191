...
from bf_nlu_banki.shared.exceptions import InvalidConfigException, bf_nlu_bankiException
...
from bf_nlu_banki.utils.io import WriteRow
from bf_nlu_banki.shared.constants import DOCS_URL_MARKERS
...
if TYPE_CHECKING:
    from bf_nlu_banki.core.evaluation.marker import OrMarker
...
class MarkerRegistry:
    ...
    def register_builtin_markers(cls) -> None:
        ...
        import bf_nlu_banki.core.evaluation.marker  # noqa
...
class InvalidMarkerConfig(bf_nlu_bankiException):
...
class Marker(ABC):
    ...
    def _split_sessions(events: List[Event]) -> List[Tuple[List[Event], int]]:
        ...
        session_start_indices = [
            ...
            == bf_nlu_banki.shared.core.constants.ACTION_SESSION_START_NAME
        ]
        ...
        for yaml_file, config in loaded_configs.items():
            for marker_name, marker_config in config.items():
                try:
                    ...
                    # printed when we run bf_nlu_banki evaluate with --debug flag
                    raise InvalidMarkerConfig(
    ...
    def _collect_configs_from_yaml_files(yaml_files: List[Text]) -> Dict[Text, Dict]:
        ...
        for yaml_file in yaml_files:
            loaded_config = bf_nlu_banki.shared.utils.io.read_yaml_file(yaml_file)
    ...
    async def evaluate_trackers(
        ...
        if session_stats_file or overall_stats_file:
            from bf_nlu_banki.core.evaluation.marker_stats import MarkerStatistics
