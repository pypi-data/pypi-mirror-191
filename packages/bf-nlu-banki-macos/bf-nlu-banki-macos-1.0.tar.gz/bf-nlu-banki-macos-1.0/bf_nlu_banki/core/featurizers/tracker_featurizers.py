...
from bf_nlu_banki.core.exceptions import InvalidTrackerFeaturizerUsageError
import bf_nlu_banki.shared.core.trackers
import bf_nlu_banki.shared.utils.io
from bf_nlu_banki.shared.nlu.constants import TEXT, INTENT, ENTITIES, ACTION_NAME
...
from bf_nlu_banki.shared.core.events import Event, ActionExecuted, UserUttered
...
from bf_nlu_banki.utils.tensorflow.constants import LABEL_PAD_ID
...
class InvalidStory(bf_nlu_bankiException):
...
class TrackerFeaturizer:
    ...
    def _choose_last_user_input(
        ...
        for states in trackers_as_states:
            ...
            if not bf_nlu_banki.shared.core.trackers.is_prev_action_listen_in_state(last_state):
                continue
    ...
    def persist(self, path: Union[Text, Path]) -> None:
        ...
        bf_nlu_banki.shared.utils.io.create_directory_for_file(featurizer_file)
    ...
    def load(path: Union[Text, Path]) -> Optional[TrackerFeaturizer]:
        ...
        if featurizer_file.is_file():
            return jsonpickle.decode(bf_nlu_banki.shared.utils.io.read_file(featurizer_file))
    ...
    def _cleanup_last_user_state_with_action_listen(
        ...
        for states in trackers_as_states:
            if not states:
                continue
            last_state = states[-1]
            if bf_nlu_banki.shared.core.trackers.is_prev_action_listen_in_state(last_state):
                del states[-1]
