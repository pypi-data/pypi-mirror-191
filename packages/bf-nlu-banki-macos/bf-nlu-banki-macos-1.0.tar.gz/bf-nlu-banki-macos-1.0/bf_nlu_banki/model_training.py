...
import bf_nlu_banki.engine.validation
from bf_nlu_banki.engine.caching import LocalTrainingCache
...
from bf_nlu_banki.engine.training.components import FingerprintStatus
from bf_nlu_banki.engine.training.graph_trainer import GraphTrainer
...
def _dry_run_result(
    ...
    if force_full_training:
        bf_nlu_banki.shared.utils.cli.print_warning("The training was forced.")
    ...
    if training_required:
        bf_nlu_banki.shared.utils.cli.print_warning("The model needs to be retrained.")
        return TrainingResult(
...
def train(
    ...
    output: Text = bf_nlu_banki.shared.constants.DEFAULT_MODELS_PATH,
    dry_run: bool = False,
    ...
    """Trains a bf_nlu_banki model (Core and NLU).
    ...
    if nlu_data.has_e2e_examples():
        bf_nlu_banki.shared.utils.common.mark_as_experimental_feature("end-to-end training")
    ...
    if stories.is_empty() and nlu_data.contains_no_pure_nlu_data():
        bf_nlu_banki.shared.utils.cli.print_error(
            ...
            "order to train a bf_nlu_banki model using the '--data' argument."
        )
    ...
    if domain_object.is_empty():
        ...
        bf_nlu_banki.shared.utils.cli.print_warning(
            "No stories present. Just a bf_nlu_banki NLU model will be trained."
        )
        training_type = TrainingType.NLU
    ...
    # We will train nlu if there are any nlu example, including from e2e stories.
        ...
        bf_nlu_banki.shared.utils.cli.print_warning(
            "No NLU data present. Just a bf_nlu_banki Core model will be trained."
        )
        training_type = TrainingType.CORE

    with telemetry.track_model_training(file_importer, model_type="bf_nlu_banki"):
        return _train_graph(
            file_importer,
...
def _train_graph(
    ...
    **kwargs: Any,
    ...
    if model_to_finetune:
        model_to_finetune = bf_nlu_banki.model.get_model_for_finetuning(model_to_finetune)
        ...
        bf_nlu_banki.shared.utils.common.mark_as_experimental_feature(
    ...
    bf_nlu_banki.engine.validation.validate(model_configuration)
    ...
    with tempfile.TemporaryDirectory() as temp_model_dir:
        ...
        with telemetry.track_model_training(
            ...
            bf_nlu_banki.shared.utils.cli.print_success(
                f"Your bf_nlu_banki model is trained and saved at '{full_model_path}'."
            )
...
def train_core(
    ...
    if nlu_data.has_e2e_examples():
        bf_nlu_banki.shared.utils.cli.print_error(
            "Stories file contains e2e stories. Please train using `bf_nlu_banki train` so that"
    ...
    if not stories_data:
        bf_nlu_banki.shared.utils.cli.print_error(
            ...
            "train a bf_nlu_banki Core model using the '--stories' argument."
        )
        return None
...
def train_nlu(
    ...
    if not nlu_data:
        bf_nlu_banki.shared.utils.cli.print_error(
            ...
            "a bf_nlu_banki NLU model using the '--nlu' argument."
        )
        return None
