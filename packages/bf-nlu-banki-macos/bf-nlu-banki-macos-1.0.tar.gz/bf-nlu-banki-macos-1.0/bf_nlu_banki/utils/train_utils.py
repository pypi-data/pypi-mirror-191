...
import bf_nlu_banki.nlu.utils.bilou_utils
from bf_nlu_banki.shared.constants import NEXT_MAJOR_VERSION_FOR_DEPRECATIONS
from bf_nlu_banki.nlu.constants import NUMBER_OF_SUB_TOKENS
...
from bf_nlu_banki.utils.tensorflow.callback import bf_nlu_bankiTrainingLogger, bf_nlu_bankiModelCheckpoint
from bf_nlu_banki.utils.tensorflow.data_generator import bf_nlu_bankiBatchDataGenerator
from bf_nlu_banki.utils.tensorflow.model_data import bf_nlu_bankiModelData
from bf_nlu_banki.shared.nlu.constants import SPLIT_ENTITIES_BY_COMMA
...
def _replace_deprecated_option(
    ...
    if isinstance(new_option, str):
        bf_nlu_banki.shared.utils.io.raise_deprecation_warning(
    ...
    for tag_spec in entity_tag_specs:
        ...
        if bilou_flag:
            (
                tags,
                confidences,
            ) = bf_nlu_banki.nlu.utils.bilou_utils.ensure_consistent_bilou_tagging(
                tags, confidences
            )
...
def create_data_generators(
    ...
    shuffle: bool = True,
) -> Tuple[bf_nlu_bankiBatchDataGenerator, Optional[bf_nlu_bankiBatchDataGenerator]]:
    ...
    if eval_num_examples > 0:
        ...
        validation_data_generator = bf_nlu_bankiBatchDataGenerator(
    ...
    data_generator = bf_nlu_bankiBatchDataGenerator(
        model_data,
        batch_size=batch_sizes,
        epochs=epochs,
        batch_strategy=batch_strategy,
...
def create_common_callbacks(
    ...
    - bf_nlu_bankiTrainingLogger callback
    - Optional TensorBoard callback
    - Optional bf_nlu_bankiModelCheckpoint callback
    ...
    callbacks = [bf_nlu_bankiTrainingLogger(epochs, silent=False)]
    ...
    if checkpoint_dir:
        callbacks.append(bf_nlu_bankiModelCheckpoint(checkpoint_dir))
...
def _check_evaluation_setting(component_config: Dict[Text, Any]) -> None:
    if (
        ...
        bf_nlu_banki.shared.utils.io.raise_warning(warning)
...
def _check_confidence_setting(component_config: Dict[Text, Any]) -> None:
    if component_config[MODEL_CONFIDENCE] == COSINE:
        raise InvalidConfigException(
            f"{MODEL_CONFIDENCE}={COSINE} was introduced in bf_nlu_banki Open Source 2.3.0 "
