...
from bf_nlu_banki.utils.tensorflow.model_data import FeatureSignature
...
from bf_nlu_banki.utils.tensorflow.exceptions import TFLayerConfigException
from bf_nlu_banki.utils.tensorflow.transformer import TransformerEncoder
from bf_nlu_banki.nlu.constants import DEFAULT_TRANSFORMER_SIZE


class bf_nlu_bankiCustomLayer(tf.keras.layers.Layer):
    """Parent class for all classes in `bf_nlu_banki_layers.py`.
    ...
    `ConcatenateSparseDenseFeatures`, `bf_nlu_bankiSequenceLayer` and
    `bf_nlu_bankiFeatureCombiningLayer` all inherit from `bf_nlu_bankiCustomLayer` and thus can
    ...
    def adjust_sparse_layers_for_incremental_training(
        ...
        for name, layer in self._tf_layers.items():
            if isinstance(layer, bf_nlu_bankiCustomLayer):
...
class ConcatenateSparseDenseFeatures(bf_nlu_bankiCustomLayer):
...
class bf_nlu_bankiFeatureCombiningLayer(bf_nlu_bankiCustomLayer):
    ...
    def __init__(
        ...
        """Creates a new `bf_nlu_bankiFeatureCombiningLayer` object."""
        ...
        super().__init__(name=f"bf_nlu_banki_feature_combining_layer_{attribute}")
...
class bf_nlu_bankiSequenceLayer(bf_nlu_bankiCustomLayer):
    ...
    1. Combine features using `bf_nlu_bankiFeatureCombiningLayer`.
    ...
    Output shape:
        outputs: `(batch_size, seq_length, units)` where `units` matches the underlying
            ...
            size of the underlying `bf_nlu_bankiFeatureCombiningLayer` if the `Ffnn` block has 0
            ...
            shape of `bf_nlu_bankiFeatureCombiningLayer`). If both feature types are present,
        ...
        seq_sent_features: `(batch_size, seq_length, hidden_dim)`, where `hidden_dim` is
            ...
            underlying `bf_nlu_bankiFeatureCombiningLayer` if the `Ffnn` block has 0 layers.
    ...
    def __init__(
        ...
        """Creates a new `bf_nlu_bankiSequenceLayer` object."""
        ...
        super().__init__(name=f"bf_nlu_banki_sequence_layer_{attribute}")

        self._tf_layers: Dict[Text, Any] = {
            self.FEATURE_COMBINING: bf_nlu_bankiFeatureCombiningLayer(
    ...
    def _calculate_output_units(
        ...
        pipeline that is `bf_nlu_bankiFeatureCombiningLayer` -> `Ffnn` -> `Transformer`, since
        ...
        # only the bf_nlu_bankiFeatureCombiningLayer is present
