...
from bf_nlu_banki.utils.tensorflow.constants import SEQUENCE, BALANCED
from bf_nlu_banki.utils.tensorflow.model_data import bf_nlu_bankiModelData, Data, FeatureArray

logger = logging.getLogger(__name__)


class bf_nlu_bankiDataGenerator(Sequence):
        ...
        for key, attribute_data in data.items():
            for sub_key, f_data in attribute_data.items():
                ...
                for v in f_data:
                    ...
                    if _data.is_sparse:
                        batch_data.extend(
                            bf_nlu_bankiDataGenerator._scipy_matrix_to_values(_data)
                        )
                    else:
                        batch_data.append(bf_nlu_bankiDataGenerator._pad_dense_data(_data))
    ...
    def _pad_dense_data(array_of_dense: FeatureArray) -> np.ndarray:
        ...
        if array_of_dense.number_of_dimensions == 4:
            return bf_nlu_bankiDataGenerator._pad_4d_dense_data(array_of_dense)
    ...
    def _pad_4d_dense_data(feature_array: FeatureArray) -> np.ndarray:
        ...
        array_of_array_of_dense = bf_nlu_bankiDataGenerator._filter_out_fake_inputs(
            feature_array
        )
    ...
    def _scipy_matrix_to_values(array_of_sparse: FeatureArray) -> List[np.ndarray]:
        ...
        if array_of_sparse.number_of_dimensions == 4:
            return bf_nlu_bankiDataGenerator._4d_scipy_matrix_to_values(array_of_sparse)
    ...
    def _4d_scipy_matrix_to_values(feature_array: FeatureArray) -> List[np.ndarray]:
        ...
        # input to tf methods.
        number_of_features = feature_array[0][0].shape[-1]
        array_of_array_of_sparse = bf_nlu_bankiDataGenerator._filter_out_fake_inputs(
            feature_array
        )
...
class bf_nlu_bankiBatchDataGenerator(bf_nlu_bankiDataGenerator):
