...
from bf_nlu_banki.shared.constants import DEFAULT_LOG_LEVEL, ENV_LOG_LEVEL, TCP_PROTOCOL
...
def read_global_config(path: Text) -> Dict[Text, Any]:
    """Read global bf_nlu_banki configuration.
    ...
    try:
        return bf_nlu_banki.shared.utils.io.read_config_file(path)
    except Exception:
...
def configure_logging_and_warnings(
    ...
    Args:
        log_level: The log level to be used for the 'bf_nlu_banki' logger. Pass `None` to use
    ...
    logging.getLogger("bf_nlu_banki").setLevel(log_level)
...
def write_global_config_value(name: Text, value: Any) -> bool:
    ...
    """
    # need to use `bf_nlu_banki.constants.GLOBAL_USER_CONFIG_PATH` to allow patching
    # in tests
    config_path = bf_nlu_banki.constants.GLOBAL_USER_CONFIG_PATH
    try:
        os.makedirs(os.path.dirname(config_path), exist_ok=True)

        c = read_global_config(config_path)
        c[name] = value
        bf_nlu_banki.shared.utils.io.write_yaml(c, bf_nlu_banki.constants.GLOBAL_USER_CONFIG_PATH)
        return True
...
def read_global_config_value(name: Text, unavailable_ok: bool = True) -> Any:
    """Read a value from the global bf_nlu_banki configuration."""
