...
from bf_nlu_banki.shared.importers import utils
...
from bf_nlu_banki.shared.utils.common import mark_as_experimental_feature
from bf_nlu_banki.shared.core.training_data.story_reader.yaml_story_reader import (
    YAMLStoryReader,
)
...
class MultiProjectImporter(TrainingDataImporter):
    def __init__(
        ...
        self.config = bf_nlu_banki.shared.utils.io.read_model_configuration(config_file)
        if domain_path:
        ...
        extra_nlu_files = bf_nlu_banki.shared.data.get_data_files(
            training_data_paths, bf_nlu_banki.shared.data.is_nlu_file
        )
        extra_story_files = bf_nlu_banki.shared.data.get_data_files(
    ...
    def _init_from_file(self, path: Text) -> None:
        ...
        if os.path.exists(path) and bf_nlu_banki.shared.data.is_config_file(path):
            config = bf_nlu_banki.shared.utils.io.read_config_file(path)
    ...
    def _is_in_imported_paths(self, path: Text) -> bool:
        return any(
            [bf_nlu_banki.shared.utils.io.is_subdirectory(path, i) for i in self._imports]
        )
