...
from bf_nlu_banki.shared.exceptions import YamlException
from bf_nlu_banki.shared.utils import validation
...
from bf_nlu_banki.shared.nlu.constants import METADATA_INTENT, METADATA_EXAMPLE
from bf_nlu_banki.shared.nlu.training_data.formats.readerwriter import (
    TrainingDataReader,
    TrainingDataWriter,
)
import bf_nlu_banki.shared.utils.io
import bf_nlu_banki.shared.nlu.training_data.util
...
class bf_nlu_bankiYAMLReader(TrainingDataReader):
    ...
    def reads(  # type: ignore[override]
        ...
        yaml_content = bf_nlu_banki.shared.utils.io.read_yaml(string)
    ...
    def _parse_intent(self, intent_data: Dict[Text, Any]) -> None:
        import bf_nlu_banki.shared.nlu.training_data.entities_parser as entities_parser
        import bf_nlu_banki.shared.nlu.training_data.synonyms_parser as synonyms_parser
    ...
    def _parse_lookup(self, nlu_item: Dict[Text, Any]) -> None:
        import bf_nlu_banki.shared.nlu.training_data.lookup_tables_parser as lookup_tables_parser  # noqa: E501
    ...
    def is_yaml_nlu_file(filename: Union[Text, Path]) -> bool:
        ...
        """
        if not bf_nlu_banki.shared.data.is_likely_yaml_file(filename):
            return False

        return bf_nlu_banki.shared.utils.io.is_key_in_yaml(filename, KEY_NLU, KEY_RESPONSES)


class bf_nlu_bankiYAMLWriter(TrainingDataWriter):
    ...
    def dump(
        ...
        if result:
            bf_nlu_banki.shared.utils.io.write_yaml(result, target, True)
    ...
    def process_intents(cls, training_data: "TrainingData") -> List[OrderedDict]:
        ...
        return bf_nlu_bankiYAMLWriter.process_training_examples_by_key(
    ...
    def process_training_examples_by_key(
        ...
        for intent_name, examples in training_examples.items():
            converted, intent_metadata = bf_nlu_bankiYAMLWriter._convert_training_examples(
            ...
            example_texts_have_escape_chars = any(
                bf_nlu_banki.shared.nlu.training_data.util.has_string_escape_chars(
            ...
            if examples_have_metadata or example_texts_have_escape_chars:
                intent[
                    key_examples
                ] = bf_nlu_bankiYAMLWriter._render_training_examples_as_objects(converted)
            else:
                intent[key_examples] = bf_nlu_bankiYAMLWriter._render_training_examples_as_text(
                    converted
                )
