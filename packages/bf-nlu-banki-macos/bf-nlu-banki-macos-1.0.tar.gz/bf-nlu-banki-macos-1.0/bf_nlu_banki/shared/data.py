...
def is_nlu_file(file_path: Text) -> bool:
    """Checks if a file is a bf_nlu_banki compatible nlu file.
    ...
    """
    from bf_nlu_banki.shared.nlu.training_data import loading as nlu_loading

    return nlu_loading.guess_format(file_path) != nlu_loading.UNK


def is_config_file(file_path: Text) -> bool:
    """Checks whether the given file path is a bf_nlu_banki config file.

    Args:
        file_path: Path of the file which should be checked.

    Returns:
        `True` if it's a bf_nlu_banki config file, otherwise `False`.
    """
