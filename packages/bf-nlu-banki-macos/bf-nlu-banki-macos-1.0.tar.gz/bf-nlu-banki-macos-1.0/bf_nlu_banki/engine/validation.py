...
from bf_nlu_banki.engine.training.fingerprinting import Fingerprintable
import bf_nlu_banki.utils.common
import typing_utils

from bf_nlu_banki.engine.exceptions import GraphSchemaValidationException
...
from bf_nlu_banki.engine.constants import RESERVED_PLACEHOLDERS
from bf_nlu_banki.engine.storage.resource import Resource
from bf_nlu_banki.engine.storage.storage import ModelStorage
from bf_nlu_banki.shared.constants import DOCS_URL_GRAPH_COMPONENTS
...
def _validate_interface_usage(node: SchemaNode) -> None:
    if not issubclass(node.uses, GraphComponent):
        raise GraphSchemaValidationException(
            ...
            f"and can hence not be run within bf_nlu_banki Open Source. Please use a different "
...
def _validate_required_packages(node: SchemaNode) -> None:
    missing_packages = bf_nlu_banki.utils.common.find_unavailable_packages(
