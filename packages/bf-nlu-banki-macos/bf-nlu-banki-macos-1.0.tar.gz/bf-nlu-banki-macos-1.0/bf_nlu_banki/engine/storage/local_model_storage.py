...
from bf_nlu_banki.engine.storage.storage import ModelMetadata, ModelStorage
from bf_nlu_banki.engine.graph import GraphModelConfiguration
from bf_nlu_banki.engine.storage.resource import Resource
from bf_nlu_banki.exceptions import UnsupportedModelVersionError
...
class LocalModelStorage(ModelStorage):
    ...
    def _extract_archive_to_directory(
        ...
        LocalModelStorage._assert_not_bf_nlu_banki2_archive(temporary_directory)

    @staticmethod
    def _assert_not_bf_nlu_banki2_archive(temporary_directory: Union[Text, Path]) -> None:
        fingerprint_file = Path(temporary_directory) / "fingerprint.json"
        if fingerprint_file.is_file():
            serialized_fingerprint = bf_nlu_banki.shared.utils.io.read_json_file(
                fingerprint_file
            )
    ...
    def _load_metadata(directory: Path) -> ModelMetadata:
        serialized_metadata = bf_nlu_banki.shared.utils.io.read_json_file(
    ...
    def _create_model_metadata(
        ...
        return ModelMetadata(
            ...
            bf_nlu_banki_open_source_version=bf_nlu_banki.__version__,
            ...
            project_fingerprint=bf_nlu_banki.model.project_fingerprint(),
