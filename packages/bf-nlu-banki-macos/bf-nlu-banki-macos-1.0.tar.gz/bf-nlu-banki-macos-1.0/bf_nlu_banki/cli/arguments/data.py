...
from bf_nlu_banki.shared.constants import DEFAULT_CONVERTED_DATA_PATH
...
def set_convert_arguments(parser: argparse.ArgumentParser, data_type: Text) -> None:
    ...
    add_out_param(
        ...
        "where to save training data in bf_nlu_banki format.",
    )
...
def set_migrate_arguments(parser: argparse.ArgumentParser) -> None:
    ...
    add_out_param(
        parser,
        default=None,
        help_text="Path (for `yaml`) where to save migrated domain in bf_nlu_banki 3.0 format."
