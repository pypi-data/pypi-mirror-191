...
import bf_nlu_banki.shared.core.domain
from bf_nlu_banki import telemetry
from bf_nlu_banki.cli import SubParsersAction
from bf_nlu_banki.cli.arguments import data as arguments
from bf_nlu_banki.cli.arguments import default_arguments
import bf_nlu_banki.cli.utils
from bf_nlu_banki.shared.constants import DEFAULT_DATA_PATH, DEFAULT_CONFIG_PATH
...
if TYPE_CHECKING:
    from bf_nlu_banki.validator import Validator
...
def add_subparser(
    ...
    data_parser = subparsers.add_parser(
        ...
        parents=parents,
        help="Utils for the bf_nlu_banki training files.",
    )
...
def _add_data_convert_parsers(
    ...
    convert_parser = data_subparsers.add_parser(
        ...
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
        parents=parents,
        help="Converts bf_nlu_banki data between different formats.",
    )
    ...
    arguments.set_convert_arguments(convert_nlu_parser, data_type="bf_nlu_banki NLU")
...
def _add_data_split_parsers(
    ...
    split_parser = data_subparsers.add_parser(
        ...
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
        parents=parents,
        help="Splits bf_nlu_banki data into training and test data.",
    )
...
def split_nlu_data(args: argparse.Namespace) -> None:
    ...
    """
    data_path = bf_nlu_banki.cli.utils.get_validated_path(args.nlu, "nlu", DEFAULT_DATA_PATH)
    data_path = bf_nlu_banki.shared.data.get_nlu_directory(data_path)

    nlu_data = bf_nlu_banki.shared.nlu.training_data.loading.load_data(data_path)
    extension = bf_nlu_banki.shared.nlu.training_data.util.get_file_format_extension(data_path)
...
def validate_files(args: argparse.Namespace, stories_only: bool = False) -> None:
    ...
    config = bf_nlu_banki.cli.utils.get_validated_path(
        args.config, "config", DEFAULT_CONFIG_PATH, none_is_valid=True
    )

    file_importer = bf_nlu_bankiFileImporter(
...
def _convert_nlu_data(args: argparse.Namespace) -> None:
    import bf_nlu_banki.nlu.convert

    if args.format in ["json", "yaml"]:
        bf_nlu_banki.nlu.convert.convert_training_data(
...
def _add_data_migrate_parsers(
    ...
    migrate_parser = data_subparsers.add_parser(
        ...
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
        parents=parents,
        help="Converts bf_nlu_banki domain 2.0 format to required format for 3.0.",
    )
...
def _migrate_domain(args: argparse.Namespace) -> None:
    import bf_nlu_banki.core.migrate

    bf_nlu_banki.core.migrate.migrate_domain_format(args.domain, args.out)
