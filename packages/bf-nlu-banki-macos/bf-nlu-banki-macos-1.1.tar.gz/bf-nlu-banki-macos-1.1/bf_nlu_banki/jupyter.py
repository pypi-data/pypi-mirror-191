...
import bf_nlu_banki.core.agent
...
def chat(
    ...
    Args:
        model_path: Path to a combined bf_nlu_banki model.
        endpoints: Path to a yaml with the action server is custom actions are defined.
        agent: bf_nlu_banki Core agent (used if no bf_nlu_banki model given).
    """
    if model_path:
        agent = asyncio.run(
            bf_nlu_banki.core.agent.load_agent(model_path=model_path, endpoints=endpoints)
        )
