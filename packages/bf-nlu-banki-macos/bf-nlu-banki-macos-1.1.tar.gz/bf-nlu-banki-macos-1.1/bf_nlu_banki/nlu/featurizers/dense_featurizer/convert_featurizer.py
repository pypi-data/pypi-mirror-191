...
from bf_nlu_banki.nlu.tokenizers.tokenizer import Token, Tokenizer
from bf_nlu_banki.nlu.featurizers.dense_featurizer.dense_featurizer import DenseFeaturizer
...
from bf_nlu_banki.shared.nlu.constants import TEXT, ACTION_TEXT
from bf_nlu_banki.exceptions import bf_nlu_bankiException
import bf_nlu_banki.nlu.utils
import bf_nlu_banki.utils.train_utils as train_utils
...
class ConveRTFeaturizer(DenseFeaturizer, GraphComponent):
    ...
    def __init__(self, name: Text, config: Dict[Text, Any]) -> None:
        ...
        self.model_url = (
            model_url
            if bf_nlu_banki.nlu.utils.is_url(model_url)
    ...
    def _validate_model_url(cls, config: Dict[Text, Any]) -> None:
        ...
        if model_url == RESTRICTED_ACCESS_URL:
            ...
            raise bf_nlu_bankiException(
                ...
                f"bf_nlu_banki Open Source only. Due to licensing issues you are "
        ...
        if not bf_nlu_banki.nlu.utils.is_url(model_url) and not os.path.isdir(model_url):
            raise bf_nlu_bankiException(
