...
from bf_nlu_banki.nlu.featurizers.featurizer import Featurizer
...
from bf_nlu_banki.engine.graph import ExecutionContext, GraphComponent
from bf_nlu_banki.engine.recipes.default_recipe import DefaultV1Recipe
from bf_nlu_banki.engine.storage.resource import Resource
from bf_nlu_banki.engine.storage.storage import ModelStorage
from bf_nlu_banki.nlu.extractors.extractor import EntityExtractorMixin
from bf_nlu_banki.nlu.classifiers.classifier import IntentClassifier
import bf_nlu_banki.shared.utils.io
import bf_nlu_banki.utils.io as io_utils
import bf_nlu_banki.nlu.utils.bilou_utils as bilou_utils
from bf_nlu_banki.shared.constants import DIAGNOSTIC_DATA
from bf_nlu_banki.nlu.extractors.extractor import EntityTagSpec
from bf_nlu_banki.nlu.classifiers import LABEL_RANKING_LENGTH
from bf_nlu_banki.utils import train_utils
from bf_nlu_banki.utils.tensorflow import bf_nlu_banki_layers
from bf_nlu_banki.utils.tensorflow.models import bf_nlu_bankiModel, Transformerbf_nlu_bankiModel
from bf_nlu_banki.utils.tensorflow.model_data import (
    bf_nlu_bankiModelData,
    FeatureSignature,
    FeatureArray,
)
from bf_nlu_banki.nlu.constants import TOKENS_NAMES, DEFAULT_TRANSFORMER_SIZE
...
from bf_nlu_banki.utils.tensorflow.constants import (
    LABEL,
    IDS,
    HIDDEN_LAYERS_SIZES,
...
class DIETClassifier(GraphComponent, IntentClassifier, EntityExtractorMixin):
    ...
    def __init__(
        ...
        model: Optional[bf_nlu_bankiModel] = None,
        ...
        if self.component_config[CHECKPOINT_MODEL]:
            self.tmp_checkpoint_dir = Path(bf_nlu_banki.utils.io.create_temporary_directory())

        self._label_data: Optional[bf_nlu_bankiModelData] = None
        self._data_example: Optional[Dict[Text, Dict[Text, List[FeatureArray]]]] = None

        self.split_entities_config = bf_nlu_banki.utils.train_utils.init_split_entities(
    ...
    @staticmethod
    def model_class() -> Type[bf_nlu_bankiModel]:
        return DIET
    ...
    def _check_input_dimension_consistency(self, model_data: bf_nlu_bankiModelData) -> None:
    ...
    def _create_label_data(
        ...
        label_data = bf_nlu_bankiModelData()
    ...
    def _create_model_data(
        ...
        """Prepare data for training and create a bf_nlu_bankiModelData object."""
        from bf_nlu_banki.utils.tensorflow import model_data_utils
        ...
        model_data = bf_nlu_bankiModelData(
    ...
    # train helpers
    def preprocess_train_data(self, training_data: TrainingData) -> bf_nlu_bankiModelData:
    ...
    @staticmethod
    def _check_enough_labels(model_data: bf_nlu_bankiModelData) -> bool:
    ...
    def train(self, training_data: TrainingData) -> Resource:
        ...
        if not self.model and self.finetune_mode:
            raise bf_nlu_banki.shared.exceptions.InvalidParameterException(
    ...
    def persist(self) -> None:
        ...
        with self._model_storage.write_to(self._resource) as model_path:
            ...
            bf_nlu_banki.shared.utils.io.create_directory_for_file(tf_model_file)
            ...
            bf_nlu_banki.shared.utils.io.dump_obj_as_json_to_file(
    ...
    def _load_from_files(
        ...
        label_data = bf_nlu_bankiModelData(data=label_data)
        ...
        entity_tag_specs = bf_nlu_banki.shared.utils.io.read_json_file(
    ...
    def _load_model(
        ...
        model_data_example = bf_nlu_bankiModelData(
    ...
    def _load_model_class(
        ...
        predict_data_example = bf_nlu_bankiModelData(
    ...
    def _instantiate_model_class(self, model_data: bf_nlu_bankiModelData) -> "bf_nlu_bankiModel":
...
class DIET(Transformerbf_nlu_bankiModel):
    def __init__(
        self,
    ...
    def _update_metrics_to_log(self) -> None:
        debug_log_level = logging.getLogger("bf_nlu_banki").level == logging.DEBUG
    ...
    def _prepare_layers(self) -> None:
        ...
        ] = bf_nlu_banki_layers.bf_nlu_bankiSequenceLayer(
        ...
        if self.config[INTENT_CLASSIFICATION]:
            ...
            ] = bf_nlu_banki_layers.bf_nlu_bankiFeatureCombiningLayer(
