...
FINGERPRINT_VERSION = "bf_nlu_banki-version"
...
class FinetuningValidator(GraphComponent):
    ...
    def _validate(self, importer: TrainingDataImporter) -> None:
        ...
        if self._is_finetuning:
            old_bf_nlu_banki_version = self._fingerprints[FINGERPRINT_VERSION]
            if version.parse(old_bf_nlu_banki_version) < version.parse(
                MINIMUM_COMPATIBLE_VERSION
            ):
                raise InvalidConfigException(
                    f"The minimum compatible bf_nlu_banki Version is "
                    f"{MINIMUM_COMPATIBLE_VERSION} but the model we attempt to "
                    f"finetune has been generated with an older version "
                    f"({old_bf_nlu_banki_version}."
                )
        self._fingerprints[FINGERPRINT_VERSION] = bf_nlu_banki_version
    ...
    def _get_fingerprint_of_schema_without_irrelevant_keys(self) -> Text:
        ...
        return bf_nlu_banki.shared.utils.io.deep_container_fingerprint(schema_as_dict)
        ...
        **kwargs: Any,
        ...
        try:
            with model_storage.read_from(resource) as path:
                fingerprints = bf_nlu_banki.shared.utils.io.read_json_file(
