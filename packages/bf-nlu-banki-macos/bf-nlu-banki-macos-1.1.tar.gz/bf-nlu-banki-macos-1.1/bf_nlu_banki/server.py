...
import bf_nlu_banki
import bf_nlu_banki.core.utils
from bf_nlu_banki.nlu.emulators.emulator import Emulator
import bf_nlu_banki.utils.common
import bf_nlu_banki.shared.utils.common
import bf_nlu_banki.shared.utils.io
import bf_nlu_banki.utils.endpoints
...
from bf_nlu_banki.shared.nlu.training_data.formats.bf_nlu_banki_yaml import bf_nlu_bankiYAMLReader
from bf_nlu_banki.core.constants import DEFAULT_RESPONSE_TIMEOUT
...
from bf_nlu_banki.shared.core.domain import InvalidDomain, Domain
from bf_nlu_banki.core.agent import Agent
from bf_nlu_banki.core.channels.channel import (
    CollectingOutputChannel,
    OutputChannel,
    UserMessage,
)
import bf_nlu_banki.shared.core.events
from bf_nlu_banki.shared.core.events import Event
from bf_nlu_banki.core.test import test
from bf_nlu_banki.shared.core.trackers import DialogueStateTracker, EventVerbosity
from bf_nlu_banki.core.utils import AvailableEndpoints
from bf_nlu_banki.nlu.emulators.no_emulator import NoEmulator
import bf_nlu_banki.nlu.test
from bf_nlu_banki.nlu.test import CVEvaluationResult
from bf_nlu_banki.shared.utils.schemas.events import EVENTS_SCHEMA
from bf_nlu_banki.utils.endpoints import EndpointConfig

if TYPE_CHECKING:
    from ssl import SSLContext  # noqa: F401
    from bf_nlu_banki.core.processor import MessageProcessor
...
class ErrorResponse(Exception):
    ...
    def __init__(
        ...
        """
        self.error_info = {
            "version": bf_nlu_banki.__version__,
            "status": "failure",
            "message": message,
            "reason": reason,
            "details": details or {},
...
def requires_auth(
    ...
    def decorator(f: "SanicView") -> "SanicView":
        def conversation_id_from_args(args: Any, kwargs: Any) -> Optional[Text]:
            argnames = bf_nlu_banki.shared.utils.common.arguments_of(f)
        ...
        async def sufficient_scope(
            ...
            jwt_data = await bf_nlu_banki.utils.common.call_potential_coroutine(
        ...
        async def decorated(
            ...
            if token is not None and provided == token:
                ...
                "USE_JWT"
            ) and await bf_nlu_banki.utils.common.call_potential_coroutine(
                # This is a coroutine since `sanic-jwt==1.6`
...
async def update_conversation_with_events(
    ...
    """
    if bf_nlu_banki.shared.core.events.do_events_begin_with_session_start(events):
...
async def authenticate(_: Request) -> NoReturn:
    ...
    raise exceptions.AuthenticationFailed(
        ...
        "a valid JWT from an authentication provider, bf_nlu_banki will just make "
...
def _create_emulator(mode: Optional[Text]) -> Emulator:
    ...
    If no emulator is specified, we will use the bf_nlu_banki NLU format.
    """
    if mode is None:
        return NoEmulator()
    elif mode.lower() == "wit":
        from bf_nlu_banki.nlu.emulators.wit import WitEmulator

        return WitEmulator()
    elif mode.lower() == "luis":
        from bf_nlu_banki.nlu.emulators.luis import LUISEmulator

        return LUISEmulator()
    elif mode.lower() == "dialogflow":
        from bf_nlu_banki.nlu.emulators.dialogflow import DialogflowEmulator
...
async def _load_agent(
    ...
    try:
        loaded_agent = await bf_nlu_banki.core.agent.load_agent(
...
def add_root_route(app: Sanic) -> None:
    ...
    async def hello(request: Request) -> HTTPResponse:
        ...
        return response.text("Hello from bf_nlu_banki: " + bf_nlu_banki.__version__)
...
def create_app(
    ...
    """Class representing a bf_nlu_banki HTTP server."""
    app = Sanic("bf_nlu_banki_server")
    ...
    async def version(request: Request) -> HTTPResponse:
        """Respond with the version number of the installed bf_nlu_banki."""
    ...
    async def retrieve_tracker(request: Request, conversation_id: Text) -> HTTPResponse:
        ...
        until_time = bf_nlu_banki.utils.endpoints.float_arg(request, "until")
    ...
    async def append_events(request: Request, conversation_id: Text) -> HTTPResponse:
        ...
        try:
            async with app.ctx.agent.lock_store.lock(conversation_id):
                ...
                if bf_nlu_banki.utils.endpoints.bool_arg(
    ...
    async def retrieve_story(request: Request, conversation_id: Text) -> HTTPResponse:
        ...
        until_time = bf_nlu_banki.utils.endpoints.float_arg(request, "until")
        fetch_all_sessions = bf_nlu_banki.utils.endpoints.bool_arg(
    ...
    async def train(request: Request, temporary_directory: Path) -> HTTPResponse:
        ...
        try:
            ...
            from bf_nlu_banki.model_training import train
    ...
    async def evaluate_stories(
        ...
        e2e = bf_nlu_banki.utils.endpoints.bool_arg(request, "e2e", default=False)
    ...
    async def evaluate_intents(
        ...
        """Evaluate intents against a bf_nlu_banki model."""
        validate_request_body(
            request,
    ...
    async def _evaluate_model_using_test_set(
        ...
        return await bf_nlu_banki.nlu.test.run_evaluation(
    ...
    async def _cross_validate(data_file: Text, config_file: Text, folds: int) -> Dict:
        ...
        evaluations = await bf_nlu_banki.nlu.test.cross_validate(
            data=nlu_data,
            n_folds=folds,
            nlu_config=config,
...
def _training_payload_from_yaml(
    ...
    decoded = request.body.decode(bf_nlu_banki.shared.utils.io.DEFAULT_ENCODING)
    _validate_yaml_training_payload(decoded)

    training_data = temp_dir / file_name
    bf_nlu_banki.shared.utils.io.write_text_file(decoded, training_data)

    model_output_directory = str(temp_dir)
    if bf_nlu_banki.utils.endpoints.bool_arg(request, "save_to_default_model_directory", True):
    ...
    return dict(
        ...
        force_training=bf_nlu_banki.utils.endpoints.bool_arg(request, "force_training", False),
...
def _validate_yaml_training_payload(yaml_text: Text) -> None:
    try:
        bf_nlu_bankiYAMLReader().validate(yaml_text)
    except Exception as e:
        raise ErrorResponse(
            HTTPStatus.BAD_REQUEST,
            "BadRequest",
...
def _extract_core_additional_arguments(request: Request) -> Dict[Text, Any]:
    return {
        "augmentation_factor": bf_nlu_banki.utils.endpoints.int_arg(request, "augmentation", 50)
    }


def _extract_nlu_additional_arguments(request: Request) -> Dict[Text, Any]:
    return {"num_threads": bf_nlu_banki.utils.endpoints.int_arg(request, "num_threads", 1)}
