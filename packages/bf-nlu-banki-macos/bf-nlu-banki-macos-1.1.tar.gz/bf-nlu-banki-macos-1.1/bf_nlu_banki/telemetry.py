...
import bf_nlu_banki
from bf_nlu_banki import model
...
from bf_nlu_banki.shared.constants import DOCS_URL_TELEMETRY
from bf_nlu_banki.shared.exceptions import bf_nlu_bankiException
import bf_nlu_banki.shared.utils.io
from bf_nlu_banki.utils import common as bf_nlu_banki_utils
import bf_nlu_banki.utils.io

if typing.TYPE_CHECKING:
    from bf_nlu_banki.core.brokers.broker import EventBroker
    from bf_nlu_banki.core.tracker_store import TrackerStore
    from bf_nlu_banki.core.channels.channel import InputChannel
    from bf_nlu_banki.core.agent import Agent
    from bf_nlu_banki.shared.nlu.training_data.training_data import TrainingData
    from bf_nlu_banki.shared.importers.importer import TrainingDataImporter
...
TELEMETRY_ENABLED_ENVIRONMENT_VARIABLE = "bf_nlu_banki_TELEMETRY_ENABLED"
TELEMETRY_DEBUG_ENVIRONMENT_VARIABLE = "bf_nlu_banki_TELEMETRY_DEBUG"

# the environment variable can be used for local development to set a test key
# e.g. `bf_nlu_banki_TELEMETRY_WRITE_KEY=12354 bf_nlu_banki train`
TELEMETRY_WRITE_KEY_ENVIRONMENT_VARIABLE = "bf_nlu_banki_TELEMETRY_WRITE_KEY"
EXCEPTION_WRITE_KEY_ENVIRONMENT_VARIABLE = "bf_nlu_banki_EXCEPTION_WRITE_KEY"
...
# https://bf_nlu_banki.com/docs/bf_nlu_banki/telemetry
...
TELEMETRY_bf_nlu_banki_X_LOCAL_STARTED_EVENT = "bf_nlu_banki X Local Started"
...
def print_telemetry_reporting_info() -> None:
    ...
    message = textwrap.dedent(
        f"""
        bf_nlu_banki Open Source reports anonymous usage telemetry to help improve the product
        for all its users.

        If you'd like to opt-out, you can use `bf_nlu_banki telemetry disable`.
...
def _write_default_telemetry_configuration(
    ...
    success = bf_nlu_banki_utils.write_global_config_value(
...
def _is_telemetry_enabled_in_configuration() -> bool:
    """Read telemetry configuration from the user's bf_nlu_banki config file in $HOME.
    ...
    """
    try:
        stored_config = bf_nlu_banki_utils.read_global_config_value(
...
def is_telemetry_enabled() -> bool:
    ...
    try:
        return bf_nlu_banki_utils.read_global_config_value(
            CONFIG_FILE_TELEMETRY_KEY, unavailable_ok=False
        )[CONFIG_TELEMETRY_ENABLED]
    except ValueError:
        return False
...
def _fetch_write_key(tool: Text, environment_variable: Text) -> Optional[Text]:
    ...
    from bf_nlu_banki import __name__ as name
...
def telemetry_write_key() -> Optional[Text]:
    ...
    versions of bf_nlu_banki Open Source. This avoids running telemetry locally when
    developing on bf_nlu_banki or when running CI builds.
...
def _is_docker() -> bool:
    ...
    try:
        return "docker" in bf_nlu_banki.shared.utils.io.read_file("/proc/self/cgroup", "utf8")
    except Exception:  # skipcq:PYL-W0703
        return False
...
def _default_context_fields() -> Dict[Text, Any]:
    ...
    if not TELEMETRY_CONTEXT:
        ...
        TELEMETRY_CONTEXT = {
            ...
            "bf_nlu_banki_open_source": bf_nlu_banki.__version__,
...
def _track(
    ...
    events, or events that are generated from many parts of the bf_nlu_banki Open Source code.
...
def get_telemetry_id() -> Optional[Text]:
    """Return the unique telemetry identifier for this bf_nlu_banki Open Source install.
    ...
    try:
        telemetry_config = (
            bf_nlu_banki_utils.read_global_config_value(CONFIG_FILE_TELEMETRY_KEY) or {}
        )
...
def toggle_telemetry_reporting(is_enabled: bool) -> None:
    ...
    Args:
        is_enabled: `True` if the telemetry reporting should be enabled,
            `False` otherwise.
    """
    configuration = bf_nlu_banki_utils.read_global_config_value(CONFIG_FILE_TELEMETRY_KEY)
    ...
    bf_nlu_banki_utils.write_global_config_value(CONFIG_FILE_TELEMETRY_KEY, configuration)
...
def initialize_error_reporting() -> None:
    ...
    # https://sentry.io/settings/bf_nlu_bankihq/projects/bf_nlu_banki-open-source/install/python/
    # for local testing, set the key using `bf_nlu_banki_EXCEPTION_WRITE_KEY=key bf_nlu_banki <command>`
    ...
    sentry_sdk.init(
        ...
        ignore_errors=[
            ...
            # expected bf_nlu_banki errors
            bf_nlu_bankiException,
            OSError,
        ],
        in_app_include=["bf_nlu_banki"],  # only submit errors in this package
        with_locals=False,  # don't submit local variables
        release=f"bf_nlu_banki-{bf_nlu_banki.__version__}",
...
def track_model_training(
    ...
    Args:
        ...
        model_type: Specifies the type of training, should be either "bf_nlu_banki", "core"
            or "nlu".
...
def track_server_start(
    ...
    """Tracks when a user starts a bf_nlu_banki server.
    ...
    Args:
        ...
        is_api_enabled: whether the bf_nlu_banki API server is enabled
    """
    from bf_nlu_banki.core.utils import AvailableEndpoints
...
def track_project_init(path: Text) -> None:
    """Track when a user creates a project using bf_nlu_banki init.
...
def track_shell_started(model_type: Text) -> None:
    """Track when a user starts a bot using bf_nlu_banki shell.

    Args:
        model_type: Type of the model, core / nlu or bf_nlu_banki.
    """
    _track(TELEMETRY_SHELL_STARTED_EVENT, {"type": model_type})


@ensure_telemetry_enabled
def track_bf_nlu_banki_x_local() -> None:
    """Track when a user runs bf_nlu_banki X in local mode."""
    _track(TELEMETRY_bf_nlu_banki_X_LOCAL_STARTED_EVENT)
