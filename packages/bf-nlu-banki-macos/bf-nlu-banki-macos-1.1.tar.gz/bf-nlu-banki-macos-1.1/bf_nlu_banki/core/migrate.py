...
from bf_nlu_banki.shared.constants import REQUIRED_SLOTS_KEY, IGNORED_INTENTS
...
from bf_nlu_banki.shared.constants import LATEST_TRAINING_DATA_FORMAT_VERSION
from bf_nlu_banki.shared.core.domain import KEY_ENTITIES, KEY_SLOTS, KEY_FORMS, Domain
...
def _create_back_up(domain_file: Path, backup_location: Path) -> Dict[Text, Any]:
    ...
    original_content = bf_nlu_banki.shared.utils.io.read_yaml(
        bf_nlu_banki.shared.utils.io.read_file(domain_file)
    )
    bf_nlu_banki.shared.utils.io.write_yaml(
...
def _write_final_domain(
    ...
    if domain_file.is_dir():
        for file in domain_file.iterdir():
            ...
            bf_nlu_banki.shared.utils.io.write_yaml(new_domain, out_file / file.name, True)
    else:
        new_domain = _assemble_new_domain(domain_file, new_forms, new_slots)
        bf_nlu_banki.shared.utils.io.write_yaml(new_domain, out_file, True)
...
def migrate_domain_format(
    ...
    original_files = (
        {
            file: bf_nlu_banki.shared.utils.io.read_yaml_file(file)
            for file in domain_path.iterdir()
            if Domain.is_domain_file(file)
        }
        if domain_path.is_dir()
        else {domain_path: bf_nlu_banki.shared.utils.io.read_yaml_file(domain_path)}
    )
    migrated_files = []
    ...
    if migrated_files:
        raise bf_nlu_bankiException(
            ...
            f"have already been migrated to bf_nlu_banki 3.0 format. Please remove these "
