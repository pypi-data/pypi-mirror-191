...
from bf_nlu_banki.core.constants import bf_nlu_banki_EXPORT_PROCESS_ID_HEADER_NAME
from bf_nlu_banki.core.tracker_store import TrackerStore
from bf_nlu_banki.shared.core.trackers import EventVerbosity
from bf_nlu_banki.exceptions import (
...
class Exporter:
    ...
    def _get_message_headers(self) -> Optional[Dict[Text, Text]]:
        ...
        Returns:
            ...
            `bf_nlu_banki_EXPORT_PROCESS_ID_HEADER_NAME` key if `self.event_broker` is a
            `PikaEventBroker`, else `None`.

        """
        if isinstance(self.event_broker, PikaEventBroker):
            return {bf_nlu_banki_EXPORT_PROCESS_ID_HEADER_NAME: uuid.uuid4().hex}
