...
from bf_nlu_banki.cli import utils as cli_utils
from bf_nlu_banki.core import utils
from bf_nlu_banki.core.channels.rest import RestInput
from bf_nlu_banki.core.constants import DEFAULT_SERVER_URL, DEFAULT_STREAM_READING_TIMEOUT
from bf_nlu_banki.shared.constants import INTENT_MESSAGE_PREFIX
from bf_nlu_banki.shared.utils.io import DEFAULT_ENCODING

logger = logging.getLogger(__name__)

STREAM_READING_TIMEOUT_ENV = "bf_nlu_banki_SHELL_STREAM_READING_TIMEOUT_IN_SECONDS"


def print_buttons(
    message: Dict[Text, Any],
    is_latest_message: bool = False,
    color: Text = bf_nlu_banki.shared.utils.io.bcolors.OKBLUE,
    ...
    if is_latest_message:
        ...
        bf_nlu_banki.shared.utils.cli.print_color("Buttons:", color=color)
        for idx, button in enumerate(message.get("buttons")):
            bf_nlu_banki.shared.utils.cli.print_color(
...
def _print_bot_output(
    ...
    if "text" in message:
        bf_nlu_banki.shared.utils.cli.print_color(message["text"], color=color)

    if "image" in message:
        bf_nlu_banki.shared.utils.cli.print_color("Image: " + message["image"], color=color)
    ...
    if "elements" in message:
        bf_nlu_banki.shared.utils.cli.print_color("Elements:", color=color)
    ...
    if "quick_replies" in message:
        bf_nlu_banki.shared.utils.cli.print_color("Quick Replies:", color=color)
    ...
    if "custom" in message:
        bf_nlu_banki.shared.utils.cli.print_color("Custom json:", color=color)
        bf_nlu_banki.shared.utils.cli.print_color(
            bf_nlu_banki.shared.utils.io.json_to_string(message["custom"]), color=color
        )
...
def _get_stream_reading_timeout(request_timeout: Optional[int] = None) -> ClientTimeout:
    ...
    Lastly fallback to `DEFAULT_STREAM_READING_TIMEOUT` from bf_nlu_banki.core.constants
    """
    timeout_str = (
        request_timeout
