...
def get_file_format_extension(resource_name: Text) -> Text:
    ...
    if not file_formats:
        return bf_nlu_banki.shared.data.yaml_file_extension()

    known_file_formats = {loading.bf_nlu_banki_YAML: bf_nlu_banki.shared.data.yaml_file_extension()}
    fformat = file_formats[0]
    if all(f == fformat for f in file_formats):
        return known_file_formats.get(fformat, bf_nlu_banki.shared.data.yaml_file_extension())
...
def remove_untrainable_entities_from(example: Dict[Text, Any]) -> None:
    ...
    are defined in `bf_nlu_banki.nlu.constants.PRETRAINED_EXTRACTORS`.
