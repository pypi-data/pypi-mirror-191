...
from bf_nlu_banki.shared.exceptions import InvalidEntityFormatException
...
def find_entities_in_training_example(example: Text) -> List[Dict[Text, Any]]:
    ...
    for match in re.finditer(ENTITY_REGEX, example):
        ...
        if match.groupdict()[GROUP_ENTITY_DICT] or match.groupdict()[GROUP_ENTITY_TYPE]:
            ...
            entity = bf_nlu_banki.shared.nlu.training_data.util.build_entity(
                start_index,
                end_index,
...
def get_validated_dict(json_str: Text) -> Dict[Text, Text]:
    ...
    """
    import json
    import bf_nlu_banki.shared.utils.validation as validation_utils
    import bf_nlu_banki.shared.nlu.training_data.schemas.data_schema as schema
