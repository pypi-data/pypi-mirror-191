...
from bf_nlu_banki.shared.utils.common import lazy_property
...
class TrainingData:
    ...
    def _load_lookup_table(lookup_table: Dict[Text, Any]) -> Dict[Text, Any]:
        ...
        if Path(potential_file).is_file():
            try:
                lookup_table["elements"] = bf_nlu_banki.shared.utils.io.read_file(
                    potential_file
                )
                return lookup_table
    ...
    def fingerprint(self) -> Text:
        ...
        return bf_nlu_banki.shared.utils.io.deep_container_fingerprint(relevant_attributes)
    ...
    def label_fingerprint(self) -> Text:
        ...
        return bf_nlu_banki.shared.utils.io.deep_container_fingerprint(labels)
    ...
    def nlu_as_json(self, **kwargs: Any) -> Text:
        ...
        from bf_nlu_banki.shared.nlu.training_data.formats import bf_nlu_bankiWriter

        return bf_nlu_bankiWriter().dumps(self, **kwargs)
    ...
    def nlg_as_yaml(self) -> Text:
        ...
        return bf_nlu_bankiYAMLWriter().dumps(TrainingData(responses=self.responses))
    ...
    def nlu_as_yaml(self) -> Text:
        ...
        return bf_nlu_bankiYAMLWriter().dumps(no_responses_training_data)

    def persist_nlu(self, filename: Text = DEFAULT_TRAINING_DATA_OUTPUT_PATH) -> None:
        """Saves NLU to a file."""
        if bf_nlu_banki.shared.data.is_likely_json_file(filename):
            bf_nlu_banki.shared.utils.io.write_text_file(self.nlu_as_json(indent=2), filename)
        elif bf_nlu_banki.shared.data.is_likely_yaml_file(filename):
            bf_nlu_banki.shared.utils.io.write_text_file(self.nlu_as_yaml(), filename)
        else:
            raise ValueError(
    ...
    def persist_nlg(self, filename: Text) -> None:
        """Saves NLG to a file."""
        if bf_nlu_banki.shared.data.is_likely_yaml_file(filename):
            bf_nlu_banki.shared.utils.io.write_text_file(self.nlg_as_yaml(), filename)
        else:
            raise ValueError(
    ...
    def get_nlg_persist_filename(nlu_filename: Text) -> Text:
        ...
        if bf_nlu_banki.shared.data.is_likely_json_file(nlu_filename):
            ...
            extension = bf_nlu_banki.shared.data.yaml_file_extension()
