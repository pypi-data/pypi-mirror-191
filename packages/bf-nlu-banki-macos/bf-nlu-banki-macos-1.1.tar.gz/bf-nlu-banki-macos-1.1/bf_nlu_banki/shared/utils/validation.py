...
def validate_yaml_schema(yaml_file_content: Text, schema_path: Text) -> None:
    ...
    try:
        ...
        # right line
        source_data = bf_nlu_banki.shared.utils.io.read_yaml(
    ...
    schema_content = bf_nlu_banki.shared.utils.io.read_yaml_file(schema_file)
    schema_utils_content = bf_nlu_banki.shared.utils.io.read_yaml_file(schema_utils_file)
...
def validate_training_data(json_data: Dict[Text, Any], schema: Dict[Text, Any]) -> None:
    """Validate bf_nlu_banki training data format to ensure proper training.
...
def validate_training_data_format_version(
    ...
    Returns:
        `True` if the file can be processed by current version of bf_nlu_banki Open Source,
        `False` otherwise.
    """
    ...
    if not version_value:
        ...
        logger.info(
            ...
            f"bf_nlu_banki Open Source will read the file as a "
    ...
    try:
        ...
        if parsed_version < latest_version:
            bf_nlu_banki.shared.utils.io.raise_warning(
                ...
                f"format version than your bf_nlu_banki Open Source installation: "
                f"{version_value} < {LATEST_TRAINING_DATA_FORMAT_VERSION}. "
                f"bf_nlu_banki Open Source will read the file as a version "
    ...
    bf_nlu_banki.shared.utils.io.raise_warning(
        ...
        f"format version than your bf_nlu_banki Open Source installation: "
        f"{version_value} > {LATEST_TRAINING_DATA_FORMAT_VERSION}. "
        f"Please consider updating to the latest version of bf_nlu_banki Open Source."
