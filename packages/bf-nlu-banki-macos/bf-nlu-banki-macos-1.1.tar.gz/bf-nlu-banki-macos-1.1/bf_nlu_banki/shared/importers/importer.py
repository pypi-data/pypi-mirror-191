...
from bf_nlu_banki.shared.core.events import ActionExecuted, UserUttered
from bf_nlu_banki.shared.core.training_data.structures import StoryGraph
...
from bf_nlu_banki.shared.nlu.constants import ENTITIES, ACTION_NAME
from bf_nlu_banki.shared.core.domain import IS_RETRIEVAL_INTENT_KEY
...
class TrainingDataImporter(ABC):
    ...
    def load_from_config(
        ...
        config = bf_nlu_banki.shared.utils.io.read_config_file(config_path)
    ...
    def load_from_dict(
        ...
        from bf_nlu_banki.shared.importers.bf_nlu_banki import bf_nlu_bankiFileImporter
        ...
        if not importers:
            importers = [
                bf_nlu_bankiFileImporter(config_path, domain_path, training_data_paths)
            ]
    ...
    def _importer_from_dict(
        ...
        from bf_nlu_banki.shared.importers.multi_project import MultiProjectImporter
        from bf_nlu_banki.shared.importers.bf_nlu_banki import bf_nlu_bankiFileImporter

        module_path = importer_config.pop("name", None)
        if module_path == bf_nlu_bankiFileImporter.__name__:
            importer_class: Type[TrainingDataImporter] = bf_nlu_bankiFileImporter
        elif module_path == MultiProjectImporter.__name__:
            importer_class = MultiProjectImporter
        else:
            try:
                importer_class = bf_nlu_banki.shared.utils.common.class_from_module_path(
                    module_path
                )
        ...
        constructor_arguments = bf_nlu_banki.shared.utils.common.minimal_kwargs(
    ...
    def fingerprint(self) -> Text:
        ...
        return bf_nlu_banki.shared.utils.io.random_string(25)
...
class NluDataImporter(TrainingDataImporter):
    ...
    @bf_nlu_banki.shared.utils.common.cached_method
    def get_config_file_for_auto_config(self) -> Optional[Text]:
        """Returns config file path for auto-config only if there is a single one."""
...
class ResponsesSyncImporter(TrainingDataImporter):
    ...
    def _construct_retrieval_action_names(retrieval_intents: Set[Text]) -> List[Text]:
        ...
        """
        return [
            f"{bf_nlu_banki.shared.constants.UTTER_PREFIX}{intent}"
...
def _additional_training_data_from_default_actions() -> TrainingData:
    additional_messages_from_default_actions = [
        ...
        for action_name in bf_nlu_banki.shared.core.constants.DEFAULT_ACTION_NAMES
    ]
