...
import bf_nlu_banki.shared.core.constants
from bf_nlu_banki.shared.core.constants import SlotMappingType, MAPPING_TYPE, MAPPING_CONDITIONS
from bf_nlu_banki.shared.exceptions import (
    bf_nlu_bankiException,
    YamlException,
    YamlSyntaxException,
)
import bf_nlu_banki.shared.utils.validation
import bf_nlu_banki.shared.utils.io
import bf_nlu_banki.shared.utils.common
import bf_nlu_banki.shared.core.slot_mappings
from bf_nlu_banki.shared.core.events import SlotSet, UserUttered
from bf_nlu_banki.shared.core.slots import Slot, CategoricalSlot, TextSlot, AnySlot, ListSlot
from bf_nlu_banki.shared.utils.validation import KEY_TRAINING_DATA_FORMAT_VERSION
...
if TYPE_CHECKING:
    from bf_nlu_banki.shared.core.trackers import DialogueStateTracker
...
class InvalidDomain(bf_nlu_bankiException):
    """Exception that can be raised when domain is not valid."""


class ActionNotFoundException(ValueError, bf_nlu_bankiException):
...
class Domain:
    ...
    def from_file(cls, path: Text) -> "Domain":
        ...
        return cls.from_yaml(bf_nlu_banki.shared.utils.io.read_file(path), path)

    @classmethod
    def from_yaml(cls, yaml: Text, original_filename: Text = "") -> "Domain":
        """Loads the `Domain` from YAML text after validating it."""
        try:
            bf_nlu_banki.shared.utils.validation.validate_yaml_schema(yaml, DOMAIN_SCHEMA_FILE)

            data = bf_nlu_banki.shared.utils.io.read_yaml(yaml)
            if not bf_nlu_banki.shared.utils.validation.validate_training_data_format_version(
    ...
    def from_dict(cls, data: Dict) -> "Domain":
        ...
        if domain_slots:
            bf_nlu_banki.shared.core.slot_mappings.validate_slot_mappings(domain_slots)
        ...
        for root, _, files in os.walk(path, followlinks=True):
            for file in files:
                ...
                if Domain.is_domain_file(full_path):
                    ...
                    other_dict = bf_nlu_banki.shared.utils.io.read_yaml(
                        bf_nlu_banki.shared.utils.io.read_file(full_path)
                    )
    ...
    def merge_domain_dicts(
        ...
        merge_func_mappings: Dict[Text, Callable[..., Any]] = {
            KEY_INTENTS: bf_nlu_banki.shared.utils.common.merge_lists_of_dicts,
            KEY_ENTITIES: bf_nlu_banki.shared.utils.common.merge_lists_of_dicts,
            KEY_ACTIONS: bf_nlu_banki.shared.utils.common.merge_lists,
            KEY_E2E_ACTIONS: bf_nlu_banki.shared.utils.common.merge_lists,
            KEY_FORMS: bf_nlu_banki.shared.utils.common.merge_dicts,
            KEY_RESPONSES: bf_nlu_banki.shared.utils.common.merge_dicts,
            KEY_SLOTS: bf_nlu_banki.shared.utils.common.merge_dicts,
        }

        for key, merge_func in merge_func_mappings.items():
            duplicates[key] = bf_nlu_banki.shared.utils.common.extract_duplicates(
                combined.get(key, []), domain_dict.get(key, [])
            )

            default: Union[List[Any], Dict[Text, Any]] = (
                {} if merge_func == bf_nlu_banki.shared.utils.common.merge_dicts else []
            )
        ...
        if duplicates:
            duplicates = bf_nlu_banki.shared.utils.common.clean_duplicates(duplicates)
    ...
    def collect_slots(slot_dict: Dict[Text, Any]) -> List[Slot]:
        ...
        # Don't sort the slots, see https://github.com/bf_nlu_bankiHQ/bf_nlu_banki-x/issues/3900
    ...
    def _transform_intent_properties_for_internal_use(
        ...
        if properties:
            ...
            raise InvalidDomain(
                ...
                f" {bf_nlu_banki.shared.constants.DOCS_URL_DOMAINS} for more information on how"
                f" to correctly add `intents` in the `domain` and"
                f" {bf_nlu_banki.shared.constants.DOCS_URL_INTENTS} for examples on"
    ...
    @bf_nlu_banki.shared.utils.common.lazy_property
                ...
                for _entity, sub_labels in entity.items():
                    ...
                    if sub_labels:
                        ...
                        raise InvalidDomain(
                            ...
                            f" {bf_nlu_banki.shared.constants.DOCS_URL_DOMAINS} for more"
                            f" information on how to correctly add `entities` in the"
                            f" `domain` and {bf_nlu_banki.shared.constants.DOCS_URL_ENTITIES}"
    ...
    def _add_default_intents(
        ...
        for intent_name in bf_nlu_banki.shared.core.constants.DEFAULT_INTENTS:
    ...
    def __init__(
        ...
        Args:
            ...
            store_entities_as_slots: If `True` bf_nlu_banki will automatically create `SlotSet`
    ...
    def _collect_overridden_default_intents(
        ...
        return sorted(
            intent_names.intersection(set(bf_nlu_banki.shared.core.constants.DEFAULT_INTENTS))
        )
    ...
    def _initialize_forms(
        ...
        Returns:
            ...
            actions.
            Returning custom actions for each forms means that bf_nlu_banki Open Source should
            not use the default `FormAction` for the forms, but rather a custom action
            for it. This can e.g. be used to run the deprecated bf_nlu_banki Open Source 1
            `FormAction` which is implemented in the bf_nlu_banki SDK.
        """
    ...
    def fingerprint(self) -> Text:
        ...
        return bf_nlu_banki.shared.utils.io.get_dictionary_fingerprint(self_as_dict)
    ...
    def _add_requested_slot(self) -> None:
        ...
        """
        if self.form_names and bf_nlu_banki.shared.core.constants.REQUESTED_SLOT not in [
            slot.name for slot in self.slots
        ]:
            self.slots.append(
                TextSlot(
                    bf_nlu_banki.shared.core.constants.REQUESTED_SLOT,
                    mappings=[],
    ...
    def _add_knowledge_base_slots(self) -> None:
        ...
        """
        if (
            bf_nlu_banki.shared.core.constants.DEFAULT_KNOWLEDGE_BASE_ACTION
            ...
            knowledge_base_slots = [
                bf_nlu_banki.shared.core.constants.SLOT_LISTED_ITEMS,
                bf_nlu_banki.shared.core.constants.SLOT_LAST_OBJECT,
                bf_nlu_banki.shared.core.constants.SLOT_LAST_OBJECT_TYPE,
            ]
    ...
    def _add_session_metadata_slot(self) -> None:
        self.slots.append(
            AnySlot(bf_nlu_banki.shared.core.constants.SESSION_START_METADATA_SLOT, mappings=[])
        )
    ...
    def concatenate_entity_labels(
        ...
        if entity:
            return [
                f"{entity}"
                f"{bf_nlu_banki.shared.core.constants.ENTITY_LABEL_SEPARATOR}"
                f"{sub_label}"
    ...
    def _get_featurized_entities(self, latest_message: UserUttered) -> Set[Text]:
        ...
        entity_names_roles = set(
            f"{entity['entity']}"
            f"{bf_nlu_banki.shared.core.constants.ENTITY_LABEL_SEPARATOR}{entity['role']}"
            for entity in entities
            if "entity" in entity.keys() and "role" in entity.keys()
        )
        entity_names_groups = set(
            f"{entity['entity']}"
            f"{bf_nlu_banki.shared.core.constants.ENTITY_LABEL_SEPARATOR}{entity['group']}"
            for entity in entities
        ...
        for slot_name, slot in tracker.slots.items():
            ...
            if slot is not None and slot.as_feature():
                if omit_unset_slots and not slot.has_been_set:
                    continue
                if slot.value == bf_nlu_banki.shared.core.constants.SHOULD_NOT_BE_SET:
                    slots[slot_name] = bf_nlu_banki.shared.core.constants.SHOULD_NOT_BE_SET
    ...
    def _get_active_loop_sub_state(
        ...
        if tracker.active_loop:
            return {bf_nlu_banki.shared.core.constants.LOOP_NAME: tracker.active_loop.name}
        else:
            return {}
    ...
    def get_active_state(
        ...
        """
        state = {
            bf_nlu_banki.shared.core.constants.USER: self._get_user_sub_state(tracker),
            bf_nlu_banki.shared.core.constants.SLOTS: self._get_slots_sub_state(
                tracker, omit_unset_slots=omit_unset_slots
            ),
            bf_nlu_banki.shared.core.constants.PREVIOUS_ACTION: self._get_prev_action_sub_state(
                tracker
            ),
            bf_nlu_banki.shared.core.constants.ACTIVE_LOOP: self._get_active_loop_sub_state(
                tracker
            ),
        }
    ...
    def _remove_rule_only_features(
        ...
        rule_only_slots = rule_only_data.get(
            bf_nlu_banki.shared.core.constants.RULE_ONLY_SLOTS, []
        )
        rule_only_loops = rule_only_data.get(
            bf_nlu_banki.shared.core.constants.RULE_ONLY_LOOPS, []
        )

        # remove slots which only occur in rules but not in stories
        if rule_only_slots:
            for slot in rule_only_slots:
                state.get(bf_nlu_banki.shared.core.constants.SLOTS, {}).pop(slot, None)
        # remove active loop which only occur in rules but not in stories
        if (
            rule_only_loops
            and state.get(bf_nlu_banki.shared.core.constants.ACTIVE_LOOP, {}).get(
                bf_nlu_banki.shared.core.constants.LOOP_NAME
            )
            in rule_only_loops
        ):
            del state[bf_nlu_banki.shared.core.constants.ACTIVE_LOOP]

    @staticmethod
    def _substitute_rule_only_user_input(state: State, last_ml_state: State) -> None:
        if not bf_nlu_banki.shared.core.trackers.is_prev_action_listen_in_state(state):
            if not last_ml_state.get(bf_nlu_banki.shared.core.constants.USER) and state.get(
                bf_nlu_banki.shared.core.constants.USER
            ):
                del state[bf_nlu_banki.shared.core.constants.USER]
            elif last_ml_state.get(bf_nlu_banki.shared.core.constants.USER):
                state[bf_nlu_banki.shared.core.constants.USER] = last_ml_state[
                    bf_nlu_banki.shared.core.constants.USER
                ]
        ...
        for tr, hide_rule_turn in tracker.generate_all_prior_trackers():
            ...
            if ignore_rule_only_turns:
                ...
                if last_ml_action_sub_state:
                    ...
                    state[bf_nlu_banki.shared.core.constants.PREVIOUS_ACTION] = cast(
                        SubState,
    ...
    def persist_specification(self, model_path: Text) -> None:
        ...
        bf_nlu_banki.shared.utils.io.create_directory_for_file(domain_spec_path)

        metadata = {"states": self.input_states}
        bf_nlu_banki.shared.utils.io.dump_obj_as_json_to_file(domain_spec_path, metadata)
    ...
    def load_specification(cls, path: Text) -> Dict[Text, Any]:
        ...
        return json.loads(bf_nlu_banki.shared.utils.io.read_file(metadata_path))
    ...
    def persist(self, filename: Union[Text, Path]) -> None:
        ...
        bf_nlu_banki.shared.utils.io.write_text_file(as_yaml, filename)
    ...
    def as_yaml(self) -> Text:
        ...
        return bf_nlu_banki.shared.utils.io.dump_obj_as_yaml_to_string(
    ...
    def _actions_for_domain_warnings(self) -> List[Text]:
        ...
        return [
            ...
            if action not in bf_nlu_banki.shared.core.constants.DEFAULT_ACTION_NAMES
        ]
    ...
    def _combine_user_with_default_actions(user_actions: List[Text]) -> List[Text]:
        ...
        return bf_nlu_banki.shared.core.constants.DEFAULT_ACTION_NAMES + unique_user_actions
    ...
    def utterances_for_response(self) -> Set[Text]:
        ...
        for action in self.action_names_or_texts:
            if not action.startswith(bf_nlu_banki.shared.constants.UTTER_PREFIX):
                continue
    ...
    def is_domain_file(filename: Union[Text, Path]) -> bool:
        """Checks whether the given file path is a bf_nlu_banki domain file.
        ...
        """
        from bf_nlu_banki.shared.data import is_likely_yaml_file

        if not is_likely_yaml_file(filename):
            return False

        try:
            content = bf_nlu_banki.shared.utils.io.read_yaml_file(filename)
        except (bf_nlu_bankiException, YamlSyntaxException):
            bf_nlu_banki.shared.utils.io.raise_warning(
...
def warn_about_duplicates_found_during_domain_merging(
    ...
    bf_nlu_banki.shared.utils.io.raise_warning(message, docs=DOCS_URL_DOMAINS)
    return None
