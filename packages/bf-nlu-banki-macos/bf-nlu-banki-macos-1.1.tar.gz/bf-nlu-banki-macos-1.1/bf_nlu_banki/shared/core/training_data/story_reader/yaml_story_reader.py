...
from bf_nlu_banki.shared.core.slots import TextSlot, ListSlot
from bf_nlu_banki.shared.exceptions import YamlException
import bf_nlu_banki.shared.utils.io
from bf_nlu_banki.shared.core.constants import LOOP_NAME
...
from bf_nlu_banki.shared.core.constants import RULE_SNIPPET_ACTION_NAME
from bf_nlu_banki.shared.core.events import UserUttered, SlotSet, ActiveLoop
from bf_nlu_banki.shared.core.training_data.story_reader.story_reader import StoryReader
...
class YAMLStoryReader(StoryReader):
    ...
    def read_from_file(
        ...
        try:
            return self.read_from_string(
                bf_nlu_banki.shared.utils.io.read_file(
                    filename, bf_nlu_banki.shared.utils.io.DEFAULT_ENCODING
                ),
                skip_validation,
            )
    ...
    def read_from_string(
        ...
        if not skip_validation:
            bf_nlu_banki.shared.utils.validation.validate_yaml_schema(string, CORE_SCHEMA_FILE)
    ...
    def is_stories_file(cls, file_path: Union[Text, Path]) -> bool:
        ...
        """
        return bf_nlu_banki.shared.data.is_likely_yaml_file(
            file_path
        ) and bf_nlu_banki.shared.utils.io.is_key_in_yaml(file_path, KEY_STORIES, KEY_RULES)
    ...
    def _user_intent_from_step(
        ...
        if response_key and not self.is_test_stories_file(self.source_name):
            bf_nlu_banki.shared.utils.io.raise_warning(
                ...
                f"bf_nlu_banki Open Source will only use base intent '{base_intent}' "
                f"for training.",
                docs=self._get_docs_link(),
            )
