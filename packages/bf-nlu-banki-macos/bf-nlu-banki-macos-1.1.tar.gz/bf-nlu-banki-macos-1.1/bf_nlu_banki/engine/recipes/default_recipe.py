...
from bf_nlu_banki.core.featurizers.precomputation import (
    CoreFeaturizationInputConverter,
    CoreFeaturizationCollector,
)
from bf_nlu_banki.shared.exceptions import FileNotFoundException
from bf_nlu_banki.core.policies.ensemble import DefaultPolicyPredictionEnsemble

from bf_nlu_banki.engine.graph import (
    GraphSchema,
    GraphComponent,
    SchemaNode,
    GraphModelConfiguration,
)
from bf_nlu_banki.engine.constants import (
    PLACEHOLDER_IMPORTER,
    PLACEHOLDER_MESSAGE,
    PLACEHOLDER_TRACKER,
)
from bf_nlu_banki.engine.recipes.recipe import Recipe
from bf_nlu_banki.engine.storage.resource import Resource
from bf_nlu_banki.graph_components.converters.nlu_message_converter import NLUMessageConverter
from bf_nlu_banki.graph_components.providers.domain_provider import DomainProvider
from bf_nlu_banki.graph_components.providers.domain_for_core_training_provider import (
    DomainForCoreTrainingProvider,
)
from bf_nlu_banki.graph_components.providers.nlu_training_data_provider import (
    NLUTrainingDataProvider,
)
from bf_nlu_banki.graph_components.providers.rule_only_provider import RuleOnlyDataProvider
from bf_nlu_banki.graph_components.providers.story_graph_provider import StoryGraphProvider
from bf_nlu_banki.graph_components.providers.training_tracker_provider import (
    TrainingTrackerProvider,
)
import bf_nlu_banki.shared.constants
from bf_nlu_banki.shared.exceptions import bf_nlu_bankiException, InvalidConfigException
from bf_nlu_banki.shared.data import TrainingType

from bf_nlu_banki.utils.tensorflow.constants import EPOCHS
from bf_nlu_banki.shared.utils.common import (
...
COMMENTS_FOR_KEYS = {
    "pipeline": (
        ...
        f"# # See {bf_nlu_banki.shared.constants.DOCS_URL_PIPELINE} for more information.\n"
    ),
    "policies": (
        ...
        f"# # See {bf_nlu_banki.shared.constants.DOCS_URL_POLICIES} for more information.\n"
    ),
}


class DefaultV1RecipeRegisterException(bf_nlu_bankiException):
...
class DefaultV1Recipe(Recipe):
    ...
    def _from_registry(cls, name: Text) -> RegisteredComponent:
        # Importing all the default bf_nlu_banki components will automatically register them
        from bf_nlu_banki.engine.recipes.default_components import DEFAULT_COMPONENTS  # noqa
    ...
    def graph_config_for_recipe(
        ...
        from bf_nlu_banki.nlu.classifiers.regex_message_handler import RegexMessageHandler
    ...
    def _create_train_nodes(
        ...
        from bf_nlu_banki.graph_components.validators.default_recipe_validator import (
            DefaultV1RecipeValidator,
        )
        from bf_nlu_banki.graph_components.validators.finetuning_validator import (
            FinetuningValidator,
        )
    ...
    def _extra_config_from_cli(
        ...
        from bf_nlu_banki.nlu.classifiers.mitie_intent_classifier import MitieIntentClassifier
        from bf_nlu_banki.nlu.extractors.mitie_entity_extractor import MitieEntityExtractor
        from bf_nlu_banki.nlu.classifiers.sklearn_intent_classifier import (
    ...
    def _add_core_predict_nodes(
        ...
        for idx, config in enumerate(predict_config["policies"]):
            ...
            from bf_nlu_banki.core.policies.rule_policy import RulePolicy
    ...
    def auto_configure(
        ...
        allowed to access code outside of `bf_nlu_banki.shared`.
    ...
    def _get_unspecified_autoconfigurable_keys(
        ...
        if training_type == TrainingType.NLU:
            all_keys = bf_nlu_banki.shared.constants.CONFIG_AUTOCONFIGURABLE_KEYS_NLU
        elif training_type == TrainingType.CORE:
            all_keys = bf_nlu_banki.shared.constants.CONFIG_AUTOCONFIGURABLE_KEYS_CORE
        else:
            all_keys = bf_nlu_banki.shared.constants.CONFIG_AUTOCONFIGURABLE_KEYS
    ...
    def _get_missing_config_keys(
        ...
        if training_type == TrainingType.NLU:
            all_keys = bf_nlu_banki.shared.constants.CONFIG_KEYS_NLU
        elif training_type == TrainingType.CORE:
            all_keys = bf_nlu_banki.shared.constants.CONFIG_KEYS_CORE
        else:
            all_keys = bf_nlu_banki.shared.constants.CONFIG_KEYS
    ...
    def complete_config(
        ...
        default_config = bf_nlu_banki.shared.utils.io.read_config_file(default_config_file)
    ...
    def _dump_config(
        ...
        current_config_content = bf_nlu_banki.shared.utils.io.read_file(config_file_path)
        ...
        bf_nlu_banki.shared.utils.io.write_text_file("".join(updated_lines), config_file_path)

        auto_configured_keys_text = transform_collection_to_sentence(
            auto_configured_keys
        )
        bf_nlu_banki.shared.utils.cli.print_info(
    ...
    def _is_config_file_as_expected(
        ...
        try:
            content = bf_nlu_banki.shared.utils.io.read_config_file(config_file_path)
    ...
    def _add_missing_config_keys_to_file(
        ...
        if not missing_keys:
            return
        with open(
            config_file_path, "a", encoding=bf_nlu_banki.shared.utils.io.DEFAULT_ENCODING
        ) as f:
