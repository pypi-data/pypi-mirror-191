...
from bf_nlu_banki.cli.arguments.default_arguments import (
    add_config_param,
    add_stories_param,
    add_nlu_data_param,
    add_out_param,
...
from bf_nlu_banki.shared.constants import DEFAULT_CONFIG_PATH, DEFAULT_DATA_PATH

USE_LATEST_MODEL_FOR_FINE_TUNING = True


def set_train_arguments(parser: argparse.ArgumentParser) -> None:
    """Specifies CLI arguments for `bf_nlu_banki train`."""
...
def set_train_core_arguments(parser: argparse.ArgumentParser) -> None:
    """Specifies CLI arguments for `bf_nlu_banki train core`."""
...
def set_train_nlu_arguments(parser: argparse.ArgumentParser) -> None:
    """Specifies CLI arguments for `bf_nlu_banki train nlu`."""
    add_config_param(parser)
...
def _add_core_config_param(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        ...
        "If multiple configuration files are provided, multiple bf_nlu_banki Core "
...
def add_dry_run_param(
    ...
    parser.add_argument(
        ...
        "still might require updating by running 'bf_nlu_banki train').\n"
...
def add_finetune_params(
    ...
    parser.add_argument(
        ...
        help="Fine-tune a previously trained model. If no model path is provided, bf_nlu_banki "
