...
import bf_nlu_banki.cli.arguments.train as train_arguments

import bf_nlu_banki.cli.utils
import bf_nlu_banki.utils.common
from bf_nlu_banki.core.train import do_compare_training
...
def add_subparser(
    ...
    train_parser = subparsers.add_parser(
        "train",
        help="Trains a bf_nlu_banki model using your NLU data and stories.",
        parents=parents,
    ...
    train_core_parser = train_subparsers.add_parser(
        ...
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
        help="Trains a bf_nlu_banki Core model using your stories.",
    )
    ...
    train_nlu_parser = train_subparsers.add_parser(
        ...
        parents=parents,
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
        help="Trains a bf_nlu_banki NLU model using your NLU data.",
    )
...
def run_training(args: argparse.Namespace, can_exit: bool = False) -> Optional[Text]:
    ...
    """
    from bf_nlu_banki import train as train_all

    domain = bf_nlu_banki.cli.utils.get_validated_path(
...
def run_core_training(args: argparse.Namespace) -> Optional[Text]:
    """Trains a bf_nlu_banki Core model only.
    ...
    Returns:
        Path to a trained model or `None` if training was not successful.
    """
    from bf_nlu_banki.model_training import train_core

    args.domain = bf_nlu_banki.cli.utils.get_validated_path(
        args.domain, "domain", DEFAULT_DOMAIN_PATH, none_is_valid=True
    )
    story_file = bf_nlu_banki.cli.utils.get_validated_path(
...
def run_nlu_training(args: argparse.Namespace) -> Optional[Text]:
    ...
    Returns:
        Path to a trained model or `None` if training was not successful.
    """
    from bf_nlu_banki.model_training import train_nlu

    config = _get_valid_config(args.config, CONFIG_MANDATORY_KEYS_NLU)
    nlu_data = bf_nlu_banki.cli.utils.get_validated_path(
...
def _get_valid_config(
    ...
    """
    config = bf_nlu_banki.cli.utils.get_validated_path(config, "config", default_config)
    ...
    missing_keys = bf_nlu_banki.cli.utils.missing_config_keys(config, mandatory_keys)
    if missing_keys:
        print_error(
