...
import bf_nlu_banki.utils.train_utils
from bf_nlu_banki.utils.tensorflow import layers
from bf_nlu_banki.utils.tensorflow import bf_nlu_banki_layers
from bf_nlu_banki.utils.tensorflow.temp_keras_modules import TmpKerasModel
from bf_nlu_banki.utils.tensorflow.data_generator import (
    bf_nlu_bankiDataGenerator,
    bf_nlu_bankiBatchDataGenerator,
)
from bf_nlu_banki.shared.nlu.constants import TEXT
...
class bf_nlu_bankiModel(TmpKerasModel):
    ...
    def __init__(self, random_seed: Optional[int] = None, **kwargs: Any) -> None:
        """Initialize the bf_nlu_bankiModel.
    ...
    def run_inference(
        ...
        (data_generator, _) = bf_nlu_banki.utils.train_utils.create_data_generators(
        ...
        while True:
            try:
                ...
                ] = self._bf_nlu_banki_predict(batch_in)
    ...
    def _merge_batch_outputs(
        ...
        for key, val in batch_output.items():
            if isinstance(val, np.ndarray):
                ...
                all_outputs[key] = bf_nlu_bankiModel._merge_batch_outputs(all_outputs[key], val)
    ...
    def load(
        ...
        predict_data_example: Optional[bf_nlu_bankiModelData] = None,
        ...
        **kwargs: Any,
        ...
        data_generator = bf_nlu_bankiBatchDataGenerator(model_data_example, batch_size=1)
        ...
        for key, values in data_signature.items():
            for sub_key, signature in values.items():
                for is_sparse, feature_dimension, number_of_dimensions in signature:
                    ...
                    if is_sparse:
                        tensor, idx = bf_nlu_bankiModel._convert_sparse_features(
                            batch, feature_dimension, idx, number_of_dimensions
                        )
                    else:
                        tensor, idx = bf_nlu_bankiModel._convert_dense_features(
                            batch, feature_dimension, idx, number_of_dimensions
                        )
...
# noinspection PyMethodOverriding
class Transformerbf_nlu_bankiModel(bf_nlu_bankiModel):
    def __init__(
        self,
        name: Text,
        ...
        label_batch = bf_nlu_bankiDataGenerator.prepare_batch(label_data.data)
    ...
    def _check_if_sparse_feature_sizes_decreased(
        ...
        Raises:
            bf_nlu_bankiException: When any of the sparse feature sizes decrease
    ...
    def _update_dense_for_sparse_layers(
        ...
        for name, layer in self._tf_layers.items():
            # `if` condition is necessary because only `bf_nlu_bankiCustomLayer`
            # can adjust sparse layers for incremental training by default.
            if isinstance(layer, bf_nlu_banki_layers.bf_nlu_bankiCustomLayer):
    ...
    def _compile_and_fit(
        ...
        data_generator = bf_nlu_bankiBatchDataGenerator(model_data, batch_size=1)
        self.fit(data_generator, verbose=False)

    def _update_data_signatures(self, model_data: bf_nlu_bankiModelData) -> None:
    ...
    def _get_mask_for(
        ...
        return bf_nlu_banki_layers.compute_mask(sequence_lengths)
