...
from bf_nlu_banki.constants import RESULTS_FILE
...
def _extract_paired_histogram_specification(
    ...
    if not histogram_data or not np.concatenate(histogram_data).size:
        bf_nlu_banki.shared.utils.io.raise_warning("No data to plot paired histogram.")
...
def plot_curve(
    ...
    data = bf_nlu_banki.shared.utils.io.read_json_file(
