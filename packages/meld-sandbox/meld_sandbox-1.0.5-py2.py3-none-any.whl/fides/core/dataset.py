...
def create_db_dataset(schema_name: str, db_tables: Dict[str, List[str]]) -> Dataset:
    ...
    dataset = Dataset(
        ...
        name=schema_name,
        description=f"M3LD Sandbox Generated Description for Schema: {schema_name}",
        data_categories=[],
        collections=[
            DatasetCollection(
                name=table_name,
                description=f"M3LD Sandbox Generated Description for Table: {table_name}",
                data_categories=[],
                fields=[
                    DatasetField(
                        name=column,
                        description=f"M3LD Sandbox Generated Description for Column: {column}",
                        data_categories=[],
                    )
                    for column in table
                ],
