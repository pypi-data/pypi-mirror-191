...
class OptionChainTicker:
    ...
    def on_ticks(self, ws, ticks):
        ...
        if ticks[0]['mode'] != ws.MODE_FULL:
            ...
            ws.stop()
            return
        threading.Thread(target=self.handle_ticks, args=(ticks,)).start()
    ...
    def handle_ticks(self, ticks):
        ...
        # todo make it debug later
        log.info(f"size: {len(ticks)} thread took {(time.time() - start_time)} seconds")
...
class OptionStream:
    ...
    def refresh_instruments(self):
        ...
        instrument_parser = KiteInstrumentManager(self.secrets['api_key'], self.secrets['access_token'], self.db)
