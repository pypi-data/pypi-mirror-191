    ...
    try:
        ...
        for item in all_layers:
            ...
            for x in range(0,linesum):
                Selection.clear_select(job,step,'line_back')
                Selection.select_feature_by_id(job,step,'line_back',[0])
                info=Information.get_selected_features_infos(job,step,'line_back')
                xs=info[0]['XS']
                xe=info[0]['XE']
                ys=info[0]['YS']
                ye=info[0]['YE']
                ...
                if size<0 and abs(size)>=orglen:
                    Layers.hierarchy_edit(job,step,['line_back'],1)
                    check=False
                if check==True:
                    ...
                    Layers.delete_feature(job,step,['line_back'])
                    Layers.add_line(job,step,['line_back'],symbol,xsnew,ysnew,xenew,yenew,pol,att)
            Selection.clear_select(job,step,'line_back')
            Layers.move2other_layer(job,step,['line_back'],job,step,item,False,0,0,0,0,0,0,0)
            ...
            for y in range(0,ovalsum):
                Selection.clear_select(job,step,'slotpad_back')
                Selection.select_feature_by_id(job,step,'slotpad_back',[0])
                info=Information.get_selected_features_infos(job,step,'slotpad_back')
                ...
                if angle==0 or angle==180 or angle==360:
                    aa=True
                if xsize<ysize and aa==True:
                    vertical_angle=True                  
                elif xsize>ysize and aa==False:
                    vertical_angle=True
                if xsize<ysize:
                    orglen=ysize*25400
                else:
                ...
                if check==True:
                    if mode==0 and datum==0:
                        location=BASE.change_datum_0_0(locationx,locationy)
                        info=BASE.oval_mode_0_0(xlen,ylen,sepcial_angle,xsize,ysize)
                    if mode==0 and datum==1:
                        location=BASE.change_datum_0_1(xsize,ysize,size,sin,cos,locationx,locationy,sepcial_angle,vertical_angle,angle,mirror)
                        info=BASE.oval_mode_0_0(xlen,ylen,sepcial_angle,xsize,ysize)
                    if mode==0 and datum==2:
                        location=BASE.change_datum_0_2(xsize,ysize,size,sin,cos,locationx,locationy,sepcial_angle,vertical_angle,angle,mirror)
                        info=BASE.oval_mode_0_0(xlen,ylen,sepcial_angle,xsize,ysize)
                    if mode==0 and datum==3:
                        location=BASE.change_datum_0_3(xsize,ysize,size,sin,cos,locationx,locationy,angle,mirror,datum)
                        info=BASE.oval_mode_0_0(xlen,ylen,sepcial_angle,xsize,ysize)
                    if mode==0 and datum==4:
                        location=BASE.change_datum_0_3(xsize,ysize,size,sin,cos,locationx,locationy,angle,mirror,datum)
                        info=BASE.oval_mode_0_0(xlen,ylen,sepcial_angle,xsize,ysize)
                    ...
                    if mode==1 and datum==1:
                        location=BASE.change_datum_1_1(xsize,ysize,size,sin,cos,locationx,locationy,sepcial_angle,vertical_angle,angle,mirror,datum)
                        info=BASE.oval_mode_1_0(xsize,ysize,size)
                    if mode==1 and datum==2:
                        location=BASE.change_datum_1_1(xsize,ysize,size,sin,cos,locationx,locationy,sepcial_angle,vertical_angle,angle,mirror,datum)
                        info=BASE.oval_mode_1_0(xsize,ysize,size)
                    if mode==1 and datum==3:
                        location=BASE.change_datum_1_3(xsize,ysize,size,sin,cos,locationx,locationy,angle,mirror,datum)
                        info=BASE.oval_mode_1_0(xsize,ysize,size)
                    if mode==1 and datum==4:
                        location=BASE.change_datum_1_3(xsize,ysize,size,sin,cos,locationx,locationy,angle,mirror,datum)
                        info=BASE.oval_mode_1_0(xsize,ysize,size)
                    ...
                    xnew=info['xnew']
                    ynew=info['ynew']
                    Layers.delete_feature(job,step,['slotpad_back'])
                    symbolname='oval'+str(xnew)+'x'+str(ynew)
                    Layers.add_pad(job,step,['slotpad_back'],symbolname,locationxnew,locationynew,pol,orient,att,angle)
            Selection.clear_select(job,step,'slotpad_back')
            Layers.move2other_layer(job,step,['slotpad_back'],job,step,item,False,0,0,0,0,0,0,0)
        Matrix.delete_layer(job,'line_back')
        Matrix.delete_layer(job,'slotpad_back')
    except Exception as e:
            print(e)
    return None
