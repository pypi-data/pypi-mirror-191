...
class BlockchainInfoClient(BaseClient):
    ...
    def getutxos(self, address, after_txid="", limit=MAX_TRANSACTIONS):
        ...
        if len(res["unspent_outputs"]) > 299:
            _logger.info(
                "BlockchainInfoClient: Large number of outputs for address %s, "
    ...
    def gettransaction(self, txid, latest_block=None):
        ...
        for n, i in enumerate(t.inputs):
            if "prev_out" in tx["inputs"][n]:
                i.value = (
                    0
                    if not tx["inputs"][n]["prev_out"]
                    else tx["inputs"][n]["prev_out"]["value"]
                )
                input_total += i.value
    ...
    def gettransactions(self, address, after_txid="", limit=MAX_TRANSACTIONS):
        ...
        for txid in txids[:limit]:
            ...
            t.confirmations = (
                0 if not t.block_height else self.latest_block - t.block_height
            )
            txs.append(t)
        return txs
    ...
    def mempool(self, txid=""):
        if txid:
            ...
            txs = self.compose_request(
                "unconfirmed-transactions", variables={"format": "json"}
            )
    ...
    def getblock(self, blockid, parse_transactions, page, limit):
        ...
        if parse_transactions:
            ...
            for tx in bd["tx"][(page - 1) * limit : page * limit]:
                # try:
                txs.append(
                    self.gettransaction(tx["hash"], latest_block=self.latest_block)
                )
    ...
    def getinfo(self):
        ...
        info = json.loads(
            requests.get("https://api.blockchain.info/stats", timeout=self.timeout).text
        )
