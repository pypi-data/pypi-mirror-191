...
class TdxAPIPool:
    ...
    def get_security_quotes(self, tdxsymbols):
        ...
        for i in range(0, len(tdxsymbols), 50):
            ...
            r.add_done_callback(lambda x: tdxticks.extend(x.result() or []))
            rets.append(r)
        list(as_completed(rets))
        return tdxticks


class vxHqAPI(DataApiBase):
    def __init__(self, host_file="etc/tdxhosts.json"):
        self._apipool = TdxAPIPool(pool_size=5, host_file=host_file)
    ...
    def __call__(self, *symbols):
        ...
        tdx_ticks = self._apipool.get_security_quotes(
            [to_tdx_symbol(symbol) for symbol in symbols]
        )
        return dict(map(parser_tdx_tick, tdx_ticks))


if __name__ == "__main__":
    from vxdataset.mdapi.base import vxMdApi
    ...
    mdapi.register(
        hq={
            "class": "vxdataset.collector.hq.tdx.vxHqAPI",
            "params": {"host_file": "etc/tdxhosts.json"},
        }
    )
