...
class Device(object):
    ...
    def add_callback(self, key: str, index: int, fnc: Callable[[Any], Any]) -> None:
        ...
        if self.__key_index_to_byte is not None:
            ...
            _LOGGER.warning("Unimplemented key_index_to_byte table for device %s", self.__inels_type)
    ...
    def callback(self, new_value: Any) -> None:
        ...
        l = self.diff(last_val=self.last_values.inels_status_value, curr_val=self.values.inels_status_value)

        _LOGGER.info("Calling update_value")
        self.update_value(new_value) #calculate status value

        _LOGGER.info("Calling associated %d callbacks", len(l))
        for i in l:
            for c in list(self.__callback_map[i]):
                c(new_value)
    ...
    def diff(self, last_val: str, curr_val: str) -> "list[int]": 
        ...
        _LOGGER.info("last_val.len %d curr_val.len %d", len(last_val), len(curr_val))
        l = []
        for i in range(min(len(last_val), len(curr_val))):
            _LOGGER.info("%d - last %s vs curr %s", i , last_val[i], curr_val[i])
