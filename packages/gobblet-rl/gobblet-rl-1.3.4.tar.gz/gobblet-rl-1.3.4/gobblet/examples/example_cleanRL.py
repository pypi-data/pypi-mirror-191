...
if __name__ == "__main__":
    ...
    """ ENV SETUP """
    env = gobblet_v1.env(render_mode="human")
    ...
    for episode in range(total_episodes):
        ...
        with torch.no_grad():
            ...
            for step in range(0, max_cycles):
                ...
                action, logprobs, _, values = agent.get_action_and_value(obs)
                ...
                rb_obs[step] = obs
                rb_rewards[step] = reward
                rb_terms[step] = termination
        ...
        for repeat in range(3):
            ...
            for start in range(0, len(b_obs)):
                # select the indices we want to train on
                end = len(obs)
                batch_index = b_index[start:end]

                _, newlogprob, entropy, value = agent.get_action_and_value(
                    b_obs[batch_index], b_actions.long()
                )
