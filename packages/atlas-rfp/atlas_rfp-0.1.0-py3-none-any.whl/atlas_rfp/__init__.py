...
def submit_rfp(session: Session, *, rfp: RFPCreationData, recipient: Optional[Person]=None) -> RFP:
    ...
    print(f'Payee vendor ID: {payee_vendor_id}')
    ...
    print(f'Request RFP token: {rfp_token}')
    ...
    for i, attachment in enumerate(rfp.attachments):
        ...
        print(attach_response.request.headers)
        print(attach_response.request.body)
    ...
    if recipient:
        ...
        print(f'Send-to CSRF token: {send_to_token}')
        ...
        print(f'Forwarding CSRF token: {forward_token}')
