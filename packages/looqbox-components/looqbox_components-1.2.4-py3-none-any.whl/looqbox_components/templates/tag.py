from looqbox import CssOption as css, ObjColumn, ObjRow, ObjHTML, ObjText
...
class Tag(LooqObject):
    ...
    def set_font_size(self, font_size: int | str):
        self._text_default_style = css.add(css.FontSize(font_size), self._text_default_style)
        return self
    ...
    def _get_content(self) -> ObjColumn:
        ...
        symbol = [ObjHTML(symbol)] if (symbol := rule.get("symbol")) else []
        ...
        tag_container = ObjColumn(
            ObjRow(
                symbol + [text_obj],
                ).set_horizontal_child_spacing("5px"),
