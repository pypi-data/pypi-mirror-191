...
#---------------------METHODS----------------------------
    ...
    def get_intrinsic_camera_parameters(self, downsampling:int=1) -> o3d.camera.PinholeCameraIntrinsic():
        ...
        #! f should be in px in opencv, why are these smaller? (this was important for open3D)
        fx=f
        fy=f
        ...
        if (getattr(self,'principalPointU',None) is not None and
            ...
            cx=width/2-0.5
            cy=height/2-0.5
        pinholeCameraIntrinsic = o3d.camera.PinholeCameraIntrinsic(width,height,fx,fy,cx,cy)
