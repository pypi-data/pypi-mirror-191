...
class Extractor(Performance):
    ...
    def _check_ad(self, news_date: str):
        ...
        return self._pattern4.search(news_date) != None
    ...
    def _extract_data_from_single_news(self, url: str, value: dict):
        ...
        raw_news_date = soup.select(
            value['date']) if value['date'] != None else ''
        ...
        for index in range(len(news_headlines)):
            ...
            complete_news = {
                'id': self.sorting._ordering_date(news_date),
    ...
    def data_extractor(self, news: list) -> list:
        ...
        with concurrent.futures.ThreadPoolExecutor() as executor:
            future_to_news = {executor.submit(
                self._extract_data_from_single_news, url, value): (url, value) for single_news in news for url, value in single_news.items()}
        ...
        return self.sorting._ordering_news(news_data)