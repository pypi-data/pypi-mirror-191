...
class Run(Resource, types.Run, Thread):
    ...
    def _log_contexts(self):
        """Log execution contexts as run fields."""
        try:
            paths = []
            git_context = GitContext().get()
            if git_context is not None:
                self.git_context = git_context
                paths.append("git_context")
            ...
            if self.client.config.debug:
                print(f"Error logging contexts: {e}")
