...
class PluginMetadata:
    ...
    # Add unit ids enum to enable unit selection through plugin params.
    unit_ids: InitVar[Optional[Type[UnitId]]] = None

    def __post_init__(self, unit_ids: Optional[Type[UnitId]]):
        self.parameters = (
            ...
            *(
                (
                    Parameter(
                        field="Mode5", label="Included units", default=unit_ids.names()
                    ),
                )
                if unit_ids
                else ()
