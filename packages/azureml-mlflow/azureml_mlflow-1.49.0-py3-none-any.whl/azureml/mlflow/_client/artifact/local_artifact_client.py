...
class LocalArtifactClient(BaseArtifactsClient):
    ...
    def upload_dir(self, local_dir, artifact_path):
        ...
        empty_artifact_res = self._create_empty_artifacts(
            paths=remote_paths, origin=self._origin, container=self._container)

        result = self._upload_files(
            local_paths=local_paths, remote_paths=remote_paths, empty_artifact_content=empty_artifact_res)
        return result
