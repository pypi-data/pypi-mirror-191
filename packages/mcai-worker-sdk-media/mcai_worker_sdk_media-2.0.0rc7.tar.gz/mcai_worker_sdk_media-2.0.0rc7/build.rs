    ...
    if let Ok(feat) = env::var("CARGO_FEATURE_MEDIA") {
      if feat == "1" {
        urls.insert(
          ...
          Value::try_from(format!(
            "https://media-cloud-ai.gitlab.io/sdks/py_mcai_worker_sdk/mcai_worker_sdk_media/v{}",
      ...
      urls.insert(
        "Documentation".to_string(),
        Value::try_from(format!(
          "https://media-cloud-ai.gitlab.io/sdks/py_mcai_worker_sdk/mcai_worker_sdk/v{}",
          env::var("CARGO_PKG_VERSION").unwrap()
        ))
        .unwrap(),
      );
