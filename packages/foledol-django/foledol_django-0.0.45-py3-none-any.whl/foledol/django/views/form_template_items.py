...
class FormTemplateItemsTable(Table):
    def __init__(self, rows):
        ...
        self.update = 'django:template_item_update'
        self.create = 'django:template_item_create'
        self.search = True
...
def form_template_item_list(request):
    ...
    template_items = FormTemplateItem.objects.all()

    search = get_search(request).strip()
    if len(search) > 0:
        template_items = FormTemplateItem.objects.filter(label=search)
    context['search'] = search

    columns = template_items.order_by('order')
