...
class FormTemplate(models.Model):
    ...
    def clone(self):
        ...
        template.comment = self.comment
        template.save()

        for template_item in self.template_item_set.all():
            template_item.clone(template)

        return template


class FormTemplateItem(models.Model):
    template: FormTemplate = models.ForeignKey(FormTemplate, on_delete=models.DO_NOTHING, related_name='items', null=True)
    ...
    def id_as_str(self):
        return 'template_item_' + str(self.id)
    ...
    def clone(self, template):
        ...
        template_item.label = self.label
        template_item.type = self.type
        template_item.order = self.order
        template_item.comment = self.comment
        template_item.optional = self.optional
        template_item.save()
