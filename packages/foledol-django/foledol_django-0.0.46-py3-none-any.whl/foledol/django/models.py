...
class FormTemplate(models.Model):
    ...
    def clone(self):
        ...
        form_template.name = self.name + "'"
        form_template.comment = self.comment
        form_template.save()

        for item in self.items.all():
            item.clone(form_template)

        return form_template


class FormTemplateItem(models.Model):
    form_template: FormTemplate = models.ForeignKey(FormTemplate, on_delete=models.DO_NOTHING, related_name='items', null=True)
    ...
    def as_str(self):
        return self.label_as_str()

    def id_as_str(self):
        return 'form_template_item_' + str(self.id)

    def label_as_str(self):
        field = self.get_field()
        return field.label if field else self.label

    def clone(self, form_template):
        form_template_item = FormTemplateItem(form_template=form_template)
        form_template_item.label = self.label
        form_template_item.type = self.type
        form_template_item.order = self.order
        form_template_item.comment = self.comment
        form_template_item.optional = self.optional
