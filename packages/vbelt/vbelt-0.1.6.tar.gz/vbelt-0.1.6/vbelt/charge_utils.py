...
class Charge:
    ...
    def __rmul__(self, x):
        if not isinstance(x, (float, int, complex)):
            raise ValueError("Charge instance can only be multiplied by a numerical scalar.")
    ...
    def __div__(self, x):
        if not isinstance(x, (float, int, complex)):
            raise ValueError("Charge instance can only be divided by a numerical scalar.")
