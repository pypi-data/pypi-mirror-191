...
class IMUPacketService(imu_packet_pb2_grpc.IMUPacketServiceServicer):
    ...
    def GetPacketArray(self, request, context):
        ...
        self.index += 1
        return imu_packet_pb2.IMUPacketArrayResponse(packets=[imu_packet_pb2.IMUPacketResponse(id=self.index, timestamp=time.time_ns()),  imu_packet_pb2.IMUPacketResponse(id=self.index, timestamp=time.time_ns())])
