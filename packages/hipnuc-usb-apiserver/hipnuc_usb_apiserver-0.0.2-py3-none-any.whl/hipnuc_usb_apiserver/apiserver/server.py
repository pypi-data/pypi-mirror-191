...
class IMUPacketService(imu_packet_pb2_grpc.IMUPacketServiceServicer):
    ...
    def GetFIFOStatus(self, request: imu_packet_pb2.IMUSetFIFOStatusRequest, context):
        return imu_packet_pb2.IMUStatusResponse(status=self.app.fifo_status)

    def SetFIFOStatus(self, request: imu_packet_pb2.IMUGetFIFOStatusRequest, context):
        if request.status:
