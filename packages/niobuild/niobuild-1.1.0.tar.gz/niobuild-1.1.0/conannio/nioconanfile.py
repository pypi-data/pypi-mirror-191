...
class NioConanFile(ConanFile):
    ...
    def init(self):
        self.sel4_sdk = 'sel4-osf/develop@'
        requires = self.conan_data.get('requires', [])
        for recipe in requires:
            recipe_name = recipe.split('/')[0]
            if recipe_name == 'sel4-osf':
                self.sel4_sdk = recipe
                if '@' not in self.sel4_sdk:
                    self.sel4_sdk += '@'
                requires.remove(recipe)
                break
    ...
    def package_info(self):
        self.user_info.deploy = self.collect_runtime_deploy()
    ...
    def collect_runtime_deploy(self):
        deploy_files = []
        ...
        if deploy_match:
            ...
            deploy_match = {"files": ['bin/*', 'etc/*', 'var/*', 'share/*']}

        package_folder = pathlib.Path(self.package_folder)
        files = []
        for deploy, matches in deploy_match.items():
            [files.append(fn) for file in matches for fn in package_folder.glob(file) if deploy == 'files']
        for fn in files:
    ...
    def disk_image(self, images_output="disk.img"):
        ...
        def package_to_files(package, prefix):
            files = []
            if package == self.name:
                deploy = self.collect_runtime_deploy()
                rootpath = self.package_folder
            else:
                deploy = eval(self.deps_user_info[package].deploy)
                rootpath = self.deps_cpp_info[package].rootpath
            for d in deploy:
        ...
        def part_args(part, value):
            ...
            part_prefix = value.pop('prefix', '/usr')
            for package in value.pop('packages', []):
                if isinstance(package, dict):
                    for package, prefix in package.items():
                        break
                else:
                    prefix = part_prefix
                packages_files = package_to_files(package, prefix)
                if packages_files:
                    if 'paths' in value:
