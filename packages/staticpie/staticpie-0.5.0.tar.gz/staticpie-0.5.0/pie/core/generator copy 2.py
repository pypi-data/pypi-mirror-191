...
class Generator:
    ...
    def generate(self, **kwargs):
        ...
        if self.config['nocache'] == False and os.path.exists(cache_filename):
        ...
        with console.status('[bold green] Generating...'):
            ...
            for md_file in md_files:
                ...
                if self.config['nocache'] == False and _hash in cache.keys():
                    _data = cache[_hash].copy()
                    print('FOUND YAML')
                else:
            ...
            for file in self.all_files:
                _hash = self._get_hash(file['content'])
                if _hash in cache.keys():
                    body, toc = cache[_hash]
                    print('FOUND MD2HTML')
                else:
                    body, toc = self.md2html(file['content'])
                    cache[_hash] = (body, toc)
                file['meta']['toc'] = toc
                ...
                try:
                    ...
                    _hash = self._get_hash((tpl, self.config, file))
                    if _hash in cache.keys():
                        html = cache[_hash]
                        print('FOUND HTML')
                    else:
