...
class Generator:
    ...
    def _get_cache_filename(self)->str:
        return f'{self.config["ROOT_FOLDER"]}/.cache.pickle'
    ...
    @if_cache_on
    def _get_cache(self, obj)->Any:
        _hash = self._get_hash(obj)
    ...
    def generate(self, **kwargs):
        ...
        with console.status('[bold green] Generating...'):
            ...
            for md_file in md_files:
                ...
                try:
                    _hash = self._get_hash((content_str, meta_str))
                    _data = self._get_cache(_hash)
                    print('FOUND YAML')
                except self.NoCacheValue:
            ...
            for file in self.all_files:
                try:
                    body, toc = self._get_cache(file['content'])
                    print('FOUND MD2HTML')
                except self.NoCacheValue:
                    body, toc = self.md2html(file['content'])
                    self._set_cache(file['content'], (body, toc))
                ...
                try:
                    ...
                    try:
                        html = self._get_cache((tpl, self.config, file))
                        print('FOUND HTML')
                    except self.NoCacheValue:
                        ...
                        self._set_cache((tpl, self.config, file), html)
