...
class UMAPTransform(object):
    def __init__(self,
        ...
        self.embedder = read_pickle(data_path=model_path)
        self.data = read_pickle(data_path=data_path)
        self.data = drop_low_variance_fields(data=self.data['DATA'], fields=self.embedder['low_var_cols'])
        self.data = scaler_transform(data=self.data, scaler=self.embedder['scaler'])

        transformed_data = pd.DataFrame(self.embedder['models'].transform(self.data), columns=['X', 'Y'])
...
model_path = '/Users/simon/Desktop/envs/troubleshooting/unsupervised/dr_models/affectionate_brahmagupta.pickle'
