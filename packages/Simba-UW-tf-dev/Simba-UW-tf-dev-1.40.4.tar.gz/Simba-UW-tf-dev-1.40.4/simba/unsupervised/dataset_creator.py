...
class DatasetCreator(object):
    def __init__(self,
        ...
        self.bp_names = getBpNames(inifile=self.config_path)[0]
    ...
    def all_data_concatenator(self):
        ...
        self.df_bp = self.df[self.bp_names]
    ...
    def all_features_concatenator(self):
        ...
        self.df = pd.concat(self.df, axis=0).reset_index(drop=True)
        self.df_bp = self.df[self.bp_names]
        self.df.drop(self.bp_names, inplace=True, axis=1)
    ...
    def slice_clf(self):
        ...
        if clf_setting == 'ALL CLASSIFIERS PRESENT':
            clf_cnt = self.df[self.clf_names].sum(axis=1)
            idx_lst = list(clf_cnt.index[clf_cnt >= 1])
            self.df = self.df[self.df.index.isin(idx_lst)]
        else:
            clf_name, self.clf_type = clf_setting.split(' ')[0], clf_setting.split(' ')[-1]
            print(self.clf_type)
            if self.clf_type == 'PRESENT':
                self.df = self.df[self.df[clf_name] == 1]
            elif self.clf_type == 'ABSENT':
                self.df = self.df[self.df[clf_name] == 0]
            elif self.clf_type == '(BOUT-AGGREGATION)':
                self.df = bout_aggregator(data=self.df, clf=clf_name)
                print('ss')
    ...
    def save(self):
        ...
        if self.clf_type != '(BOUT AGGREGATION)':
            self.results['DATA'] = self.df.drop(self.clf_cols + ['FRAME', 'VIDEO'], axis=1)
        else:
            self.results['DATA'] = self.df.drop(self.clf_cols + ['START FRAME', 'END FRAME', 'VIDEO'], axis=1)
...
settings = {'data_slice': 'ALL FEATURES (EXCLUDING POSE)',
            'clf_slice': 'ALL PRESENT (BOUT-AGGREGATION)',
