...
def UMAPTransform(model_path: str,
        ...
        data_df = drop_low_variance_fields(data=data['DATA'], fields=embedder['low_var_cols'])
        ...
        transformed_data = pd.DataFrame(embedder['models'].transform(scaled_data), columns=['X', 'Y'])
...
# settings = {'feature_values': True, 'scaled_features': True, 'format': 'csv'}
