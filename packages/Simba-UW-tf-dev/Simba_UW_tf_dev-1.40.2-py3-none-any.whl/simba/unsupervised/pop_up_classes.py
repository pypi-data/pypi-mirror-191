...
class BatchDataExtractorPopUp(object):
    def __init__(self):
        ...
        self.feature_options = ['INCLUDE FEATURE DATA (ORIGINAL)',
        ...
        settings_frm = LabelFrame(self.main_frm, text='SETTINGS', font=Formats.LABELFRAME_HEADER_FORMAT.value, padx=5, pady=5, fg='black')
        self.features_dropdown = DropDownMenu(settings_frm, 'SCATTER SIZE:', self.feature_options, '25')
    ...
    def run(self):
        ...
        if self.features_dropdown.getChoices() == self.feature_options[0]:
            settings['include_features'] = True
        if self.features_dropdown.getChoices() == self.feature_options[1]:
            settings['include_features'] = True
