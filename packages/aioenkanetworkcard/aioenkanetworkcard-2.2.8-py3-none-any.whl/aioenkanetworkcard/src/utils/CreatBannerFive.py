...
async def generationFive(characters,assets,img,lvl,splash,signatureRes):
    ...
    if img != None:
        fra = await background(img,False,characters.element,signatureRes)
    else:
        fra = await background(person.images.banner.url,True,characters.element,signatureRes)
    we = await weapon(characters.equipments[-1],lvl)
    st = await stats(characters,assets)
    info = await infoCharter(characters,lvl,characters.element)
    const = await constant(characters)
    talant = await talants(characters.skills)
    art  = await artifact(characters.equipments)
    await build(fra,we,st,info,const,talant,art["art"], art["set"])