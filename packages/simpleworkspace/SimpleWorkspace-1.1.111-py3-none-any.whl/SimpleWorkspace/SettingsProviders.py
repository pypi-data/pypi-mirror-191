...
class SettingsManager_BasicConfig(SettingsManager_Base):
    '''
        ...
        * This parser is partially compatible with INI format aswell. The differece is that sections are not parsed/used,
            ther are instead marked as a comment and left untouched, this means that sections have same role as comments
            and most importantly will therefore not fail when parsing and not be removed/altered when saving new settings
    '''
    def __init__(self, settingsPath):
        super().__init__(settingsPath)
        ...
        with open(filepath) as fp:
            for line in fp:
                ...
                if line == '': #only a blank line
                    ...
                    continue
                if line.startswith('#') or line.startswith('['): #only a comment line, '[' is added as comment for INI compatibility
                ...
                if(len(keyValue) != 2):
                    raise ValueError("file contains bad line format: key/value pair is not separated with '='")
