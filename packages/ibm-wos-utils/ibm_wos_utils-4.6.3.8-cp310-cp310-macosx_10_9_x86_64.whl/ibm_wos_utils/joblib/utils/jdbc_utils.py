...
class DB2JDBCUtil(BaseJDBCUtil):
    ...
    def _read(self, query, columns_to_map=None, partition_column=None, lower_bound=None, upper_bound=None, num_partitions=None):
        ...
        # If some column contains space, reading with spark-JDBC fails with pyspark.sql.utils.ParseException. WI #28110
        # To fix this, disabling predicate pushdown.
        if columns_to_map is None or len(columns_to_map) == 0 or any(" " in column for column in columns_to_map):
