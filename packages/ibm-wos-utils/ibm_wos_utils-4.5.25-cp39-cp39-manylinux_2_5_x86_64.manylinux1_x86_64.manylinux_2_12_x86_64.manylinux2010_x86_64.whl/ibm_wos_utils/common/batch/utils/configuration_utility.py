    ...
    def __make_single_archive(self,output_path, client_engine):
        ...
        if self.enabled_explainability:
            ...
            with open(archive_directory + "/explainability_statistics.json") as stats_file:
                ...
                perturbations=Perturbations(training_stats=explainability_configuration, problem_type=model_type)
                ...
                if model_type == "regression":
                    perturbation_df = pd.DataFrame(
                        {
                            self.common_config["prediction"]: predict_probability.tolist()
                        })
                else:
                    perturbation_df = pd.DataFrame(
                        {
                            self.common_config["probability"]: predict_probability[0].tolist(),
                            self.common_config["prediction"]: predict_probability[1].tolist()
                        })
