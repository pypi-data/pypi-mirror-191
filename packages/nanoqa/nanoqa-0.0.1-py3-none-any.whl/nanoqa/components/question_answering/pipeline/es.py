...
from nanoQA.components.document_store import ElasticsearchDocumentStore as DocumentStore
from nanoQA.components.question_answering import Reader, Retriever, Ranker
from nanoQA.components.rich_utils import display_time_taken, display_retrieved_documents, display_answers, display_json
from nanoQA.schemas import ReaderArguments, FinalPrediction, Document
from nanoQA.schemas.pipeline.pipeline_schemas import PipelineAnswer, PipelineResponse
...
class ElasticSearchPipeline:
    ...
    def _init_document_store(self) -> DocumentStore:
        return DocumentStore(host="localhost", port=9200, index=self.index_hash)
    ...
    def run(
        ...
        # TODO: if reader is not applied, return the answer from retrieval
        if not use_reader:
