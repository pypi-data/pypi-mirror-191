...
@PIPELINES.register_module()
class Random_Paste(object):
    """ expand the image & bboxes.
    ...
    def __call__(self, results):
        ...
        h, w, c = img.shape
        # print(img.shape)
        assert h<self.canvas_size[1], '{}<canvas.h={}'.format(h,self.canvas_size[1])
        assert w<self.canvas_size[0], '{}<canvas.w={}'.format(w,self.canvas_size[0])
        expand_img = np.full((int(self.canvas_size[1]), int(self.canvas_size[0]), c),
                             self.mean,
                             dtype=img.dtype)
        left = int(random.uniform(0, self.canvas_size[0]-w))
        top = int(random.uniform(0, self.canvas_size[1]-h))
...
class RandomErasingInBboxes:
    ...
    def __init__(self,
        ...
        self.max_bboxes_num = max_bboxes_num
    ...
    def __call__(self, results):
        ...
        if len(gt_bboxes) > self.max_bboxes_num:
            return results
        ...
        for gt_bbox in gt_bboxes:
            ...
            if random.uniform(0, 1) > self.bbox_probability:
                continue
            ...
            if w < bbox_w and h < bbox_h:
                y1 = random.randint(0, bbox_h - h)
                x1 = random.randint(0, bbox_w - w)
                x1 = x1 + int(gt_bbox[0])
                y1 = y1 + int(gt_bbox[1])
                # pdb.set_trace()
                assert results['img'].shape[2] == 3
                results['img'][y1:y1 + h, x1:x1 + w, :] = self.mean

        return results

    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += f'(random_prob={self.probability}, '
        repr_str += (f'earsed_area_ratio={[self.sl, self.sh]},')
        repr_str += f'earsed_aspect_ratio={self.r1, 1/self.r1},'
        repr_str += f'earsing_value={self.mean})'
        return repr_str

@PIPELINES.register_module()
class RandomPatchInBboxes:
    ...
    def __init__(self,
                 prob_happen=0.5, pool_capacity=20000, min_sample_size=100,
    ...
    #         # pdb.set_trace()
    #         # collect new patch
    #         # bbox_img = img.crop((gt_bbox[0], gt_bbox[1],
    #         #                      gt_bbox[2], gt_bbox[3]))
    #         bbox_img = img.crop(gt_bbox)
    #         self.patchpool.append(bbox_img)
    #
    #         if random.uniform(0, 1) > self.bbox_probability:
    #             continue
    #
    ...
    #             continue
    #
    #         w, h = self.generate_wh(bbox_w, bbox_h)
    #
    ...
    #             min_w = min(patchW, w)
    #             min_h = min(patchH, h)
    #             patch_x1 = random.randint(0, patchW - min_w)
    #             patch_y1 = random.randint(0, patchH - min_h)
    #             new_patch = patch.crop((
    #                 patch_x1, patch_y1, patch_x1+min_w, patch_y1+min_h
    #             ))
    #             new_patchW, new_patchH = new_patch.size
    #             x1 = random.randint(0, bbox_w - new_patchW)
    #             y1 = random.randint(0, bbox_h - new_patchH)
    #             patch = self.transform_patch(new_patch)
    #             bbox_img.paste(patch, (x1, y1))
    #             img.paste(bbox_img, gt_bbox)
    ...
    #         # patch = random.sample(self.patchpool, 1)[0]
    #         # patchW, patchH = patch.size
    #         # x1 = random.randint(0, bbox_w - patchW)
    #         # y1 = random.randint(0, bbox_h - patchH)
    ...
    def __call__(self, results):
        ...
        for gt_bbox in gt_bboxes:
            ...
            if w is not None and h is not None:
                x1 = random.randint(0, bbox_w - w)
                y1 = random.randint(0, bbox_h - h)
                new_patch = bbox_img.crop((x1, y1, x1 + w, y1 + h))
    ...
    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += f'(prob_happen={self.prob_happen})'
        return repr_str