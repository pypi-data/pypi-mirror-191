...
class SingleTwoStageDetectorNAEContrastDomainAll(BaseContrastDetector):
    ...
    def __init__(self,
                 ...
                 det_num_domain=1,
                 reid_num_domain=1,
                 weight_da_img_loss=1,
                 weight_da_ins_loss=1,
                 weight_da_cst_loss=0,
                 only_detection_imglevel_da=True,
                 imgda_channel=1024, insda_channel_det=2048,insda_channel_reid=256,
                 ...
                 pretrained=None):
        super(SingleTwoStageDetectorNAEContrastDomainAll, self).__init__()
        self.contrast_mode =contrast_mode
        self.detection_data_randomerasing = detection_data_randomerasing

        self.backbone = build_backbone(backbone)
        self.weight_da_img_loss = weight_da_img_loss
        self.weight_da_ins_loss = weight_da_ins_loss
        self.weight_da_cst_loss = weight_da_cst_loss
        self.stride = stride
        ...
        self.only_detection_imglevel_da=only_detection_imglevel_da
        if self.only_detection_imglevel_da:
            from .da_head import DomainAdaptationModule
        else:
            from .da_head_img_instance_ce import DomainAdaptationModule
            from .da_head_img_instance_ce import DomainAdaptationModule_
        if det_num_domain>1:
            self.da_heads=DomainAdaptationModule(det_num_domain, self.weight_da_img_loss,self.weight_da_ins_loss,
                                                 self.weight_da_cst_loss,
                                                 imgda_channel=imgda_channel,insda_channel=insda_channel_det)
        self.det_num_domain=det_num_domain
        self.reid_num_domain=reid_num_domain
        if reid_num_domain > 1:
            self.da_heads_reid = DomainAdaptationModule(reid_num_domain, self.weight_da_img_loss, self.weight_da_ins_loss,
                                                   self.weight_da_cst_loss,
                                                         imgda_channel=imgda_channel,insda_channel=insda_channel_reid)
    ...
    def init_weights(self, pretrained=None):
        ...
        Args:
            pretrained (str, optional): Path to pre-trained weights.
                Defaults to None.
        """
        super(SingleTwoStageDetectorNAEContrastDomainAll, self).init_weights(pretrained)
        # pdb.set_trace()
    ...
    def encoder(self,
        ...
        for ind, img_meta in enumerate(img_metas):
            ...
            if data_type == 're_id':
                ...
                if len(gt_ids[ind])==1 and gt_ids[ind][0]==-1:
                    contrastive_data_id.append(ind)
            else:
        ...
        if self.with_rpn and len(det_data_id) > 0:
            ...
            roi_losses_det, feats_pids = self.roi_head.forward_train(xb_det, img_metas_det, proposal_list_det,
                                                                     gt_bboxes_det, gt_labels_det, gt_ids_det,
                                                                     gt_bboxes_ignore_det, gt_masks,
                                                                     **kwargs)
            #img level domain alignment
            if self.det_num_domain>1:
                # pdb.set_trace()
                # bbox_feats=feats_pids['bbox_feats']
                gt_domain_ids_det = [gt_domain_ids[ind] for ind in det_data_id]
                _, feats_pids_da = self.roi_head.forward_train(xb_det, img_metas_det, proposal_list_det,
                                                               gt_bboxes_det, gt_labels_det, gt_ids_det,
                                                               gt_bboxes_ignore_det, gt_masks,da_alignment=True,
                                                               gt_domains=gt_domain_ids_det,
                                                               **kwargs)

                if self.only_detection_imglevel_da:
                    loss_img_align=self.da_heads(xb_det,gt_domain_ids_det)
                else:
                    instance_level_feats_det = feats_pids_da['bbox_feats_2048']
                    # if instance_level_feats_det.shape[0]%128!=0:
                    #     pdb.set_trace()
                    # pdb.set_trace()
                    instance_level_feats_domains_det = feats_pids_da['ins_domains']
                    loss_img_align = self.da_heads(xb_det, gt_domain_ids_det,
                                                   instance_level_feats_det,
                                                   instance_level_feats_domains_det
                                                   )

                losses.update(loss_img_align)

        xb_contrastive = None
        return_rois = None
        if len(contrastive_data_id)>0:
            # print('========{}'.format(xb[0].shape))
            # print('========contrastive_data_id{}'.format(contrastive_data_id))
            xb_contrastive = (xb[0][contrastive_data_id],)  # tensor
            gt_bboxes_contrastive = [gt_bboxes[ind] for ind in contrastive_data_id]  # list
            if self.contrast_mode == 'gt_bbox':
                # fetch features with gt_bboxes to do contrastive learning
                return_rois = gt_bboxes_contrastive
            # elif self.contrast_mode == 'bbox_pred':
            #     bbox_pred=feats_pids['bbox_pred']
        ...
        if len(reid_data_id) > 0:  # mask sure proposal_list is wrt ori images???
            ...
            gt_domain_ids_reid = [gt_domain_ids[ind] for ind in reid_data_id]
            ...
            bbox_results = self.roi_head._bbox_forward(xb_reid, rois,
                                                       gt_domains=gt_domain_ids_reid
                                                       )
            roi_losses_reid = self.roi_head.bbox_head.loss(None, None, bbox_results["feature"], None,
                                                           torch.cat(gt_ids_reid),
                                                           None, None, None, None)
            if self.reid_num_domain > 1:

                # pdb.set_trace()
                reid_imgs = xb_reid[0]  # [3, 1024, 94, 120] n,c,h,w
                gt_domain_ids_reid = [gt_domain_ids[ind] for ind in reid_data_id]
                reid_da_loss = {'loss_da_image_r': torch.tensor(0, dtype=rois.dtype, device=rois.device),
                             'loss_da_ins_r': torch.tensor(0, dtype=rois.dtype, device=rois.device),
                             'loss_da_consist_r': torch.tensor(0, dtype=rois.dtype, device=rois.device),
                             }
                for index, (i_, x1_, y1_, x2_, y2_) in enumerate(rois):
                    img_id = int(i_)
                    x1 = int(x1_/self.stride)
                    x2 = int(x2_/self.stride)
                    y1 = int(y1_/self.stride)
                    y2 = int(y2_/self.stride)
                    img_feat4 = reid_imgs[img_id, :, y1:y2, x1:x2].unsqueeze(0)
                    if 0 in img_feat4.shape:
                        pdb.set_trace()
                    instance_level_feats_reid = bbox_results['feature']
                    # pdb.set_trace()
                    instance_level_feats_domain_reid = bbox_results['ins_domains'][index].unsqueeze(0)
                    loss_img_align = self.da_heads_reid((img_feat4,), [gt_domain_ids_reid[img_id]],
                                                        instance_level_feats_reid[img_id].unsqueeze(0),
                                                        instance_level_feats_domain_reid
                                                        )
                    reid_da_loss['loss_da_image_r']+=loss_img_align['loss_da_image']
                    reid_da_loss['loss_da_ins_r'] += loss_img_align['loss_da_ins']
                    reid_da_loss['loss_da_consist_r'] += loss_img_align['loss_da_consist']
                reid_da_loss['loss_da_image_r']=reid_da_loss['loss_da_image_r']/len(rois)
                reid_da_loss['loss_da_ins_r'] = reid_da_loss['loss_da_ins_r'] / len(rois)
                reid_da_loss['loss_da_consist_r'] = reid_da_loss['loss_da_consist_r'] / len(rois)
                losses.update(reid_da_loss)
        ...
        detection_data_randomerasing_condition = self.detection_data_randomerasing['probability'] >= random.uniform(0,
                                                 ...
                                                 and len(return_rois) < self.detection_data_randomerasing[
                                                     'max_bboxes_num']
        xb_contrastive_feat = xb_contrastive
        if return_rois is not None and xb_contrastive_feat is not None:
            ...
            if detection_data_randomerasing_condition:
                ...
                # pdb.set_trace()
                det_features_list = []
                for index, det_img_ori in enumerate(xb_contrastive_feat[0]): #xb_det[0] [2, 1024, 94, 92]
                    det_img=det_img_ori.clone()
                    for roi in return_rois[index]/self.stride:
                        bbox_h = roi[3] - roi[1]
                        bbox_w = roi[2] - roi[0]
                        area = bbox_h * bbox_w
                ...
                xb_contrastive_feat = (torch.stack(det_features_list),)#tuple must have ,

        # pdb.set_trace()
        return return_rois, losses, xb_contrastive_feat
