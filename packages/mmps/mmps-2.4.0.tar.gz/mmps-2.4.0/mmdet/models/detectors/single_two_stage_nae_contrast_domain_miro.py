...
class SingleTwoStageDetectorNAEContrastDomainMIRO(BaseContrastDetector):
    ...
    def __init__(self,
                 ...
                 pretrained=None):
        super(SingleTwoStageDetectorNAEContrastDomainMIRO, self).__init__()
        self.contrast_mode =contrast_mode
        self.detection_data_randomerasing = detection_data_randomerasing

        self.backbone = build_backbone(backbone)
        # self.backbone_pred = build_backbone(backbone_pred)
        ...
        self.miro=MIRO(backbone_pred,weight_miro,pretrained=pretrained)
        ...
        if det_num_domain>1:
            self.da_heads=DomainAdaptationModule(det_num_domain, self.weight_da_img_loss,self.weight_da_ins_loss,self.weight_da_cst_loss)
        self.det_num_domain=det_num_domain
        self.reid_num_domain=reid_num_domain
        if reid_num_domain > 1:
            self.da_heads_reid = DomainAdaptationModule_(reid_num_domain, self.weight_da_img_loss, self.weight_da_ins_loss,
    ...
    def init_weights(self, pretrained=None):
        ...
        Args:
            pretrained (str, optional): Path to pre-trained weights.
                Defaults to None.
        """
        super(SingleTwoStageDetectorNAEContrastDomainMIRO, self).init_weights(pretrained)
        # pdb.set_trace()
    ...
    def encoder(self,
        ...
        xb_ = self.extract_feat(img)
        # print("=====here", xb.shape)
        # print("===={}".format(img_metas))
        # pdb.set_trace()
        loss_miro=self.miro(xb_,img)
        # pdb.set_trace()
        xb=(xb_[1],)
        img_types=[]
        # pdb.set_trace()
        ...
        if self.with_rpn and len(det_data_id) > 0:
            ...
            if self.det_num_domain>1:
                ...
                if self.only_detection_imglevel_da:
                    ...
                    instance_level_feats_pids_det = feats_pids_da['gt_pids']
                    loss_img_align = self.da_heads(xb_det, gt_domain_ids_det,instance_level_feats_det,instance_level_feats_pids_det)
