...
class SingleTwoStageDetectorNAEContrastReidWithoutBG(BaseContrastDetector):
    ...
    def __init__(self,
        ...
        super(SingleTwoStageDetectorNAEContrastReidWithoutBG, self).__init__()
    ...
    def init_weights(self, pretrained=None):
        ...
        Args:
            pretrained (str, optional): Path to pre-trained weights.
                Defaults to None.
        """
        super(SingleTwoStageDetectorNAEContrastReidWithoutBG, self).init_weights(pretrained)
        # pdb.set_trace()
    ...
    def encoder(self,
        ...
        if self.with_rpn and len(det_data_id) > 0:
            # pdb.set_trace()
            xb_det = self.extract_feat(img[det_data_id,:])
        ...
        if len(reid_data_id) > 0:  # mask sure proposal_list is wrt ori images???
            # pdb.set_trace()
            imgs_reid=img[reid_data_id, :]#n,c,h,w
            ...
            reid_loss={'loss_oim':torch.tensor(0,dtype=rois.dtype,device=rois.device)}
            for index,(i_,x1_,y1_,x2_,y2_) in enumerate(rois):
                ...
                y1 = int(y1_)
                y2 = int(y2_)
                img_=imgs_reid[img_id,:,y1:y2,x1:x2].unsqueeze(0)
                # img_ = imgs_reid[[0,1], :, y1:y2,x1:x2]
                xb_reid = self.extract_feat(img_)
                new_rois=torch.tensor([0.0,0.0,x2_ - x1_,0.0,y2_ - y1_],dtype=rois.dtype,device=rois.device).unsqueeze(0)
                # new_rois_1 = torch.tensor([1.0, 0.0, x2_ - x1_, 0.0, y2_ - y1_], dtype=rois.dtype,
                #                         device=rois.device).unsqueeze(0)
                # new_rois=torch.cat((new_rois,new_rois_1),0)
                # gt_id_reid=[gt_ids_reid[img_id],gt_ids_reid[img_id]]
                gt_id_reid = [gt_ids_reid[img_id]]
                bbox_results = self.roi_head._bbox_forward(xb_reid, new_rois)
                roi_losses_reid = self.roi_head.bbox_head.loss(None, None, bbox_results["feature"], None,
                                                               torch.cat(gt_id_reid),
                                                               None, None, None, None)
                reid_loss['loss_oim'] +=roi_losses_reid['loss_oim']
                # pass
            reid_loss['loss_oim']=reid_loss['loss_oim']/len(rois)
            # pdb.set_trace()
            # # imgs_crop_reid =
            # xb_reid = self.extract_feat(imgs_reid)
