...
from mmdet.core import distance2bbox, force_fp32, multi_apply, multiclass_nms_reid
from .anchor_free_head_reid import AnchorFreeHeadReid
from .labeled_matching_layer_queue import LabeledMatchingLayerQueue, LabeledMatchingLayerQueueNorm
from .unlabeled_matching_layer import UnlabeledMatchingLayer, UnlabeledMatchingFullLayer
...
class FCOSReidHeadFocalOimSubTriplet(AnchorFreeHeadReid):
    ...
    def __init__(self,
                 ...
                 norm_on_bbox=False,
                 is_test=False,
                 test_level=[0],  # list. default=[0],means P3.
                 # At test stage, test_level means the layer level to test while the neck is 'FPNDcnLconv3DcnChoose'(outs 5 lvls)
                 ...
                 queue_size=5000,
                 label_norm=False,####
                 unlabel_full=False,#是否用256d匹配
                 unlabel_weight=10,####
                 temperature=10,####
                 loss_oim_reduction='none',
                 **kwargs):
        ...
        self.loss_oim_reduction = loss_oim_reduction
    ...
    def _init_layers(self):
        ...
        if self.is_test:
            from .labeled_matching_layer_queue_seperate_update_and_backforward import LabeledMatchingLayerQueue, LabeledMatchingLayerQueueNorm
            from .unlabeled_matching_layer_seperate_update_and_backforward import UnlabeledMatchingLayer, UnlabeledMatchingFullLayer
        else:
            from .labeled_matching_layer_queue import LabeledMatchingLayerQueue, LabeledMatchingLayerQueueNorm
            from .unlabeled_matching_layer import UnlabeledMatchingLayer, UnlabeledMatchingFullLayer
        if self.label_norm:
            self.labeled_matching_layer = LabeledMatchingLayerQueueNorm(num_persons=num_person, feat_len=self.in_channels, is_test=self.is_test)
        else:
            self.labeled_matching_layer = LabeledMatchingLayerQueue(num_persons=num_person, feat_len=self.in_channels, is_test=self.is_test) # for mot17half
        if self.unlabel_full:
            self.unlabeled_matching_layer = UnlabeledMatchingFullLayer(queue_size=queue_size, feat_len=self.in_channels, is_test=self.is_test)
        else:
            self.unlabeled_matching_layer = UnlabeledMatchingLayer(queue_size=queue_size, feat_len=self.in_channels, is_test=self.is_test)
    ...
    def get_center_feature_p3(self,feats,proposals):
        # pdb.set_trace()
        # out = self(feats) #cls_score, bbox_pred, centerness, reid_feat
        feats = list(feats)
        ...
        mean_value = mean_value.repeat(1, 1, h, w)
        # pdb.set_trace()
        ...
        reid_feats=[]
        for rois_per_img in proposals:
            # for bbox in rois_per_img:
            # pdb.set_trace()
            center_xs = (rois_per_img[..., 0] + rois_per_img[..., 2]) / 2
            center_ys = (rois_per_img[..., 1] + rois_per_img[..., 3]) / 2  # 中心点坐标center_xs,center_ys
            map_center_xs = (center_xs/self.strides[0]).long()
            map_center_ys = (center_ys / self.strides[0]).long()
            if sum(map_center_ys>=feat_p3.shape[2])!=0 or sum(map_center_xs>=feat_p3.shape[3])!=0:
                pdb.set_trace()
            center_feat_per_img = feat_p3[:,:,map_center_ys, map_center_xs]
            center_feat_per_img = center_feat_per_img.permute(2, 1, 0).reshape(-1, self.feat_channels)
            reid_feats.append(center_feat_per_img)
        reid_feats = torch.cat(reid_feats)
        return reid_feats #Nx256
    ...
    def loss(self,
        ...
        if num_pos > 0:
            ...
            # labeled_matching_reid为lut里的feature， labeled_matching_ids为对应的id(eg.ids=[1,1,1,2,3],则feature对应1的feature相同)
            ...
            loss_oim = F.nll_loss(focal_p_i, pid_labels, reduction=self.loss_oim_reduction, ignore_index=-1)

            #pos_reid为input进来的reid feature，labeled_matching_reid为lut里的reid feature(里面有很多重复)
    ...
    def _get_bboxes_single(self,
        ...
        assert max(self.test_level) < len(cls_scores)
        # print('lvl_num:')
    ...
    def _get_target_single(self, gt_bboxes, gt_labels, gt_ids, points, regress_ranges,
        ...
        if self.center_sampling:
            ...
            center_xs = (gt_bboxes[..., 0] + gt_bboxes[..., 2]) / 2  # (num_points, num_gts, 1)
            center_ys = (gt_bboxes[..., 1] + gt_bboxes[..., 3]) / 2  # 中心点坐标center_xs,center_ys
            center_gts = torch.zeros_like(gt_bboxes)  # (num_points, num_gts, 4)
            stride = center_xs.new_zeros(center_xs.shape)  # (num_points, num_gts, 1)

            # project the points on current lvl back to the `original` sizes
            lvl_begin = 0
            for lvl_idx, num_points_lvl in enumerate(num_points_per_lvl):
                lvl_end = lvl_begin + num_points_lvl
                stride[lvl_begin:lvl_end] = self.strides[lvl_idx] * radius  # 8*1.5,16*1.5 ...
                lvl_begin = lvl_end
