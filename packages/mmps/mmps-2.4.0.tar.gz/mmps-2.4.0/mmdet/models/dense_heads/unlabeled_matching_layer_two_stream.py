...
class UnlabeledMatchingLayer_two_stream(nn.Module):
    """
    Unlabeled matching of OIM loss function.
    """

    def __init__(self, queue_size=5000, feat_len=256, stream_name=''):
        """
        Args:
        ...
        """
        super(UnlabeledMatchingLayer_two_stream, self).__init__()
        self.stream_name = stream_name
        self.register_buffer("queue_" + stream_name, torch.zeros(queue_size, feat_len))
        self.register_buffer("tail_" + stream_name, torch.tensor(0))
    ...
    def forward(self, features, pid_labels):
        ...
        """
        scores = UnlabeledMatching.apply(features, pid_labels, eval("self.queue_"+self.stream_name), eval("self.tail_"+self.stream_name))
        return scores
...
class UnlabeledMatchingFullLayer_two_stream(nn.Module):
    """
    Unlabeled matching of OIM loss function.
    """
    ...
    def __init__(self, queue_size=5000, feat_len=256, stream_name=''):
        ...
        Args:
            ...
            feat_len (int): Length of the feature extracted by the network.
        """
        super(UnlabeledMatchingFullLayer_two_stream, self).__init__()
        self.stream_name = stream_name
        self.register_buffer("queue_"+stream_name, torch.zeros(queue_size, feat_len))
        self.register_buffer("tail_"+stream_name, torch.tensor(0))
    ...
    def forward(self, features, pid_labels):
        ...
        Returns:
            scores (Tensor[N, queue_size]): Unlabeled matching scores, namely the similarities
                                            between proposals and unlabeled persons.
        """
        scores = UnlabeledMatchingFull.apply(features, pid_labels, eval("self.queue_"+self.stream_name), eval("self.tail_"+self.stream_name))
        return scores
