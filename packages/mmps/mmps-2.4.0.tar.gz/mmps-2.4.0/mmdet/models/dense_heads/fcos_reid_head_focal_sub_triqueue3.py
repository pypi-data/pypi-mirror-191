...
class FCOSReidHeadFocalSubTriQueue3(AnchorFreeHeadReid):
    ...
    def _init_layers(self):
        ...
        if self.is_test:
            from .labeled_matching_layer_queue_seperate_update_and_backforward import LabeledMatchingLayerQueue, \
                LabeledMatchingLayerQueueNorm
            from .unlabeled_matching_layer_seperate_update_and_backforward import UnlabeledMatchingLayer, \
                UnlabeledMatchingFullLayer
        else:
        ...
        if self.label_norm:
            self.labeled_matching_layer = LabeledMatchingLayerQueueNorm(num_persons=num_person,
                                                                        feat_len=self.in_channels, is_test=self.is_test)
        else:
            self.labeled_matching_layer = LabeledMatchingLayerQueue(num_persons=num_person, feat_len=self.in_channels,
                                                                    is_test=self.is_test)  # for mot17half
    ...
    def loss(self,
        ...
        if num_pos > 0:
            ...
            # reid oim loss
            labeled_matching_scores, labeled_matching_reid, labeled_matching_ids = self.labeled_matching_layer(pos_reid,
                                                                                                               pos_reid_ids)
            ...
            p_i = torch.zeros(pos_reid.shape[0], device=pos_reid.device)
            print('no gt box')
