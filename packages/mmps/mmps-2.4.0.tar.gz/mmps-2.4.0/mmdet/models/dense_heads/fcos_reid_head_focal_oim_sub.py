...
from .labeled_matching_layer import LabeledMatchingLayer, LabeledMatchingLayerNorm
...
class FCOSReidHeadFocalOimSub(AnchorFreeHeadReid):
    ...
    def __init__(self,
                 ...
                 regress_ranges=((-1, INF), (-2, -1), (-2, -1), (-2, -1),
                                (-2, -1)),
                 # regress_ranges=((-1, INF), (-1, INF), (-2, -1), (-2, -1),
                 #               (-2, -1)),
    ...
    def _init_layers(self):
        ...
        if self.label_norm:
            self.labeled_matching_layer = LabeledMatchingLayerNorm(num_persons=num_person, feat_len=self.in_channels)
        else:
            self.labeled_matching_layer = LabeledMatchingLayer(num_persons=num_person, feat_len=self.in_channels) # for mot17half
        if self.unlabel_full:
            self.unlabeled_matching_layer = UnlabeledMatchingFullLayer(queue_size=queue_size, feat_len=self.in_channels)
        else:
