...
REQUIRED_SQL: list = ['azure', 'connector', 'server', 'database']
REQUIRED_SHOPIFY: list = ['store_name', 'access_token']
...
class Config:
    ...
    def test_required(self) -> None:
        """Ensure required configuration items are set."""
        for itm in REQUIRED_SQL:
            if self.sql_conf.get(itm) is None:
                raise ValueError(f"'{itm}' is required in SQL configuration")
        for itm in REQUIRED_SHOPIFY:
            if self.shopify.get(itm) is None:
                raise ValueError(f"'{itm}' is required in Shopify configuration")
