...
AZURE_MYSQL_SSL = 'https://cacerts.digicert.com/DigiCertGlobalRootG2.crt.pem'
...
class DBFactory:
    ...
    azure : bool
        Azure SQL Server flag.
    ssl_cert : str
        URL or path to SSL certificate for Azure SQL Server.
    ...
    Instantiate class with windows authentication and specify ODBC driver
    >>> db_factory = DBFactory(connector='mssql+pyodbc', server='localhost',
    ...                  port=1433, db_name='shop_rest',
    ...                  windows_auth=True,
    ...                  connection_query={'driver': 'ODBC Driver 17 for SQL Server'})

    Connect to Azure with SSL
    >>> db_factory = DBFactory(connector='mssql+pyodbc', server='localhost',
    ...                        port=1433, db_name='shop_rest', sa_user='sa',
    ...                        sa_pass='password', azure=True, encrypt=True)
    ...
    def get_db_engine(self, db: Optional[str] = None) -> Engine:
        ...
        engine_container = EngineContainer(db_config)
        return engine_container.engine
    ...
    def create_db(self) -> None:
        ...
        if self.master_engine is None:
            logger.debug("Get engine for Master database table before creating database.")
        else:
    ...
    def create_user(self, db_user: Optional[str] = None,
                    db_pass: Optional[str] = None) -> bool:
        ...
        if self.db_engine is None:
            logger.debug('Get engine for database before creating user.')
            return False
        else:
            if self.dialect == 'mssql':
                if self.master_engine is not None:
                    with self.master_engine.connect().execution_options(
                            isolation_level='AUTOCOMMIT') as conn:
                    ...
                    logger.debug("Master engine not connected")
                    return False
...
class EngineContainer:
    ...
    def __init__(self, sql_conf: Union[dict, SectionProxy]) -> None:
        """Initialize engine container & connect to engine."""
        if create_engine is None:
            raise ImportError("Error importing SQL Server dependencies")
        self.connector = sql_conf['connector']
        self.dialect = self.connector.split('+')[0]
        if self.dialect not in SUPPORTED_DIALECTS:
            raise Exception(f"{self.dialect} is not a supported dialect.")
        if isinstance(sql_conf, SectionProxy):
            windows_auth = sql_conf.getboolean('windows_auth', False)
            port = sql_conf.getint('port', None)
            self.azure = sql_conf.getboolean('azure', False)
            encrypt = sql_conf.getboolean('encrypt', False)
        elif isinstance(sql_conf, dict):
            windows_auth = sql_conf.get('windows_auth', False)
            port = sql_conf.get('port', None)
            self.azure = sql_conf.get('azure', False)
            encrypt = sql_conf.get('encrypt', False)
        self.con_qry = self.parse_query(sql_conf.get('connection_query', None))
        self.ssl_cert = None
        ...
        logger.debug('\n')
        logger.debug(json.dumps(self.conn_dict, indent=4))
        logger.debug('\n')

        if self.dialect in MYSQL_SYNONYMS:
            if self.azure is True and encrypt is True:
                # Test for None because SectionProxy has funky fallback logic
                if sql_conf.get('ssl_cert') is None:
                    self.conn_dict['ssl_cert'] = AZURE_MYSQL_SSL
                else:
                    self.conn_dict['ssl_cert'] = sql_conf['ssl_cert']
            self.engine = self.create_mysql_engine(self.conn_dict)
        elif self.dialect == 'mssql':
            self.engine = self.create_mssql_engine(self.conn_dict)

    def create_mssql_engine(self, sql_conf):
        url = self.mssql_connection_string(sql_conf)
        return create_engine(url)

    def create_mysql_engine(self, sql_conf):
        url = self.mysql_connection_string(sql_conf)
        logger.debug("Encryption set to %s", sql_conf.get('encrypt'))
        con_args = None
        if sql_conf.get('encrypt') is True:
            if sql_conf.get('azure') is True:
                self.cert_path = self.get_certificate(
                    sql_conf.get('ssl_cert', AZURE_MYSQL_SSL))
            elif sql_conf.get('encrypt') is True:
                if sql_conf.get('ssl_cert') is not None:
                    self.cert_path = self.get_certificate(sql_conf.get('ssl_cert'))
        ...
        if con_args is not None:
            return create_engine(url, connect_args=con_args)
        return create_engine(url)

    @staticmethod
    def parse_query(raw_query: Union[str, dict, None]) -> dict:
        if raw_query is not None and isinstance(raw_query, str):
            split_qry = raw_query.replace('\n', '').split(',')
            qry_dict = {x.split(':')[0].strip(): x.split(':')[1].strip()
                        for x in split_qry}
    ...
    def get_certificate(self, cert_path: str) -> Optional[str]:
        """Get certificate for SSL connection."""
        if cert_path.find('https://') == 0:
            resp = requests.get(cert_path)
            cert = resp.content
            temp_dir = tempfile.TemporaryDirectory()
            cert_path_obj = pathlib.Path(temp_dir.name).joinpath(
            ...
            if pathlib.Path(cert_path).exists():
                return cert_path
            else:
                logger.debug("Certificate not found at %s", cert_path)
        return None

    def mysql_connection_string(self, sql_conf: dict) -> URL:
        ...
        if sql_conf.get('query') is not None:
            kwargs['query'] = sql_conf['query']

        return URL(**kwargs)

    def mssql_connection_string(self, sql_conf: dict) -> URL:
        ...
        if sql_conf.get('windows_auth') is False and \
            ...
            raise Exception("user and password required if not using Windows Auth")

        qry = sql_conf.get('query', {})
        driver = qry.get('driver')
        if driver is None:
            driver = self.find_odbc_driver()
        ...
        # Default encryption to no for SQL ODBC Driver 18+
        if sql_conf.get('encrypt') is True:
            qry['encrypt'] = 'yes'
        else:
            qry['encrypt'] = 'no'
    ...
    @staticmethod
    def find_odbc_driver() -> str:
        """Find ODBC Driver Version installed."""
        driver_installed = ''
        try:
            import pyodbc
...
def get_engine(sql_conf: Union[SectionProxy, dict]) -> Engine:
    ...
    if isinstance(sql_conf, SectionProxy):
        windows_auth = sql_conf.getboolean('windows_auth', False)
        port = sql_conf.getint('port', None)
        azure = sql_conf.getboolean('azure', False)
        encrypt = sql_conf.getboolean('encrypt', False)
    elif isinstance(sql_conf, dict):
        windows_auth = sql_conf.get('windows_auth', False)
        port = sql_conf.get('port', None)
        azure = sql_conf.get('azure', False)
        encrypt = sql_conf.get('encrypt', False)
    ...
    conn_dict = {
        ...
        'azure': azure,
        'encrypt': encrypt,
        'ssl_cert': sql_conf.get('ssl_cert')
    }
    if dialect == 'mssql':
...
def mssql_connection_string(sql_conf: dict) -> URL:
    ...
    if matches is not None:
        ...
        raise Exception(f"Invalid ODBC Driver found - {qry['driver']}")
    qry['TrustedServerCertificate'] = qry.get('TrustedServerCertificate', 'yes')
...
class DBWriter:
    ...
    def __init__(self, sql_config: SectionProxy) -> None:
        ...
        self.engine_container = EngineContainer(sql_config)
        self.engine = self.engine_container.engine
