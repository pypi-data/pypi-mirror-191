...
class DjangoMigrationConfig:
	...
	_db_to_drop_tables_from: list = tuple()
	_apps_to_migrate: list = tuple()
	_dbs_to_migrate: list = tuple()
	...
	def __init__(self, env: str, **kwargs):
		...
		if not isinstance(env, str):
			raise Exception( "Invalid or empty environment name.")
		...
		if self.is_local_env:
			...
			self._db_to_drop_tables_from = kwargs.get("db_to_drop_tables_from") or [msb_const.names.DEFAULT_DATABASE_NAME]
