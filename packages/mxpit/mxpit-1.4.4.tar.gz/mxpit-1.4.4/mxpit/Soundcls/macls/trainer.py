...
from mxpit.Soundcls.macls import SUPPORT_MODEL
from mxpit.Soundcls.macls.data_utils.featurizer import AudioFeaturizer
from mxpit.Soundcls.macls.data_utils.reader import CustomDataset
from mxpit.Soundcls.macls.models.ecapa_tdnn import EcapaTdnn
from mxpit.Soundcls.macls.models.panns import CNN6, CNN10, CNN14
from mxpit.Soundcls.macls.utils.logger import setup_logger
from mxpit.Soundcls.macls.utils.utils import dict_to_object, plot_confusion_matrix
...
class PPAClsTrainer(object):
    ...
    def __setup_dataloader(self, augment_conf_path=None, is_train=False):
        # 获取训练数据
        augmentation_config=[{"type": "noise","aug_type": "audio","params": {
                        "min_snr_dB": 10,
                        "max_snr_dB": 50,
                        "repetition": 2,
    ...
    def __train_epoch(self, epoch_id, local_rank, nranks=0):
    ...
    def train(self,
        ...
        for epoch_id in range(last_epoch, self.configs.train_conf.max_epoch):
            ...
            # 训练一个epoch
            self.__train_epoch(epoch_id=epoch_id, local_rank=local_rank,  nranks=nranks)
    ...
    def evaluate(self, resume_model='models/ecapa_tdnn_spectrogram/best_model/', save_matrix_path=None):
        ...
        with torch.no_grad():
            for batch_id, (audio, label) in enumerate(self.test_loader):
                audio = audio.to(self.device)
                label = label.to(self.device).long()
                features = self.audio_featurizer(audio)
