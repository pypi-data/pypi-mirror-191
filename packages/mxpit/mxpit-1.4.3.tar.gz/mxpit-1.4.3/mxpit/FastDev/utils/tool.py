...
# 解析yaml配置文件
class LoadYaml:
    def __init__(self, path,label,batch_size, lr, epoch):
        self.val_txt = os.path.join(path,'Train_data','val.txt')
        self.train_txt = os.path.join(path,'Train_data','train.txt')
        self.names = os.path.join('configs/coco.names')
        self.learn_rate = lr
        self.batch_size = batch_size
        self.milestones = [100, 200, 250]
        self.end_epoch = epoch
        self.input_width = 352
        self.input_height = 352
        self.category_num = len(label)
        self.cpu_cuda = 'cuda'
        self.test = ''
