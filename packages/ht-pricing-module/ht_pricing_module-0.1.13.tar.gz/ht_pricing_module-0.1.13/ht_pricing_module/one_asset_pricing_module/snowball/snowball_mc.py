...
class SnowballMc(OneAssetOptionBase):
    ...
    def __calculate_present_value__(self) -> float:
        ...
        current_date_index = math.ceil(self.param.current_date) + math.ceil(residual_intraday)
    ...
    def mc_snowball(self, notional, spot_price, entrance_price, knock_in_barrier_price, knock_out_barrier_price,
                    floor_rate, margin_rate, bonus_rate, option_type, is_knock_in, riskfree_rate, dividend, vol,
                    residual_intraday, expiry_date, year_base, knock_in_obs, knock_out_obs, price=None):
        ...
        if expiry_date <= 0:
            ...
            ki_flag = phi * (spot_price - ki_adj * knock_in_barrier_price) <= 0
            ...
            ko_flag = phi * (spot_price - ko_adj * knock_out_barrier_price) >= 0
        ...
        intraday = round(1 - residual_intraday, 4) if round(1 - residual_intraday, 4) < 1 else round(1 - residual_intraday, 4) - 1
        if price is None:
        ...
        t_arr = (ki_obs_idx if 0 in ki_obs_idx else np.hstack([0, ki_obs_idx])) - intraday

        assert round(expiry_date - intraday, 6) == round(t_arr[-1], 6)

        ki_mat_flag = np.zeros((len(price), 1), dtype=bool) if len(ki_obs_idx) == 0 else phi * (price[:, ki_obs_idx] - knock_in_barrier_price * ki_adj) <= 0
        ko_mat_flag = np.zeros((len(price), 1), dtype=bool) if len(ko_obs_idx) == 0 else phi * (price[:, ko_obs_idx] - knock_out_barrier_price * ko_adj) >= 0
        ...
        # 敲出
        ko_payoff = dis_factor * notional * (np.where(ko_idx == 0, 0, ko_coupon[ko_idx - 1]) - margin_rate * np.where(ko_idx == 0, 0, np.exp(riskfree_rate * t_arr[ko_obs_idx][ko_idx - 1] / year_base) - 1))
        ...
        ki_no_ko_payoff = ki_no_ko_arr_flag * dis_factor * notional * (np.maximum((floor_rate - 1), np.minimum(phi * (price[:, -1] / entrance_price - 1), 0)) - margin_rate * (comp_factor - 1))
