...
class AggregateStateDescriptor:
    ...
    def __eq__(self, y: bool | int | str) -> Predicate:  # type: ignore[override]
        if not isinstance(y, (bool, int, str)):
            raise TypeError(f"Cannot compare against {type(y).__name__}")
        return Predicate(self.name, PredicateOperator.eq, y)