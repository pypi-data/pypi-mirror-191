...
def calc_lateral_earth_pressures(vstress, layerout):
    vstress = np.asarray(vstress)
    layerout = np.asarray(layerout)
    latstress = np.zeros((vstress.shape[0], 2))
    latstress[:, 0] = calc_active_earth_pressure(
        vstress[:, 3], layerout[:, 0], layerout[:, 1]
    )
    latstress[:, 1] = calc_passive_earth_pressure(
        vstress[:, 3], layerout[:, 0], layerout[:, 1]
    )
    return latstress


def get_lateral_earth_pressure_params(soilprofile, pts):
    """Return a list of soil parameters that can be used for calculation of"""

    # list of soil parameters required for lateral earth pressure computations, phi and c
    keylist = ["phi", "c"]

    return soilprofile.get_params(pts, keylist)


def test_lateral_earth_pressure_01():
    ...
    # compute vertical stresses at the points
    soilprofile.vertical_stresses(pts)
    
    # extract depths and vertical stresses in numpy array
    vstress = np.asarray(
        [[pt.z, pt.total_stress, pt.pore_pressure, pt.effective_stress] for pt in pts]
    )
    
    # soil parameters phi and c for computing lateral earth pressures
    params = get_lateral_earth_pressure_params(soilprofile, pts)
    lateral_stresses = calc_lateral_earth_pressures(vstress, params)
    soil_stresses = np.c_[vstress, lateral_stresses]
    results = np.round(soil_stresses, 1).tolist()
...
def test_lateral_earth_pressure_02():
    # testing: vertical stresses and lateral stresses
...
def test_lateral_earth_pressure_03():
    # testing: vertical stresses and lateral stresses
...
def test_lateral_earth_pressure_04():
    # testing: total stresses and lateral stress
