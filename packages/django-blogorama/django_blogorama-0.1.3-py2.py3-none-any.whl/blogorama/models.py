...
class Post(models.Model):
    ...
    active = models.BooleanField(
        _("Active"), help_text="Auto managed field based on state criteria when saved.", default=False
    )
    ...
    locked = models.BooleanField(_("Locked"), default=False, help_text=_("Users can not add Comments or Replies."))
    hide_responses = models.BooleanField(_("Hide Responses"), default=False, help_text=_("Hide responses in display."))
    ...
    def save(self, **kwargs):
        if self.state == StateChoices.PUBLISHED:
            self.active = True
        else:
            self.active = False
    ...
    def get_absolute_url(self):
        return reverse("blogorama_post", kwargs={"slug": self.slug})
...
class Topic(models.Model):
    ...
    Topics are site specific but more than one can be attached to a single post
    """

    site = models.ForeignKey(Site, on_delete=models.CASCADE, help_text=_("Set which sites this Topic is available on"))
    ...
    author = models.ForeignKey(
        settings.AUTH_USER_MODEL, verbose_name=_("User"), on_delete=models.CASCADE, related_name="post_topics"
    )
    ...
    # def get_absolute_url(self):
    #     return reverse("blogorama_topic_list", kwargs={"slug": self.slug})
...
class Comment(models.Model):
    ...
    solution = models.BooleanField(
        _("Solution"), help_text=_("This will tag a comment as an accepted solution if used."), default=False
    )
    state = models.IntegerField(
        _("State"),
        default=StateChoices.DRAFT,
    ...
    def __str__(self):
        return self.post.title + " Comment: " + self.created.strftime("%m/%d/%Y %H:%M")
...
class PostFlag(models.Model):
    ...
    class Meta:
        verbose_name = _("PostFlag")
        verbose_name_plural = _("PostFlags")
