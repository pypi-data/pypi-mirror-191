...
class JobMonitor():
    ...
    def check_inprocess_jobs(self):
        ...
        print("ATTEMPTING TO COUNT QUEUED JOBS", flush=True)
        queued_jobs = job_tracker.get_jobs('queued', {
            'parent_job_ref': {"$exists": False},
            'child_job_spawning_status': 'failure'
        })
        counter = 0
        for job in queued_jobs:
            counter += 1
        print("THERE WERE THIS MANY QUEUED JOBS THAT HAD CHILD JOB SPAWNING STATUS FAILURE", flush=True)
        print(str(counter))
        # self.attempt_to_resolve_jobs(queued_jobs, current_time)
    
    def attempt_to_resolve_jobs(inprocess_jobs, current_time):
