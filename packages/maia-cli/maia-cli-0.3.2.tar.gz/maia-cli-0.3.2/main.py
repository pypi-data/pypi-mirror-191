...
def main():
    ...
    while True:
        ...
        if action == "upload":
            bucket_name = click.prompt("Bucket name", type=str, default=config.get('project'))
            source_file_name = click.prompt(f"Local Path", type=str)
            destination_blob_name = click.prompt(f"Blob Path ('{config.get('bucket_name')}/<example.py>')", type=str)
            ...
            source_blob_name = click.prompt(f"Blob Path, ('{config.get('bucket_name')}/<example.py>')", type=str)
