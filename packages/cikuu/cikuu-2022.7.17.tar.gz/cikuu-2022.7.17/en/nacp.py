...
def run(infile, host='sntvec.wrask.com' if platform.system() in ('Windows') else '172.17.0.1', port=3309, db='nacp',):
	...
	print ("started:", infile ,  ' -> ',  name, host, port, db, flush=True)
	...
	fire.cursor.execute(f"CREATE TABLE if not exists {name}(name varchar(64) COLLATE latin1_bin not null, attr varchar(64) COLLATE latin1_bin not null, count int not null default 0, per float not null default 0, primary key(name,attr), KEY `attr` (`attr`) ) engine=myisam  DEFAULT CHARSET=latin1 COLLATE=latin1_bin") # not null default ''
