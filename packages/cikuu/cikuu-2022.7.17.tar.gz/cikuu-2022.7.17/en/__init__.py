...
postag_func_kp = {  # doc,start, end, tag 
	"prmods": lambda d,s,e,tag:d.user_data.update({f"{tag}:{d[s].lemma_} {d[s+1:e].text.lower()}[{s},{e})":
		{"nac": [d[e-1].lemma_ + f":VERB:prmods|" + d[s].lemma_ + " " + d[s+1:e-1].text, d[e-1].lemma_ + f":VERB|prmods"], "tags": ' '.join([t.tag_ for t in d[s:e]]) }}) , 
	"VP": lambda d,s,e,tag: d.user_data.update({f"{tag}:{d[s].lemma_} {d[s+1:e].text.lower()}[{s},{e})":
		{"nac": [d[s].lemma_ + f":{d[s].pos_}:VP|" + d[s].lemma_ + " " + d[s+1:e].text, d[s].lemma_ + f":{d[s].pos_}|VP"],  "tags": ' '.join([t.tag_ for t in d[s:e]])}}) , 
	"PP": lambda d,s,e,tag: d.user_data.update({f"{tag}:{d[s].lemma_} {d[s+1:e].text.lower()}[{s},{e})":
		{"nac": [d[e-1].lemma_ + f":{d[e-1].pos_}:PP|" + d[s:e].text, d[e-1].lemma_ + f":{d[e-1].pos_}|PP"], "tags": ' '.join([t.tag_ for t in d[s:e]])}}) ,
	...
	"vtv": lambda d,s,e,tag:d.user_data.update({ f"{tag}:{d[s].lemma_} {d[s+1:e].text.lower()}[{s},{e})":
		{"nac": [f"{d[s].lemma_}:VERB:vtv|{d[s].lemma_} {d[s+1:e].text}", f"{d[s].lemma_}:VERB:vtv|_{d[s].lemma_} {d[s+1:e-1].text} _VERB", f"{d[s].lemma_}:VERB|vtv", f"{d[e-1].lemma_}:VERB|~vtv"]}} ) ,
	"vg": lambda d,s,e,tag: d.user_data.update({ f"{tag}:{d[s].lemma_} {d[s+1:e].text.lower()}[{s},{e})":
		{"nac": [f"{d[s].lemma_}:VERB:vg|{d[s].lemma_} {d[e-1].text}", f"{d[s].lemma_}:VERB:vg|_{d[s].lemma_} _VBG", f"{d[s].lemma_}:VERB|vg"]}}) , 
...
skenp_func_kp	= lambda doc, start, end, tag: doc.user_data.update({f"{tag}:{doc[start].lemma_} " + span_NP(doc[start+1:end]) : {"vlem":doc[start].lemma_, "start":start, "end":end, "nac": [f"{doc[start].lemma_}:VERB:{tag}|" +span_NP(doc[start:end]), f"{doc[start].lemma_}:VERB|{tag}", f"*:VERB|{tag}"]} })
...
trp3_func_kp = { #drink:VERB:dobj:NOUN:water:amod:ADJ safe
	...
	"vpN":			lambda doc, i, j, k, tag:  doc.user_data.update({f"vpN:{doc[i].lemma_} {doc[j].text} {doc[k].text}[{i},{j},{k}]":{"tag":tag, "from":"trp3", # jump over the dog / come into force/vpn 
						"nac":[f"{doc[i].lemma_}:{doc[i].pos_}:vpN|{doc[k].lemma_}",f"{doc[i].lemma_}:{doc[i].pos_}|vpN",f"{doc[k].lemma_}:{doc[k].pos_}:vpN|{doc[i].lemma_} {doc[j].lemma_}",f"{doc[k].lemma_}:{doc[k].pos_}|vpN"]}}), 
	}
...
def kp_born(doc, base:bool=True):
	...
	if base:
		[ doc.user_data.update({f"{t.pos_}:{t.lemma_}[{t.i}]": {"type":"tok", 
				"nac":[f"{t.pos_}|{t.lemma_}", f"{t.lemma_}|{t.pos_}", f"*|{t.pos_}", f"{t.lemma_}:{t.pos_}|{t.tag_}",f"*:{t.pos_}|{t.tag_}"]}, 
		...
		[ doc.user_data.update({f"{t.dep_}-{t.head.pos_}-{t.pos_}:{t.head.lemma_} {t.lemma_}[{t.head.i},{t.i}]": {"type":"trp",
			...
			for t in doc if t.pos_ in ("NOUN","VERB","ADJ","ADV","ADP") and t.dep_ not in ('ROOT','punct') and t.is_alpha and t.head.is_alpha and t.lemma_ in wordlist  and t.head.lemma_ in wordlist]
...
if __name__	== '__main__': 
	...
	[ print ( kp, item) for kp, item in kp_born(spacy.nlp("I open the door.")).items()]  if platform.system() in ('Windows') else fire.Fire({"sqlsi":sqlsi, "test":test, "es_submit":es_submit, "fasttext":fasttext })
