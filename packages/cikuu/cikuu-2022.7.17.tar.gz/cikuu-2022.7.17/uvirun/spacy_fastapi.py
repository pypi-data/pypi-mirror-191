...
def spacy_show(snt:str="Justice delayed is justice denied.", mergenp:bool= False):
	...
	return [ { 'word': t.text, 'pos':t.pos_, 'tag':t.tag_, 'lemma': t.lemma_,'dep':t.dep_, 'head':t.head.orth_, 'gpos':t.head.pos_, 
	 'n_lefts':t.n_lefts, 'left_edge':t.left_edge.text,  'n_rights':t.n_rights, 'right_edge': t.right_edge.text,
	'subtree': "_".join([t.text for t in t.subtree]), 'children':"_".join([t.text for t in t.children]),
	'ent_type':  t.ent_type_ , 'ent_id':  t.ent_id_,
	}  for t in doc]

@app.get('/spacy/span', tags=["spacy"])
def spacy_span(snt:str="I plan to go swimming."):
	''' 2023.2.7 '''
	import en
	doc = spacy.nlp(snt)
	en.tag_span(doc)
	return [ dict(item, **{"kp":kp}) for kp, item in doc.user_data.items() ]
...
def spacy_es(text:str="I plan to go swimming. Justice delayed is justice denied.", filename:str=None):
	...
	tdoc = spacy.nlp(text)
	return [ {"filename":filename, "sid":sid, "snt":sp.text.strip(), "source":doc_to_actions(f"{filename}-{sid}", sp.as_doc() )} for sid, sp in enumerate(tdoc.sents) ]
