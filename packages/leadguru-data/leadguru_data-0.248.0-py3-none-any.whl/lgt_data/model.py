...
class UserLeadModel(LeadModel):
    ...
    @staticmethod
    def to_csv(lead: UserLeadModel, board_name: str) -> List[str]:
        return [lead.message.source, lead.message.profile.get_name(), lead.message.profile.title,
                lead.message.profile.company, lead.message.profile.location, lead.message.profile.email,
                lead.notes, board_name, lead.status,
                lead.followup_date.strftime("%d.%m.%Y %H:%M") if lead.followup_date else "",
                lead.message.message.replace('\n', ' ').strip()]
