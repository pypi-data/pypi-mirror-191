...
# Read first 128kB of binary shotfile
        ...
        if len(byt_str) < 128:
            logger.error('Error: shotfile %s has < 128 bytes, ignored', self.properties.file)
...
class SF_OBJECT:
    ...
    def getData(self, nbeg=None, nend=None):
        ...
        if self.objectType in (oid.ParamSet, oid.Device):
            ...
            self.getObject(nbeg=None, nend=None)
    ...
    def getObject(self, nbeg=None, nend=None):
...
if __name__ == '__main__':
    ...
    nshot = 28053
    diag = 'CEZ'
    cez = SF_READ(diag, nshot)
    print(cez.properties.time)
    print(cez.Ti.phys_unit)
    print(cez.properties.parsets)
    print(cez.LOSInfo.__dict__)
    cez.Ti.getData()
    titim = cez.Ti.timebase
    titim.getData()
    tiarea = cez.Ti.areabase
    tiarea.getData()
    print(cez.Ti.data[0, :])
    print(titim.data[:50])
    print(tiarea.data.shape)
    print(cez.Ti.areabase.objectName)
    print(cez.Ti.timebase.objectName)
