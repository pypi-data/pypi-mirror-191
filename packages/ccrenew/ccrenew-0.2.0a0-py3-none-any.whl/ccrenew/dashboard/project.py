...
class Project:
    ...
    def _update_bool(self):
        ...
        sensor_df = self.df.query("index.dt.date > @self.publish_date").reindex(columns=sensor_cols).copy()

        self.df_sensors = sensor_df[sensor_cols].copy()
        df_weather = self.df_sensors[weather_cols]
        df_poa = self.df_sensors[poa_cols]
        ...
        band_pass = det.band_pass(self.df_sensors,
                                            [(temp_cols, (-30, 120)),
                                             (wind_cols, (0, 67)),
                                             (poa_cols, (0, 1800))]
                                            )
        ...
        if self.df_bool.empty:
            ...
            self.df_bool = self.df_bool.reindex(columns = df_bool.columns)
        ...
        while any_nulls:
            for neighbor_name, df_neighbor_avg in neighbor_sensor_avgs.items():
                ...
                if self.neighbor_list[neighbor_name]:
                    # Use POA data
                    df_sensors_avg.loc[df_sensors_avg['POA_avg'].isnull(), 'POA_source'] = neighbor_name
            ...
            if get_solcast and self.df_solcast.empty:
                ...
                df_solcast = df_solcast.rename(columns={'Tamb': 'Tamb_avg', 'Tmod': 'Tmod_avg', 'poa': 'POA_avg'})
                df_sensors_avg.loc[df_sensors_avg['POA_avg'].isnull(), 'POA_source'] = 'Solcast'
                df_sensors_avg = df_sensors_avg.fillna(df_solcast)
                
                # Store a copy of the Solcast df in the object
                self.df_solcast = df_solcast.copy()
