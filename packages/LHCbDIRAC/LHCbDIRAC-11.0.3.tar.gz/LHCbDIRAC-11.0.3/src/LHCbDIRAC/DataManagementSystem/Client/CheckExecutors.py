...
def _dumpErrorAndFiles(title, lfnList, maxFiles, dumpStr, fileName, fp):
    ...
    gLogger.notice(f"Whole list available with 'grep {dumpStr1} {fileName}'")
    return fp
...
def doCheckFC2SE(cc, bkCheck=True, fixIt=False, replace=False, maxFiles=None, fixOption="FixIt"):
    ...
    if cc.inSEbutNotInFC:
        ...
        for lfn, seList in cc.inSEbutNotInFC.items():
            ...
            gLogger.notice("\t{} ({}): {}".format(lfn, "in BK" if inBK else "not in BK", ",".join(seList)))
            if fixIt:
                if inBK:
    ...
    if cc.existLFNsBKRepNo:
        gLogger.notice(">>>>")
        affectedRuns = {str(run) for run in cc.existLFNsBKRepNo.values() if run}
    ...
    if cc.existLFNsNoSE:
        ...
        seOK = False
        nOfExistLFNsNoSE = len(cc.existLFNsNoSE)
        title = f"{nOfExistLFNsNoSE} file{'' if nOfExistLFNsNoSE == 1 else 's'}"
        title += f" {'is' if nOfExistLFNsNoSE == 1 else 'are'} in the BK and FC"
        title += f" but {'is' if nOfExistLFNsNoSE == 1 else 'are'} missing at some SEs"
        ...
        if fixIt:
            ...
            gLogger.notice(f"Use --{fixOption} to fix it ({fixStr})")
        gLogger.notice("<<<<")
    ...
    if cc.existLFNsBadReplicas:
        ...
        badChecksum = ["{} @ {}".format(lfn, ",".join(sorted(se))) for lfn, se in cc.existLFNsBadReplicas.items()]
...
def doCheckFC2BK(cc, fixFC=False, fixBK=False, listAffectedRuns=False, checkFC2SE=True):
    ...
    if cc.existLFNsBKRepNo:
        ...
        affectedRuns = list({str(run) for run in cc.existLFNsBKRepNo.values()})
        nOfExistLFNsBKRepNo = len(cc.existLFNsBKRepNo)
        errMsg = f"{nOfExistLFNsBKRepNo} file{'' if nOfExistLFNsBKRepNo == 1 else 's'} "
        errMsg += f"{'is' if nOfExistLFNsBKRepNo == 1 else 'are'} in the FC but have replica = NO in BK"
        gLogger.error(errMsg)
        if checkFC2SE:
            from LHCbDIRAC.DataManagementSystem.Client.ConsistencyChecks import ConsistencyChecks

            ccAux = ConsistencyChecks()
            gLogger.notice(f"====== Now checking {nOfExistLFNsBKRepNo} files from FC to SE ======")
        ...
        if cc.existLFNsBKRepNo:
            msg = f"====== Completed, {nOfExistLFNsBKRepNo} file{'s' if nOfExistLFNsBKRepNo == 1 else 's'} "
            msg += f"{'is' if nOfExistLFNsBKRepNo == 1 else 'are'} in the FC and SE but have replica = NO in BK ======"
            gLogger.notice(msg)
            if fp is None:
                fp = open(fileName, "wt")
            ...
            filesInvisible = {lfn for lfn, meta in success.items() if meta["VisibilityFlag"][0].upper() == "N"}
            ...
            if not gLogger.info(
                "\n".join(
                    "{} : Visi {}".format(lfn, success.get(lfn, {}).get("VisibilityFlag", "?"))
