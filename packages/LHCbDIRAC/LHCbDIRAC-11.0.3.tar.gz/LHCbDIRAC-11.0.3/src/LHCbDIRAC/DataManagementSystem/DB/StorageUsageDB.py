...
class StorageUsageDB(DB):
    ...
    def publishDirectories(self, directoryDict):
        ...
        for path, pathInfo in directoryDict.items():
            ...
            if path in dirIDs:
                ...
                if not result["OK"]:
                    self.log.error("Cannot update directory", "{}: {}".format(path, result["Message"]))
                    return result
            else:
                ...
                result = self._update(sqlCmd)
                if not result["OK"]:
                    self.log.error("Cannot insert directory", "{}: {}".format(path, result["Message"]))
                    continue
                dirIDs[path] = result["lastRowId"]
            result = self.__updateSEUsage(dirIDs[path], seUsage, path)
            if not result["OK"]:
                self.log.error("Cannot update SE usage", "{}:{}: {}".format(path, seUsage, result["Message"]))
                return result
        return S_OK()
    ...
    def publishToProblematicDirs(self, directoryDict):
        ...
        for path, pathInfo in directoryDict.items():
            ...
            if did:
                ...
                result = self._update(sqlCmd)
                if not result["OK"]:
                    self.log.error("Cannot update row ({}, {}) in problematicDirs".format(path, site))
                    return result
            else:
                ...
                result = self._update(sqlCmd)
                if not result["OK"]:
                    self.log.error("Cannot insert row ({}, {}) in problematicDirs".format(path, site))
                    return result
        return S_OK()
    ...
    def publishToSEReplicas(self, directoryDict):
        ...
        for path, pathInfo in directoryDict.items():
            ...
            if sqlRes:
                ...
                result = self._update(sqlCmd)
                if not result["OK"]:
                    self.log.error("Cannot update row ({}, {}) in se_Usage".format(path, seName))
                    return result
            else:
                # entry is not there, make an insert of a new row
                ...
                result = self._update(sqlCmd)
                if not result["OK"]:
                    self.log.error("Cannot insert row ({}, {}) in se_Usage".format(path, seName))
                    return result
        return S_OK()
    ...
    def __updateSEUsage(self, dirID, seUsage, path):
        ...
        if not seUsage:
            ...
            self.log.warn("Ooops. Dir has no SEUsage!", "ID {}, path {}".format(dirID, path))
            return S_OK()
        sqlVals = []
        # Insert data
    ...
    def removeDirectory(self, dirPath):
        ...
        for tableName in ("su_Directory", "su_SEUsage"):
            sqlCmd = "DELETE FROM `{}` WHERE DID in ( {} )".format(tableName, sqlIDs)
            result = self._update(sqlCmd)
            if not result["OK"]:
                return result
        return S_OK()
    ...
    def purgeOutdatedEntries(self, rootDir=False, outdatedSeconds=86400, preserveDirsList=None):
        ...
        while True:
            ...
            for tblName in ("su_Directory", "su_SEUsage"):
                result = self._update("DELETE FROM `{}` WHERE DID IN ( {} )".format(tblName, sqlIdList))
                if not result["OK"]:
                    return result
    ...
    def getUserStorageUsage(self, userName=False):
        ...
        if userName:
            ...
            sqlCond.append("d.Path LIKE '/lhcb/user/{}/{}/%".format(userName[0], userName))
        else:
    ...
    def getUserSummaryPerSE(self, userName=False):
        ...
        if userName:
            sqlCond.append("{} = {}".format(sqlUser, self._escapeString(userName)["Value"]))
    ...
    def publishTose_STSummary(self, site, spaceToken, totalSize, totalFiles, storageDumpLastUpdate):
        ...
        if not res["OK"]:
            return S_ERROR(res)
        if not res["Value"]:
            self.log.warn("Entry for site={}, spaceToken={} is not there => insert new entry ".format(site, spaceToken))
            sqlCmd = (
            ...
            self.log.verbose("Entry for site={}, spaceToken={} is there => update entry ".format(site, spaceToken))
            sqlCmd = (
        ...
        result = self._update(sqlCmd)
        if not result["OK"]:
            self.log.error("Cannot insert entry", "{},{}: {}".format(sqlSite, sqlSpaceToken, result["Message"]))
            return result
        return S_OK()
    ...
    def getUserSummary(self, userName=False):
        if userName:
            userName = self._escapeString(userName)["Value"][1:-1]
            path = "/lhcb/user/{}/{}".format(userName[0], userName)
        else:
            path = "/lhcb/user/_/%"
    ...
    def sendDataUsageReport(self, site, directoryDict, status="New"):
        ...
        self.log.verbose("in addPopCount: dirDict: {} site: {}".format(directoryDict, site))
        if not directoryDict:
            return S_OK()
        sqlSite = self._escapeString(site)["Value"]
        insertedEntries = 0
    ...
    def updatePopEntryStatus(self, idList, newStatus):
        ...
        sqlCmd = "UPDATE `Popularity` SET Status={} WHERE ID IN ( {} )".format(sqlStatus, sqlIdList)
        result = self._update(sqlCmd)
        if not result["OK"]:
            return result
        return result
