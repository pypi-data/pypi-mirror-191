    ...
    for transID in transList:
        ...
        if transName:
            ...
            if tName:
                ...
                if not res["OK"]:
                    gLogger.error("Error setting transformation name", "as {} : {}".format(tName, res["Message"]))
                else:
        ...
        for key, val in pluginSEParams.items():
            ...
            if not res["OK"]:
                gLogger.error("Error setting SE parameter", "{} = {} : {}".format(key, val, res["Message"]))
            else:
        ...
        for key, val in pluginParams.items():
            ...
            if not res["OK"]:
                gLogger.error("Error setting additional parameter", "{} = {} : {}".format(key, val, res["Message"]))
            else:
                gLogger.notice("Successfully set parameter", f"{key} = {val}")
