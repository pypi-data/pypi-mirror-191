...
class DataRecoveryAgent(AgentModule):
        ...
        for transStatus in transformationStatus:
            ...
            if not result["OK"]:
                self.log.error(
                    ...
                    "Status '{}': {}".format(transStatus, result["Message"]),
                )
                return result
        ...
        for transformation, typeName in transformationDict.items():
            ...
            if not result["OK"]:
                self.transLogger.error(
                    "Could not select files for transformation", "{}: {}".format(transformation, result["Message"])
                )
                continue
    ...
    def __removePendingRequestsJobs(self, jobFileDict):
        ...
        for jobID, requestID in result["Value"]["Successful"].items():
            ...
            if not res["OK"]:
                self.transLogger.error("Failed to get Status for Request", "{}:{}".format(requestID, res["Message"]))
            elif res["Value"] != "Done":
