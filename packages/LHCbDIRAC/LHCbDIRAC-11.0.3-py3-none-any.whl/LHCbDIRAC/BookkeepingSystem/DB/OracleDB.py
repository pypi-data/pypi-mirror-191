...
class OracleDB:
    ...
    def __init__(self, userName, password="", tnsEntry="", confDir="", mode="", maxQueueSize=100):
    ...
    def __newConnection(self):
        ...
        if not gModeFixed:
            ...
            if self.__mode == "":
                try:
                    connection = oracledb.connect(
                        user=self.__userName, password=self.__passwd, dsn=self.__tnsName, config_dir=self.__confDir
                    )
                except Exception as ex:
            ...
            if self.__mode == "Thick":
                oracledb.init_oracle_client(config_dir=self.__confDir)
