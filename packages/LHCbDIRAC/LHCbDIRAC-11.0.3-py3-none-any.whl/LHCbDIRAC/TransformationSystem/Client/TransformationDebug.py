            ...
            if task["ExternalStatus"] == "Submitted":
                ...
                if self.kickRequests:
                    ...
                    if res["OK"]:
                        ...
                        prString += " - Failed to set task {} ({})".format(newStatus, res["Message"])
                else:
    ...
    def __checkJobs(self, jobsForLfn, byFiles=False, checkLogs=False):
        ...
        for lfnStr, allJobs in jobsForLfn.items():  # can be an iterator
            ...
            gLogger.notice(
                "Sites (CPU):",
                ", ".join(
                    "{} ({})".format(jobSites.get(int(job), "Site unknown"), jobCPU.get(int(job), "CPU unknown"))
                    for job in allJobs
                ),
            )
            prevStatus = None
            ...
            for runDict in runsDictList:
                ...
                for taskID in sorted(taskList) if taskList else sortedTasks:
                    ...
                    if byTasks:
                        ...
                        prString += "- {}: {} - Status: {}".format(taskType, task["ExternalID"], task["ExternalStatus"])
                        if targetSE:
