...
async def get_multiple_choices(request: web.Request, result: Optional[Union[DesktopObjectDict, List[DesktopObjectDict]]]) -> web.Response:
    ...
    if result is not None:
        ...
        response.etag = etag
        response.update(NO_CACHE_HEADERS)
        return response
    else:
        return web.HTTPNotFound(headers=NO_CACHE_HEADERS)
...
async def get_streaming(request: web.Request, out: SupportsAsyncRead, content_type: str = 'application/json') -> web.StreamResponse:
    ...
    response_.update(NO_CACHE_HEADERS)
    return response_
...
async def _handle_get_result_from_wstl(request: web.Request,
    ...
    if run_time_docs is not None:
        ...
        if request.if_none_match and etag in request.if_none_match:
            return web.HTTPNotModified()
        response = status_ok(body=body, content_type=representor.MIME_TYPE)
        response.etag = etag
        response.update(NO_CACHE_HEADERS)
        return response
    else:
