...
class Scenario:
  ...
  def send_msg(self, msg, nis=False):
    if nis:
      self.nis = True
    j = {"send": msg}
    self._decide(j)

  def send_as_user(self, msg, nis=False):
    if nis:
      self.nis = True
    j = {"send_as_user": msg}
    self._decide(j)

  def send_status(self, msg_id, msg, nis=False):
    if nis:
      self.nis = True
    j = {"send_status": {"id": msg_id, "msg": msg}}
    self._decide(j)

  def send_error(self, msg, nis=False):
    if nis:
      self.nis = True
    j = {"send_warning": msg}
    self._decide(j)

  def store_cells(self, values_dict, nis=False):
    if nis:
      self.nis = True
