...
class ExcelFormat(Format):
    ...
    def open_file(self):
        ...
        filemgr.makedirs(filemgr.dirname(self.workbook_path), exist_ok=True)
        return filemgr.open_file(self.workbook_path, mode='wb', newline=self.newline, encoding=self.encoding)
    ...
    def recreate_table(self, new_ref):
        ...
        # Copy columns from previous table so that we keep formulas and styles (in particular dataDxfId, referring to workbook._differential_styles)
        max_id = 0
        for i in range(0, self.last_col_index - self.first_col_index + 1):
            if i < len(self.previous_table.tableColumns):
                previous_column = self.previous_table.tableColumns[i]
                if previous_column.id > max_id:
                    max_id = previous_column.id 
                newcolumns.append(previous_column)
            else:
                name = self.worksheet.cell(self.first_row_index, self.first_col_index + i).value
                newcolumns.append(TableColumn(id=max_id+1, name=name))
                max_id += 1

        newtable = Table(name=self.table_name, displayName=self.table_name, ref=new_ref, tableColumns=newcolumns)
