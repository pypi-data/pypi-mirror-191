...
__all__ = ['Driver', 'get_driver', 'WildberriesCategoryScrapper', 'get_category_df_from_page', 'update_page_num', 'run',
           ...
           'WildberriesMultipleItemScrapper', 'run', 'ShopstatSingleItemScrapper', 'authorize', 'run',
           'ShopstatMultipleItemScrapper', 'run']
...
def get_category_df_from_page(self, page_url):
    ...
    # WAIT THE PAGE TO UPLOAD UP TO 10 SECS:
...
def run(self,
    ...
    dfs = []
    for page_num in tqdm(range(1, n+1)):
        # try to scrap page, except - pass it:
        try:
            start_time = time()
            ...
            logger.info(f"CPU: {psutil.cpu_percent():.0f}%, RAM: {psutil.virtual_memory().percent:.0f}%, TIMEDELTA: {timedelta:.1f}")
        except:
...
class WildberriesSingleItemScrapper:
    def __init__(self, driver, code):
        ...
        # wait page up to 10 secs to upload:
        driver.get(item_page_url)
        WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.CLASS_NAME, "product-params__caption"))) # changed here
        ...
        if self.feedbacks_qty != 0:
            ...
            # wait page up to 10 secs to upload:
            WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.CLASS_NAME, "sorting__selected")))
            ...
            # wait page up to 10 secs to upload:
            WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.CLASS_NAME, "feedback__info")))
...
def run(self, code_list, postgres_creds, save_to_postgres=False):
    ...
    for batch in tqdm(batches, desc='Running batches'):
        ...
        # at each batch - create new driver and authorize:
        driver = Driver(executable_path=self.executable_path).get_driver()
        ssis = ShopstatSingleItemScrapper(driver, self.email, self.password)
        ssis.authorize()

        # within created authorization - scrap batch:
        for code in batch:
            try:
        ...
        # after batch is scrapped - close driver:
        driver.close()
