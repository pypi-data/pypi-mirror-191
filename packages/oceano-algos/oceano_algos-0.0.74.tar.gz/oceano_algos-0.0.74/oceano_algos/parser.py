...
def сheck_if_exists(self, code):
    ...
    existance = bs.find('div', {'class':'------src-components-common-ui-TableAdaptive__inner--w3Au6'})==None
...
def run(self, code_list, postgres_creds, save_to_postgres=False):
    ...
    for batch in tqdm(batches, desc='Running batches'):
        ...
        # drop codes which don't exist on shopstat.ru:
        cleaned_batch = []
        for code in batch:
            if ssis.сheck_if_exists(code):
                cleaned_batch.append(code)


        # scrap only items that exist on shopstat.ru:
        for code in cleaned_batch:
            try:
                item_df = ssis.run(code)
                dfs.append(item_df)
            except:
                logger.info(f"FAIL: can't scrap code {code} though it exists at shopstat.ru. Passing it.")
