...
import argparse
import pyperclip
from app.gui import choose_file
import os
import sys
...
def get_setup_path():
    try:
        # Get the path to the current package (clipboard-files)
        dist = pkg_resources.get_distribution('clipboard-files')
        ...
        # If the package is not installed, return None
        return None

def main():
    setup_path = get_setup_path()
    if setup_path is None:
        print('File not found: setup.py!')
        sys.exit(1)
