...
from sarus.typing import MOCK, PYTHON_TYPE, Client
...
from .dataspec_api import (
    compile_dataspec,
    dataspec_status,
    get_dataspec,
    launch_dataspec,
    pull_dataspec_graph,
    pull_dataspec_status_graph,
    push_dataspec_graph,
)
...
class SDKManager(DelegatingManager):
    ...
    def is_delegated(self, dataspec: st.DataSpec) -> bool:
        ...
        return get_dataspec(self.client(), dataspec.uuid()) is not None
    ...
    def _post_dataspec(
        ...
        alt_dataspec_uuid, privacy_policy = compile_dataspec(
            self.client(), dataspec.uuid()
        )
        pull_dataspec_graph(self.client(), alt_dataspec_uuid)
        if execute:
            launch_dataspec(self.client(), alt_dataspec_uuid)
        ...
        **named_arguments: st.DataSpec,
        ...
        if transform.is_external():
            ...
            if transform.protobuf().spec.HasField("select_sql"):
                ...
                push_dataspec_graph(self._client, referrables)
            return sp.type_name(sp.Dataset), attach_info
    ...
    def dot(
        ...
        if remote:
            statuses_proto = pull_dataspec_status_graph(
