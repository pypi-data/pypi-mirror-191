...
def get_dataspec(client: Client, uuid: str) -> t.Optional[st.DataSpec]:
    ...
    proto = dict_deserialize(resp.json())
...
def compile_dataspec(
    ...
    return resp.json()["uuid"], resp.json()["privacy_policy"]
...
def dataset_result(
    ...
    if resp.headers.get("Content-Type") == "application/parquet":
        ...
        async def arrow_iterator_from_response():
            with gzip.open(resp.raw, "rb") as source:
