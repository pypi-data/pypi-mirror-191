...
from crop_coach.envs.generate_site_cab import generate_site_cab
from crop_coach.envs.generate_soil_cab import generate_soil_cab
...
class Wofost:
    @staticmethod
    def init_wofost(site_dict, soil_dict,crop_name, crop_variety, files_paths : dict,latitude: float = 51.97, longitude: float = 5.67):
        ...
        if files_paths == None:
            for file in cabo_files:
                ...
                data_["site"] = read_cabo_file(generate_site_cab(site_dict["gene_site_dict"], site_dict["n_site_dict"], site_dict["p_site_dict"], site_dict["k_site_dict"]))
                data_["soil"] = read_cabo_file(generate_soil_cab(soil_dict["water_retention_dict"]), generate_soil_cab(soil_dict["hydraulic_conductivity_dict"]), generate_soil_cab(soil_dict["workability_parameters_dict"]))
        else:
            for file in cabo_files:
