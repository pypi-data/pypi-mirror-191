    ...
    def init_wofost(crop_name, crop_variety, files_paths : dict,latitude: float = 51.97, longitude: float = 5.67, **kwargs):
        ...
        if files_paths == None:
            for file in cabo_files:
                data_[file] = read_cabo_file(os.path.join(os.path.dirname(__file__),"..","default_data", file, file+".cab"))
            ...
            for file in cabo_files:
                if check_file_path(files_paths[file]) :
                    ...
                    data_[file] = read_cabo_file(fpath=os.path.join(os.path.dirname(__file__),"..","default_data", file, file+".cab"))
                    # data_["crop"] = YAMLCropDataProvider(repository = "https://raw.githubusercontent.com/ajwdewit/WOFOST_crop_parameters/master/")
                    # data_["crop"].set_active_crop(crop_name, crop_variety)
